...
def upgrade():
    ...
    for (_id, resource_id, _, action, content, _,
        ...
        if action == 'deleted':
            ...
            if user_role_person_id:
                ...
                rev_result = connection.execute(
                    revisions_table.select(revisions_table).where(
                        and_(revisions_table.columns.action == 'deleted',
                             revisions_table.columns.resource_type ==
                             'UserRole', revisions_table.columns.resource_id ==
        ...
        people_result = connection.execute(people_table.select().where(
            people_table.columns.id == person_id)).fetchone()
        if people_result:
            ...
            app.logger.warning(
                "No person for revision {}, `unknown` will be used".format(
                    _id))
            email = 'unknown'
