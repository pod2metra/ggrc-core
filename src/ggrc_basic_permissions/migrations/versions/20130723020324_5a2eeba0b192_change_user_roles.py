...
def upgrade():
    ...
    op.execute(
        'DELETE FROM user_roles WHERE user_email NOT IN (SELECT email FROM people)'
    )
    op.execute(
        'UPDATE user_roles SET person_id = (SELECT id FROM people WHERE people.email = user_roles.user_email)'
    )
...
def downgrade():
    ...
    op.add_column('user_roles',
                  sa.Column(
                      u'user_email', sa.String(length=128), nullable=False))
    op.execute(
        'UPDATE user_roles SET user_email = (SELECT email FROM people WHERE people.id = user_roles.person_id)'
    )
