  ...
  """
    ...
    if rbac_permissions.is_admin():
        if hasattr(settings, 'BOOTSTRAP_ADMIN_USERS'):
            public_config[
                'BOOTSTRAP_ADMIN_USERS'] = settings.BOOTSTRAP_ADMIN_USERS
    ...
    for action, resource_permissions in src_permissions.items():
        ...
        for resource_permission in resource_permissions:
            ...
            if context_id is not None:
                permissions[action][resource_type]['contexts'].append(
                    context_id)
  ...
  """
    ...
    if hasattr(settings, 'BOOTSTRAP_ADMIN_USERS') \
       and user.email in settings.BOOTSTRAP_ADMIN_USERS:
        ...
        collect_permissions(admin_permissions,
...
def load_user_roles(user, permissions):
  ...
  """
    ...
    for user_role in user_roles:
        source_contexts_to_rolenames.setdefault(user_role.context_id,
                                                list()).append(
                                                    user_role.role.name)
        if isinstance(user_role.role.permissions, dict):
            collect_permissions(user_role.role.permissions,
                                user_role.context_id, permissions)
...
def load_access_control_list(user, permissions):
    ...
    access_control_list = db.session.query(
        acl.object_type, acl.object_id, func.max(acr.read),
        func.max(acr.update), func.max(acr.delete)).filter(
            and_(all_models.AccessControlList.person_id == user.id,
                 all_models.AccessControlList.ac_role_id == acr.id)).group_by(
