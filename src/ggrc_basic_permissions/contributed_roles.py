...
from ggrc_basic_permissions.roles import (
    Auditor, AuditorProgramReader, AuditorReader, Editor, ProgramAuditEditor,
    ProgramAuditOwner, ProgramAuditReader, ProgramBasicReader, ProgramCreator,
    ProgramEditor, ProgramMappingEditor, ProgramOwner, ProgramReader, Reader,
    Creator, gGRC_Admin)
...
def get_declared_role(rolename, resolved_roles={}):
    ...
    if rolename in declarations:
        ...
        contribute_role_permissions(role_definition.permissions,
                                    role_contributions)
...
def lookup_contributions(rolename):
    ...
    contributions = {}
    for extension_module in extension_modules:
        ext_contributions = getattr(extension_module, "ROLE_CONTRIBUTIONS",
                                    None)
        if ext_contributions:
            ext_role_contributions = ext_contributions.contributions_for(
                rolename)
    ...
    for extension_module in extension_modules:
        ...
        if ext_implications:
            role_implications.extend(
                ext_implications.implications_for(rolename,
                                                  context_implication))
