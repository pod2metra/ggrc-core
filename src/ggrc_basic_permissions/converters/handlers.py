...
class ObjectRoleColumnHandler(UserColumnHandler):
    ...
    def get_value(self):
        ...
        return "\n".join(
            cache[self.row_converter.obj.context_id][self.role.id])
    ...
    def insert_object(self):
        ...
        for auditor in self.value:
            ...
            if auditor.id in user_roles or any(
                ...
                continue
            user_role = UserRole(
                role=self.reader, context=context, person=auditor)
            db.session.add(user_role)
...
class UserRoleColumnHandler(UserColumnHandler):
    ...
    _allowed_roles = ["Administrator", "Creator", "Editor", "Reader", ""]
    ...
    def remove_current_roles(self):
        ...
        UserRole.query.filter(
            and_(
                ...
                UserRole.person_id == self.row_converter.obj.id)).delete(
