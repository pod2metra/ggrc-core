...
from ggrc_workflows.roles import (WorkflowOwner, WorkflowMember,
                                  BasicWorkflowReader, WorkflowBasicReader,
                                  WorkflowEditor)
...
def update_cycle_dates(cycle):
  ...
  """
    ...
    for ctg in cycle.cycle_task_groups:
        ...
        ctg.next_due_date = _get_min_end_date(ctg.cycle_task_group_tasks)
    ...
    for task_group_object in task_group.task_group_objects:
        ...
        for task_group_task in task_group.task_group_tasks:
            cycle_task_group_object_task = _create_cycle_task(
                task_group_task, cycle, cycle_task_group, current_user)
            Relationship(
                source=cycle_task_group_object_task, destination=object_)
        ...
        if workflow.is_old_workflow:
            ...
            for task_group_task in task_group.task_group_tasks:
                ...
                for task_group_object in task_group.task_group_objects:
                    object_ = task_group_object.object
                    Relationship(
                        source=cycle_task_group_object_task,
                        destination=object_)
...
def update_cycle_task_child_state(obj):
  ...
  """
    status_order = (None, 'Assigned', obj.IN_PROGRESS, 'Declined', 'Finished',
                    'Verified')
    status = obj.status
...
def update_cycle_task_tree(objs):
    ...
    task_ids = [
        t.id for t in db.session.deleted
    ...
    if task_ids:
        query = query.filter(
            models.CycleTaskGroupObjectTask.id.notin_(task_ids))
    ...
    for group in groups_dict.itervalues():
        old_state = [
            group.status, group.start_date, group.end_date, group.next_due_date
        ]
        _update_parent_status(group,
                              {t.status
                               for t in group_task_dict[group]})
        group.start_date, group.end_date = _get_date_range(
            group_task_dict[group])
        group.next_due_date = _get_min_end_date(group_task_dict[group])
        if old_state != [
                group.status, group.start_date, group.end_date,
                group.next_due_date
        ]:
...
def update_cycle_task_group_parent_state(objs):
    ...
    ).options(orm.undefer_group(
        "CycleTaskGroup_complete")).distinct().with_for_update().all()
    for group in groups:
    ...
    for cycle in cycles_dict.itervalues():
        ...
        _update_parent_status(cycle,
                              {g.status
                               for g in cycle_groups_dict[cycle]})
...
def start_end_date_validator(tgt):
    ...
    if max(tgt.start_date.isoweekday(),
        ...
        if workflow.unit == workflow.DAY_UNIT:
            raise ValueError(
                "Daily tasks cannot be started or stopped on weekend")
...
def handle_task_group_task_delete(sender, obj=None, src=None, service=None):  # noqa pylint: disable=unused-argument
    ...
    task_group.task_group_tasks = [
        t for t in task_group.task_group_tasks if t.id != obj.id
    ]
...
def handle_task_group_delete(sender, obj=None, src=None, service=None):  # noqa pylint: disable=unused-argument
    ...
    workflow.task_groups = [
        t for t in obj.workflow.task_groups if t.id != obj.id
    ]
...
def handle_cycle_object_status(sender,
                               obj=None,
                               src=None,
                               service=None,
                               event=None,
...
def handle_cycle_task_group_object_task_post(sender,
                                             obj=None,
                                             src=None,
                                             service=None):  # noqa pylint: disable=unused-argument
    Signals.status_change.send(
...
def handle_cycle_task_group_put(sender, obj=None, src=None, service=None):  # noqa pylint: disable=unused-argument
    if inspect(obj).attrs.status.history.has_changes():
...
def handle_cycle_put(sender, obj=None, src=None, service=None):  # noqa pylint: disable=unused-argument
...
def _validate_put_workflow_fields(workflow):
    ...
    """
    if (inspect(workflow).attrs.unit.history.has_changes()
            or inspect(workflow).attrs.repeat_every.history.has_changes()):
        raise ValueError("'unit', 'repeat_every' fields are unchangeable")
    if (inspect(workflow).attrs.recurrences.history.has_changes()
            and workflow.recurrences and workflow.unit is None
            and workflow.repeat_every is None):
...
def handle_workflow_put(sender, obj=None, src=None, service=None):
    ...
    if (old, new) == (obj.DRAFT, obj.ACTIVE):
        ...
        if not obj.task_groups:
            raise ValueError(
                "Workflow with no Task Groups can not be activated.")
        build_cycles(obj)
...
def _validate_post_workflow_fields(workflow):
  ...
  """
    if ((workflow.repeat_every is None and workflow.unit is not None)
            or (workflow.repeat_every is not None and workflow.unit is None)):
