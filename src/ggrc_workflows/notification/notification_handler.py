...
def handle_workflow_modify(sender, obj=None, src=None, service=None):
    ...
    pusher.update_or_create_notifications(obj, obj.next_cycle_start_date,
                                          "{}_workflow_starts_in".format(
                                              obj.unit), "cycle_start_failed")
    query = pusher.get_notification_query(*list(
        obj.tasks), **{"notification_names": ["cycle_start_failed"]})
...
def done_tasks_notify(tasks):
  ...
  """
    ...
    task_query = all_models.CycleTaskGroupObjectTask.query.filter(
        ...
        db.Load(all_models.CycleTaskGroupObjectTask).undefer_group(
            "CycleTaskGroupObjectTask_complete")).all()
    for task in task_query:
        cycle_tasks_dict[cycles_dict[task.cycle_id]].append(task)

    done_cycles = [
        cycle for cycle, cycle_tasks in cycle_tasks_dict.iteritems() if all(
            task.is_done for task in cycle_tasks)
    ]
...
def not_done_tasks_notify(tasks):
  ...
  """
    ...
    for task in tasks:
        ...
        if task.status == models.CycleTaskGroupObjectTask.DECLINED:
            notification_send_at_tasks_dict[(
                "cycle_task_declined", datetime.date.today())].append(task)
        ...
        for n_type in [
            ...
            notification_send_at_tasks_dict[(n_type,
                                             task.end_date)].append(task)
    ...
    pusher.get_notification_query(
        *(list(cycles) + tasks), **{
            "notification_names":
            ["all_cycle_tasks_completed"] + list(notification_types)
        }).delete(synchronize_session="fetch")
    notification_type_query = pusher.get_notification_types(
        *notification_types)
    ...
    for pair, pair_tasks in notification_send_at_tasks_dict.iteritems():
        ...
        pusher.create_notifications_for_objects(notification_type, send_at,
                                                *pair_tasks)
...
def handle_cycle_task_group_object_task_put(obj):
    ...
    if inspect(obj).attrs.end_date.history.has_changes() and not obj.is_done:
        ...
        pusher.update_or_create_notifications(obj, obj.end_date,
...
def handle_cycle_modify(obj):
    ...
    if not obj.is_current:
        ...
        pusher.get_notification_query(
            *list(obj.cycle_task_group_object_tasks)).delete(
