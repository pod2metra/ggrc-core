...
def get_notification_query(*objs, **kwargs):
    ...
    query = Notification.query.filter(
        or_(Notification.sent_at.is_(None),
            Notification.repeating == true())).filter(notif_key.in_(keys))
    notification_names = kwargs.get('notification_names') or []
    if notification_names:
        query = query.filter(
            Notification.notification_type_id.in_(
                [n.id for n in get_notification_types(*notification_names)]))
    return query


def notification_exists_for(*obj, **kwargs):
    return db.session.query(get_notification_query(
        *obj, **kwargs).exists()).scalar()


def get_notification_types(*names):
    return NotificationType.query.filter(
        NotificationType.name.in_(names)).all()
...
def update_or_create_notifications(obj, send_at, *notification_names):
    ...
    for notification_name in notification_names:
        ...
        query = get_notification_query(obj).filter(
