...
class WorkflowColumnHandler(handlers.ParentColumnHandler):
    ...
    def __init__(self, row_converter, key, **options):
        ...
        super(WorkflowColumnHandler, self).__init__(row_converter, key,
                                                    **options)
...
class TaskGroupColumnHandler(handlers.ParentColumnHandler):
    ...
    def __init__(self, row_converter, key, **options):
        ...
        super(TaskGroupColumnHandler, self).__init__(row_converter, key,
                                                     **options)
...
class TaskTypeColumnHandler(handlers.ColumnHandler):
    ...
    def parse_item(self):
        ...
        if value is None:
            ...
            if self.raw_value:
                self.add_warning(
                    errors.WRONG_REQUIRED_VALUE,
                    value=self.raw_value,
                    column_name=self.display_name)
            else:
                self.add_warning(
                    errors.MISSING_VALUE_WARNING,
...
class ObjectsColumnHandler(multi_object.ObjectsColumnHandler):
    def get_value(self):
        ...
        lines = [
            "{}: {}".format(t.object._inflector.title_singular.title(),
                            t.object.slug) for t in task_group_objects
    ...
    def insert_object(self):
        ...
        existing = set(
            (t.object_type, t.object_id) for t in obj.task_group_objects)
