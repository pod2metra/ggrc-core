...
def upgrade():
    ...
    for _table_name, _slug_prefix in _tables_prefixes:
        op.add_column(_table_name,
                      sa.Column(
                          _column_name, sa.String(length=250), nullable=True))

        op.execute(
            "UPDATE {table_name} SET slug = CONCAT('{prefix}', id)".format(
                table_name=_table_name, prefix=_slug_prefix))
        ...
        op.create_unique_constraint(_constraint_name, _table_name,
                                    [_column_name])
