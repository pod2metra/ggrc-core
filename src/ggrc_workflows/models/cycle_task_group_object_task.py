  ...
  """
    ...
    _fulltext_attrs = [
        ...
        ft_attributes.MultipleSubpropertyFullTextAttr(
            "comments", "cycle_task_entries", ["description"]),
        "folder",
    ]
    ...
    selected_response_options = db.Column(
        types.JsonType(), nullable=False, default=[])
    ...
    _api_attrs = reflection.ApiAttributes(
        ...
        reflection.Attribute(
            'related_destinations', create=False, update=False),
        ...
        reflection.Attribute(
            'cycle_task_group_title', create=False, update=False),
    )
    ...
    """
        return Cycle.query.filter((Cycle.id == cls.cycle_id) & (
            predicate(Cycle.slug) | predicate(Cycle.title))).exists()
    ...
    """
        ...
        return query.options(
            orm.joinedload('cycle').joinedload('workflow')
    ...
    def indexed_query(cls):
        return super(CycleTaskGroupObjectTask, cls).indexed_query().options(
            ...
            orm.Load(cls).subqueryload("cycle_task_entries").load_only(
                "description", "id"),
            orm.Load(cls).joinedload("cycle").joinedload("workflow")
            .undefer_group("Workflow_complete"),
        )
...
class CycleTaskable(object):
  ...
  """
    ...
    """
        sources = [
            r.CycleTaskGroupObjectTask_source for r in self.related_sources
            if r.CycleTaskGroupObjectTask_source is not None
        ]
        destinations = [
            r.CycleTaskGroupObjectTask_destination
    ...
    def eager_query(cls):
        ...
        return query.options(
            ...
            .joinedload('CycleTaskGroupObjectTask_source').undefer_group(
                'CycleTaskGroupObjectTask_complete').joinedload('cycle'),
            orm.subqueryload('related_destinations')
            .joinedload('CycleTaskGroupObjectTask_destination').undefer_group(
                'CycleTaskGroupObjectTask_complete').joinedload('cycle'))
