...
class Workflow(roleable.Roleable, mixins.CustomAttributable, HasOwnContext,
  ...
  """
    ...
    is_verification_needed = db.Column(
        db.Boolean, default=IS_VERIFICATION_NEEDED_DEFAULT, nullable=False)

    repeat_every = deferred(
        db.Column(db.Integer, nullable=True, default=None), 'Workflow')
    DAY_UNIT = 'day'
    WEEK_UNIT = 'week'
    MONTH_UNIT = 'month'
    ...
    """
        return not any(tg for tg in self.task_groups
                       if not tg.task_group_tasks)
    ...
    """
        ...
        if self.unit == self.MONTH_UNIT:
            ...
            if setup_day == setup_date.day:
                calc_date = datetime.date(calc_date.year, calc_date.month,
                                          calendar.monthrange(
                                              calc_date.year,
                                              calc_date.month)[1])
    ...
    _api_attrs = reflection.ApiAttributes(
        ...
        reflection.Attribute(
            'next_cycle_start_date', create=False, update=False),
        reflection.Attribute('can_start_cycle', create=False, update=False),
        reflection.Attribute(
            'non_adjusted_next_cycle_start_date', create=False, update=False),
    ...
    """
        ...
        target = self.copy_into(
            _other, columns, access_control_list=access_control_list, **kwargs)
        return target
