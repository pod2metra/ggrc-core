...
from ggrc.models.mixins import (Titled, Slugged, Described, Timeboxed,
                                WithContact)
...
class TaskGroup(WithContact, Timeboxed, Described, Titled, Slugged, Indexed,
    ...
    def ensure_assignee_is_workflow_member(self):  # pylint: disable=invalid-name
        ...
        wf_member_role_id = next(
            ind
            for ind, name in role.get_custom_roles_for("Workflow").iteritems()
        ...
        for model_name in all_models.__all__:
            ...
            parts.append(
                query.filter((TaskGroupObject.object_type == model_name) & (
                    model.id == TaskGroupObject.object_id) & predicate(field))
                .exists())
