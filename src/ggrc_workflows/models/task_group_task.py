...
class TaskGroupTask(roleable.Roleable, mixins.Titled, mixins.Described,
                    mixins.Slugged, mixins.Timeboxed, Indexed, db.Model):
    ...
    def copy(self, _other=None, **kwargs):
        columns = [
            'title', 'description', 'task_group', 'sort_index', 'start_date',
            'end_date', 'access_control_list', 'modified_by', 'task_type',
            'response_options'
        ]

        if kwargs.get('clone_people', False):
            access_control_list = [{
                "ac_role_id": i.ac_role_id,
                "person": {
                    "id": i.person_id
                }
