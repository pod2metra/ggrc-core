...
def _query_filtered_by_contact(person):
    ...
    if any([
            attrs["email"].history.has_changes(),
...
class Cycle(mixins.WithContact, wf_mixins.CycleStatusValidatedMixin,
            mixins.Timeboxed, mixins.Described, mixins.Titled, mixins.Slugged,
  ...
  """
    ...
    def _filter_by_cycle_workflow(cls, predicate):
        ...
        return Workflow.query.filter((Workflow.id == cls.workflow_id) & (
            predicate(Workflow.slug) | predicate(Workflow.title))).exists()
    ...
    def indexed_query(cls):
        return super(Cycle, cls).indexed_query().options(
            ...
            .joinedload("access_control_list").load_only(
                "person_id", "ac_role_id"),
            orm.Load(cls).subqueryload("cycle_task_group_object_tasks")
            .joinedload("cycle_task_entries").load_only("description", "id"),
