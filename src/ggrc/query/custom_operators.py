        ...
        def decorated_operator(exp, *args, **kwargs):
            ...
            if error_fields:
                raise BadQueryException("\n".join([
                    required_tmpl.format(
                        field=field, operation=operation_name)
    ...
    def decorated(exp, object_class, target_class, query):
        ...
        return object_class.id.in_(
            db.session.query(Record.key).filter(
                Record.type == object_class.__name__, Record.property == key,
...
def related_people(exp, object_class, target_class, query):
  ...
  """
    ...
    res.extend(
        db.session.query(AccessControlList.person_id).filter(
            sqlalchemy.and_(
...
def relevant(exp, object_class, target_class, query):
    ...
    ids = exp['ids']
    check_snapshots = (object_class.__name__ in rules.Types.scoped
                       | rules.Types.trans_scope
                       and object_name in rules.Types.all)
    check_direct = (not check_snapshots
                    or object_class.__name__ in rules.Types.trans_scope)
    ...
    if check_snapshots:
        ...
        ).options(load_only(models.Snapshot.id),
                  ).distinct().subquery("snapshot")
        ...
        source_qs = db.session.query(
            models.Relationship.destination_id).filter(
...
def in_operation(exp, object_class, target_class, query):
    ...
    return object_class.id.in_(
        db.session.query(
            Record.key).filter(Record.type == object_class.__name__,
...
def cascade_unmappable(exp, object_class, target_class, query):
    ...
    if not issue_id:
        raise BadQueryException("Missing 'id' key in 'issue': {}".format(
            exp["issue"]))
    if not assessment_id:
        raise BadQueryException("Missing 'id' key in 'assessment': {}".format(
            exp["assessment"]))

    if object_class.__name__ not in {"Audit", "Snapshot"}:
        raise BadQueryException(
            "'cascade_unmapping' can't be applied to {}".format(
                object_class.__name__))

    mapped_to_issue = aliased(
        sqlalchemy.union_all(
            ...
            db.session.query(
                models.Relationship.source_id.label("target_id"), ).filter(
    ...
    other_assessments = aliased(
        sqlalchemy.union_all(
            ...
            db.session.query(
                models.Relationship.source_id.label("assessment_id"), ).filter(
                    models.Relationship.destination_id == issue_id,
                    models.Relationship.destination_type == "Issue",
                    models.Relationship.source_id != assessment_id,
                    models.Relationship.source_type == "Assessment",
    ...
    mapped_to_other_assessments = aliased(
        sqlalchemy.union_all(
            db.session.query(
                models.Relationship.destination_id.label("target_id"),
            ).filter(
    ...
    result = db.session.query(mapped_to_issue.c.target_id, ).join(
        mapped_to_assessment,
    ...
    ).filter(mapped_to_other_assessments.c.target_id.is_(None), )
