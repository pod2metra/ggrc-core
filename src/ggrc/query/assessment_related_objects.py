...
class AssessmentRelatedObjects(DefaultHandler):
  ...
  """
    ...
    def set_audit_result(self, assessment):
        ...
        ).filter(models.Audit.id == assessment.audit_id).first()
    ...
    def set_snapshot_result(self, assessment):
        ...
        with benchmark("Get assessment snapshot relationships"):
            snapshots = db.session.query(models.Snapshot).join(
                models.Relationship,
                and_(models.Snapshot.id == models.Relationship.source_id,
                ...
                db.session.query(models.Snapshot).join(
                    models.Relationship,
                    and_(models.Snapshot.id ==
    ...
    def set_comment_result(self, assessment):
        ...
        with benchmark("Get assessment snapshot relationships"):
            comments = db.session.query(models.Comment).join(
                models.Relationship,
                and_(models.Comment.id == models.Relationship.source_id,
                     models.Relationship.source_type == "Comment",
                     models.Relationship.destination_id == assessment.id,
                     models.Relationship.destination_type == "Assessment")
            ).union(
                db.session.query(models.Comment).join(
                    models.Relationship,
                    and_(models.Comment.id ==
                         models.Relationship.destination_id,
    ...
    def set_document_result(self, assessment):
        ...
        self.query[1]["last_modified"] = None
        with benchmark("Get assessment snapshot relationships"):
            documents = db.session.query(models.Document).join(
                models.Relationship,
                and_(models.Document.id == models.Relationship.source_id,
                     models.Relationship.source_type == "Document",
                     models.Relationship.destination_id == assessment.id,
                     models.Relationship.destination_type == "Assessment")
            ).union(
                db.session.query(models.Document).join(
                    models.Relationship,
                    and_(models.Document.id ==
                         models.Relationship.destination_id,
