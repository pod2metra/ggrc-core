...
class BaseClient(object):
    ...
    def _post(self, url, payload=None, headers=None):
        ...
        return self._perform_request(
            url, method=urlfetch.POST, payload=payload, headers=headers)

    def _put(self, url, payload=None, headers=None):
        """Performs PUT HTTP request to given URL with given data."""
        return self._perform_request(
            url, method=urlfetch.PUT, payload=payload, headers=headers)
...
class PersonClient(JsonClient):
    ...
    """
        response = self._post(
            '%s:search' % self._BASE_PATH, payload={'usernames': usernames})
    ...
    """
        response = self._post(
            '%s:suggest' % self._BASE_PATH, payload={'tokens': tokens})
        return response['persons']
