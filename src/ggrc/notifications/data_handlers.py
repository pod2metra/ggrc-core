...
ParentObjInfo = namedtuple("ParentObjInfo",
                           ["id", "object_type", "title", "url"])
...
def _get_updated_roles(new_list, old_list, roles):
    ...
    for role_id in role_ids:
        if sorted(new_dict.get(role_id, [])) != sorted(
                old_dict.get(role_id, [])):
...
def _get_assignable_roles(obj):
    ...
    query = db.session.query(models.AccessControlRole.id,
...
def get_assessment_url(assessment):
    return urlparse.urljoin(utils.get_url_root(), "assessments/{}".format(
        assessment.id))
...
def generate_comment_notification(obj, comment, person):
  ...
  """
    parent_info = ParentObjInfo(obj.id, obj._inflector.title_singular.title(),
                                obj.title, get_object_url(obj))
...
def get_comment_data(notif):
  ...
  """
    ...
    if rel and (issubclass(type(rel.source), Commentable)
                or issubclass(type(rel.destination), Commentable)):
