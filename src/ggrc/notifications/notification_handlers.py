  ...
  """
    ...
    for notification in db.session:
        if not isinstance(notification, models.Notification):
            continue
        notif_key = (notification.object_id, notification.object_type,
    ...
    return models.Notification.query.filter(
        ...
        (models.Notification.sent_at.is_(None) |
         (models.Notification.repeating == true()))).first()
...
def _add_state_change_notif(obj, state_change, remove_existing=False):
  ...
  """
    ...
    models.Notification.query.filter(
        models.Notification.object_id == obj.id,
        models.Notification.object_type == obj.type,
        models.Notification.notification_type_id.in_(notif_type_ids)).delete(
...
def handle_assignable_modified(obj):  # noqa: ignore=C901
  ...
  """
    ...
    if (old_state == Statusable.DONE_STATE
            and new_state == Statusable.PROGRESS_STATE):
    ...
    for attr_name, val in attrs.items():
        ...
        if val.history.has_changes():
            ...
            if attr_name == u"recipients" and not _recipients_changed(
                    val.history):
                continue
            is_changed = True
            break

    is_changed = is_changed or _ca_values_changed(
        obj)  # CA check only if needed
    ...
    if obj.status in Statusable.DONE_STATES:
        _add_state_change_notif(
            obj, Transitions.TO_REOPENED, remove_existing=True)
    elif obj.status == Statusable.START_STATE:
        _add_state_change_notif(
            obj, Transitions.TO_STARTED, remove_existing=True)
...
def _align_by_ids(items, items2):
  ...
  """
    ...
    while first is not STOP or second is not STOP:
        ...
        id_one, id_two = getattr(first, "id", None), getattr(
            second, "id", None)

        yield (first if id_one == min_id else None, second
               if id_two == min_id else None)
...
def handle_assignable_created(objects):
    names = [
        "{}_open".format(obj._inflector.table_singular) for obj in objects
    ]
    notif_types = models.NotificationType.query.filter(
...
def register_handlers():  # noqa: C901
    ...
    def relationship_altered_listener(sender, obj=None, src=None,
                                      service=None):
