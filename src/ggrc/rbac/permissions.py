...
def is_allowed_read(resource_type, resource_id, context_id):
  ...
  """
    if has_system_wide_read():
        return True
    return permissions_for(get_user()).is_allowed_read(resource_type,
...
def is_allowed_view_object_page_for(instance):
  ...
  """
    return permissions_for(
        get_user()).is_allowed_view_object_page_for(instance)
...
def has_conditions(action, resource):
  ...
  """
    ...
    return bool(
        _permissions.get(action, {}).get(resource, {}).get('conditions', {}))
