...
def has_not_changed_condition(instance, property_name, prevent_if=None, **_):
  ...
  """
    if prevent_if and getattr(instance, property_name, False) == prevent_if:
        return False
    return not db.inspect(instance).get_history(property_name,
                                                False).has_changes()
...
def has_changed_condition(instance, property_name, prevent_if=None, **_):
  ...
  """
    if getattr(instance, property_name, False) == prevent_if:
        return True
    return db.inspect(instance).get_history(property_name, False).has_changes()
...
class DefaultUserPermissions(UserPermissions):
    ...
    def _admin_permission_for_context(self, context_id):
        ...
        return Permission(self.ADMIN_PERMISSION.action,
                          self.ADMIN_PERMISSION.resource_type, None,
                          context_id)
    ...
    def _is_allowed_for(self, instance, action):
        ...
        if (not permissions.get(action) or not permissions[action].get(
                instance._inflector.model_singular)):
            return False
    ...
    resource_type"""
        ...
        for resource_type in resource_types:
            ret.extend(
                permissions.get(action, {}).get(resource_type, {}).get(
                    'resources', set()))
        return ret
    ...
    def _get_contexts_for(self, action, resource_type):
        ...
        admin_list = list(
            permissions.get(self.ADMIN_PERMISSION.action, {}).get(
                self.ADMIN_PERMISSION.resource_type, {}).get('contexts', ()))
