...
class MysqlRecordProperty(db.Model):
    ...
    def __table_args__(cls):  # pylint: disable=no-self-argument
        return (
            ...
            db.Index('ix_{}_context_id'.format(cls.__tablename__),
                     'context_id'),
        )
...
class MysqlIndexer(SqlIndexer):
    ...
    def _get_filter_query(terms):
        ...
        if not terms:
            ...
            return sa.and_(whitelist,
                           MysqlRecordProperty.content.contains(terms))

    @staticmethod
    def get_permissions_query(model_names,
    ...
    """
        ...
        for model_name in model_names:
            ...
            statement = sa.and_(MysqlRecordProperty.type == model_name,
                                context_query_filter(
                                    MysqlRecordProperty.context_id, contexts))
            if resources:
                statement = sa.or_(
        ...
        return sa.and_(
            MysqlRecordProperty.type.in_(model_names), sa.or_(*type_queries))
    ...
    """
        ...
        union_query = my_objects.get_myobjects_query(
            types=types, contact_id=contact_id)

        return query.join(
            union_query,
            sa.and_(union_query.c.id == MysqlRecordProperty.key,
    ...
    def _add_extra_params_query(self, query, type_name, extra_param):
        ...
        return query.filter(
            self.record_type.key.in_(
                db.session.query(model_klass.id.label('id')).filter_by(
                    **extra_param)))
    ...
    full text table."""
        ...
        columns = (self.record_type.key.label('key'),
        ...
        query = query.filter(
            self.get_permissions_query(model_names, permission_type,
                                       permission_model))
        ...
        all_queries = aliased(
            all_queries.order_by(all_queries.c.sort_key,
                                 all_queries.c.content))
    ...
    def counts(self,
     ...
     the requested objects."""
        ...
        model_names = self._get_grouped_types(types, extra_params)
        query = db.session.query(self.record_type.type,
                                 sa.func.count(
                                     sa.distinct(self.record_type.key)),
                                 sa.literal(""))
        query = query.filter(self.get_permissions_query(model_names))
        query = query.filter(self._get_filter_query(terms))
        query = self.search_get_owner_query(query, types, contact_id)
        query = query.group_by(self.record_type.type)
        all_extra_columns = dict(
            extra_columns.items() + [(p, p) for p in extra_params
        ...
        for key, value in all_extra_columns.iteritems():
            ...
            extra_q = self._add_extra_params_query(extra_q, value,
