...
class RecordBuilder(object):
    ...
    """
        ...
        if person_id in self.indexer.cache['people_map']:
            person_name, person_email = self.indexer.cache['people_map'][
                person_id]
        else:
            if isinstance(person, dict):
            ...
            self.indexer.cache['people_map'][person_id] = (person_name,
                                                           person_email)
    ...
    """
        ...
        if ac_role_id not in self.indexer.cache['ac_role_map']:
            ac_role = db.session.query(
                all_models.AccessControlRole).get(ac_role_id)
            ...
            if ac_role and ac_role.internal:
                ...
                if ac_role_name is None:
                    ...
                    LOGGER.error("Trying to index not existing ACR with id %s",
                                 ac_role_id)
    ...
    """
        ...
        if value and definition.attribute_type == "Map:Person":
            ...
            properties[attribute_name].update(
                self.build_list_sort_subprop([value]))
        else:
            properties[attribute_name] = {
                "": definition.get_indexed_value(value)
            }
        return properties
    ...
    """
        ...
        if isinstance(obj, CustomAttributable):
            cavs = {
                ...
                for v in obj.custom_attribute_values
            }
            for cad in obj.custom_attribute_definitions:
                ...
                properties.update(
                    self.get_custom_attribute_properties(cad, value))
