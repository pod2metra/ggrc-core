...
def create_gdrive_file(csv_string, filename):
    ...
    try:
        ...
        media = http.MediaInMemoryUpload(
            csv_string, mimetype='text/csv', resumable=True)
...
def copy_file_request(drive_service, file_id, body):
    ...
    response = drive_service.files().copy(
        fileId=file_id, body=body, fields='webViewLink,name').execute()
    return response
...
def process_gdrive_file(folder_id,
                        file_id,
                        postfix,
                        separator,
                        is_uploaded=False):
    ...
    try:
        ...
        if is_uploaded:
            ...
            response = rename_file_request(
                drive_service, file_id, body={'name': new_file_name})
        else:
...
def generate_file_name(original_name, postfix, separator):
    ...
    # sanitize file name
    new_name = ''.join([
        char if char.isalnum() or char in ALLOWED_FILENAME_CHARS else '-'
