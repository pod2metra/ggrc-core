  ...
  """
    ...
    for (klass_name, table_) in tables:
        sql_base_left = select([
            ...
            and_(relationships_table.c.source_type == klass_name,
                 relationships_table.c.destination_type == "Audit")).group_by(
        ...
        sql_base_right = select([
            ...
            and_(relationships_table.c.destination_type == klass_name,
                 relationships_table.c.source_type == "Audit")).group_by(
        ...
        all_object_ids = {
            x.id
            for x in connection.execute(select([table_.c.id])).fetchall()
        }
        to_audit_mapped_ids = {x.object_id for x in result_more + result_one}
