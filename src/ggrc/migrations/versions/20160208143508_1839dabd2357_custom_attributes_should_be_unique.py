...
def upgrade():
  ...
  """
    ...
    for a in attributes:
        lowercase_title = a[
            'title'].lower()  # mysql comparison is case insensitive (!) # noqa
        ...
        while True:
            ...
            existing_records_alike = conn.execute(
                text(unique_count_query),
                title=new_title,
                definition_type=a['definition_type']).first()[0]
            ...
            if index > 1000:
                ...
                raise StandardError
        records_to_insert[(new_title, a['definition_type'])] = (a['id'],
                                                                new_title)
...
def downgrade():
    op.drop_constraint(
        "uq_custom_attribute", "custom_attribute_definitions", type_="unique")
