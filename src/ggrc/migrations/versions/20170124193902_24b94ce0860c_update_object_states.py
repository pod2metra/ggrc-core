...
tables = {
    'access_groups', 'clauses', 'controls', 'data_assets', 'directives',
    'facilities', 'issues', 'markets', 'objectives', 'org_groups', 'products',
    'programs', 'projects', 'sections', 'systems', 'vendors'
}
...
def downgrade():
    ...
    op.add_column('object_people',
                  sa.Column(
                      'status', mysql.VARCHAR(length=250), nullable=True))
    op.add_column('object_owners',
                  sa.Column(
                      'status', mysql.VARCHAR(length=250), nullable=True))
    op.add_column('object_documents',
                  sa.Column(
                      'status', mysql.VARCHAR(length=250), nullable=True))
    op.alter_column(
        'background_tasks',
