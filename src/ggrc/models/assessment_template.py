  ...
  """
    ...
    def clone(self, target):
        ...
        rel = relationship.Relationship(
            source=target, destination=assessment_template_copy)
        db.session.add(rel)
        db.session.flush()
    ...
    def archived(self):
        ...
        if hasattr(self, 'context') and hasattr(self.context,
                                                'related_object'):
...
def create_audit_relationship(audit_stub, obj):
    ...
    if not permissions_for()._is_allowed_for(parent_audit, "update"):
        raise Forbidden()
    rel = relationship.Relationship(
        source=parent_audit, destination=obj, context=parent_audit.context)
    db.session.add(rel)
