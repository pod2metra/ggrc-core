  ...
  """
    ...
    _api_attrs = reflection.ApiAttributes(
        ...
        reflection.Attribute(
            "original_object_deleted", create=False, update=False),
...
def _ensure_program_relationships(objects):
  ...
  """
    ...
    required_relationships = set(
        relationship_stub(o.parent.program_id, "Program", o.child_id,
                          o.child_type) for o in objects)
    ...
    existing_mappings = columns.filter(
        tuple_(rel.source_id, rel.source_type, rel.destination_id,
               rel.destination_type).in_(required_relationships))
    existing_mappings_reverse = columns.filter(
        tuple_(rel.destination_id, rel.destination_type, rel.source_id,
               rel.source_type).in_(required_relationships))

    required_relationships -= set(
        relationship_stub(row.source_id, row.source_type, row.destination_id,
                          row.destination_type) for row in existing_mappings)
    required_relationships -= set(
...
def _insert_program_relationships(relationship_stubs):
    ...
    db.session.execute(
        inserter.values([{
            ...
            now,
            "source_type":
            relationship_stub.source_type,
            "source_id":
            relationship_stub.source_id,
            "destination_type":
            relationship_stub.destination_type,
            "destination_id":
            relationship_stub.destination_id,
            "context_id":
            None,
            "status":
            None,
