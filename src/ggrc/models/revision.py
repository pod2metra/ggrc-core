...
class Revision(Base, db.Model):
    ...
    action = db.Column(
        db.Enum(u'created', u'modified', u'deleted'), nullable=False)
    ...
    def populate_acl(self):
        ...
        access_control_list = self._populate_acl_with_people(
            access_control_list)
        return {
            "access_control_list":
            self._filter_internal_acls(access_control_list)
        }
    ...
    def populate_status(self):
        ...
        return {
            "status": statuses_mapping.get(
                self._content.get("status"), "Draft")
        }
