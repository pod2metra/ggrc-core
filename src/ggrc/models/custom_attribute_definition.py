...
class CustomAttributeDefinition(attributevalidator.AttributeValidator,
  ...
  """
    ...
    def inflector_model_name_dict(self):  # pylint: disable=no-self-use
        ...
        return {
            m._inflector.table_singular: m.__name__
    ...
    _extra_table_args = (UniqueConstraint(
        'definition_type',
        'definition_id',
        'title',
    ...
    _api_attrs = reflection.ApiAttributes(
        reflection.Attribute(
            "default_value", read=True, create=False, update=False),
        *_include_links)
    ...
    def validate_multi_choice_options(self, _, value):
        ...
        if value:
            ...
            if "" in value_set:
                raise ValidationError(
                    "Empty dropdown options are not allowed: '{}'"
                    .format(value))
...
class CustomAttributeMapable(object):
    ...
    """
        return db.relationship(
            'CustomAttributeValue',
            primaryjoin=
            lambda: ((CustomAttributeValue.attribute_value == cls.__name__) & (CustomAttributeValue.attribute_object_id == cls.id)),
