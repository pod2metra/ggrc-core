  ...
  """
    ...
    """
        ...
        if value and value.issubset(self.VALID_RECIPIENTS):
            ...
            raise ValueError(
                value,
                'Value should be either empty ' + 'or comma separated list of '
                + ', '.join(sorted(self.VALID_RECIPIENTS)))

    recipients = db.Column(
        db.String, nullable=True, default=u"Assignees,Creators,Verifiers")
...
class Comment(Roleable, Relatable, Described, Notifiable, Base, Indexed,
    ...
    custom_attribute_definition_id = deferred(
        db.Column(
            db.Integer,
            db.ForeignKey(
                'custom_attribute_definitions.id', ondelete='SET NULL'),
            nullable=True,
        ), 'Comment')
    ...
    _api_attrs = reflection.ApiAttributes(
        ...
        reflection.Attribute(
            "custom_attribute_revision", create=False, update=False),
...
class CommentInitiator(object):  # pylint: disable=too-few-public-methods
    ...
    Links comments to object that are the reason of that comment generation."""
        ...
        def join_function():
            return sa.and_(
                sa.orm.foreign(
                    Comment.initiator_instance_type) == cls.__name__,
