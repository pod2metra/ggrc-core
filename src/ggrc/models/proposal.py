...
class Proposal(
        mixins.person_relation_factory("applied_by"),
  ...
  Collect all information about propose change to Proposable instances."""
    ...
    _api_attrs = reflection.ApiAttributes(
        ...
        reflection.Attribute(
            'full_instance_content', create=True, update=False, read=False),
...
def get_propsal_acr_dict():
    return {
        r.name: r
        for r in ac_role.AccessControlRole.query.filter(
            ac_role.AccessControlRole.object_type == Proposal.__name__,
            ac_role.AccessControlRole.name.in_(
                [Proposal.ACRoles.EDITOR, Proposal.ACRoles.READER]))
    }
...
def permissions_for_proposal_setter(proposal, proposal_roles):
    ...
    for acl in proposal.instance.full_access_control_list:
        ...
        if acl.ac_role.update:
            ...
            continue
        ac_list.AccessControlList(
            ac_role=role, object=proposal, person=acl.person, parent=acl)
