...
def get_people_ids_based_on_role(assignee_role, default_role,
...
def generate_role_object_dict(snapshot, audit):
  ...
  """
    ...
    auditors_role = next(
        id_ for id_, val in audit_acr.iteritems() if val == "Auditors")
    leads_role = next(
        id_ for id_, val in audit_acr.iteritems() if val == "Audit Captains")
...
def relate_assignees(assessment, snapshot, template, audit):
  ...
  """
    ...
    assignee_ids = get_people_ids_based_on_role(
        "assignees",
        "Audit Lead",  # default assignee
        template_settings,
        acl_dict)
    verifier_ids = get_people_ids_based_on_role(
        "verifiers",
...
def relate_ca(assessment, template):
  ...
  """
    ...
    ca_definitions = all_models.CustomAttributeDefinition.query.options(
        orm.undefer_group('CustomAttributeDefinition_complete'), ).filter_by(
            definition_id=template.id,
            definition_type="assessment_template",
        ).order_by(all_models.CustomAttributeDefinition.id)
