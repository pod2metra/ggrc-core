...
class AuditRolesHandler(object):
    ...
    def handle_relationship(self, obj):
        ...
        elif (isinstance(first, (all_models.Comment, all_models.Document))
              and isinstance(second, all_models.Issue)):
        ...
        if not isinstance(
                other,
            (all_models.Assessment, all_models.AssessmentTemplate,
             all_models.Audit, all_models.Issue, all_models.Snapshot,
             all_models.Document, all_models.Comment)):
            return
        ...
        auditors_mapped_dict = defaultdict(
            lambda: audit_roles["Auditors Mapped"], {
                ...
                audit_roles["Auditors Assessment Mapped"],
        ...
        role_map = {
            audit_roles["Auditors"]:
            auditors_mapped_dict,
            audit_roles["Audit Captains"]:
            defaultdict(lambda: audit_roles["Audit Captains Mapped"]),
            audit_roles["Audit Captains Mapped"]:
            defaultdict(lambda: audit_roles["Audit Captains Mapped"]),
            audit_roles["Auditors Assessment Mapped"]:
            auditors_mapped_dict,
            audit_roles["Auditors Issue Mapped"]:
            auditors_mapped_dict,
        }
        for acl in access_control_list:
