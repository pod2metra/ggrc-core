...
def _handle_del_audit_issue_mapping(audit, issue):
    ...
    if audit not in db.session.deleted and not issue.allow_unmap_from_audit:
        raise exceptions.ValidationError(
            "Issue#{issue.id} can't be unmapped "
            "from Audit#{audit.id}: common mappings.".format(
                issue=issue, audit=audit))
...
def _handle_new_audit_issue_mapping(audit, issue):
    ...
    if not issue.allow_map_to_audit:
        raise exceptions.ValidationError(
            ...
            "to Audit#{mapped_audit_id}".format(
                issue=issue,
                audit=audit,
...
def _is_audit_issue(src, dst):
    ...
    return ((src.type, dst.type) == (all_models.Audit.__name__,
  ...
  """
    ...
    def decorator(function):
      ...
      """
            ...
            items = (o for o in itertools.chain(*sets)
  ...
  """
    ...
    for instance in instances:
        ...
        if not instance.destination:
            ...
            LOGGER.error(
                u"Relationship.destination is not filled in properly: "
                u"id=%r, source_id=%r, source_type=%r, "
                u"destination_id=%r, destination_type=%r.", instance.id,
                instance.source_id, instance.source_type,
                instance.destination_id, instance.destination_type)
            continue
...
def related_id(related_to_type, label):
    ...
    return sa.case(
        ...
        else_=all_models.Relationship.source_id).label(label)
...
def unmap_issue_cascade(asmnt, issue):
  ...
  """
    ...
    other_asmnt_snap_ids = db.session.query(
        related_id("Assessment", "snapshot_id"), ).filter(
            sa.or_(
                sa.and_(
    ...
    asmnt_snap_ids = db.session.query(
        related_id("Assessment", "snapshot_id"), ).filter(
            related_condition(asmnt, "Snapshot")).subquery()
...
def related(base_objects, rel_cache):
  ...
  """
    ...
    return {
        o: (rel_cache.cache[o] if o in rel_cache.cache else set())
...
def related_regulation_snaps(snapshot_ids):
  ...
  """
    related_snapshots = sa.union_all(
        ...
        .join(all_models.Snapshot,
              sa.and_(
                  all_models.Relationship.source_id == all_models.Snapshot.id,
                  all_models.Relationship.source_type == "Snapshot",
                  all_models.Relationship.destination_type == "Snapshot",
              )),
        db.session.query(all_models.Relationship.source_id,
                         all_models.Relationship.destination_id,
                         all_models.Snapshot.child_type)
        .join(all_models.Snapshot,
              sa.and_(
                  all_models.Relationship.destination_id ==
                  all_models.Snapshot.id,
                  all_models.Relationship.source_type == "Snapshot",
                  all_models.Relationship.destination_type == "Snapshot",
    ...
    for base_stub, related_stubs in related_objects.items():
        for related_stub in related_stubs:
            for acl in base_objects[base_stub]:
                acr_id = acl.ac_role.id if acl.ac_role else acl.ac_role_id
                mapped_acr_id = get_mapped_role(base_stub.type,
                                                get_custom_roles_for(
                                                    base_stub.type)[acr_id],
                                                related_stub.type)
...
def handle_relationship_delete(relationship):
    ...
    if (issubclass(type(relationship.source), Assignable)
            or issubclass(type(relationship.destination), Assignable)):
...
def copy_snapshot_test_plan(objects):
    ...
    for obj in objects:
        if (obj.source_type == "Assessment"
                and obj.destination_type == "Snapshot") or (
                    obj.source_type == "Snapshot"
...
def init_hook():  # noqa
    ...
    def relationship_deleted_listener(sender, obj=None, src=None,
                                      service=None):
    ...
    sa.event.listen(all_models.Relationship, 'before_delete',
    ...
    def handle_cascade_delete(sender, obj, service):
        ...
        # pylint: disable=unused-argument
        is_asmnt_issue_rel = (obj.source_type == "Assessment"
                              and obj.destination_type == "Issue") or (
                                  obj.source_type == "Issue"
                                  and obj.destination_type == "Assessment")
