...
_COMMENT_TMPL = ('A new comment is added by %s to the Assessment: %s. '
...
_DISABLED_TMPL = ('Changes to this GGRC Assessment will no longer be '
...
_VERIFIER_STATUSES = {
    ...
    # State: FIXED
    ('Not Started', 'In Review', False):
    'FIXED',
    ('In Progress', 'In Review', False):
    'FIXED',
    ('Rework Needed', 'In Review', False):
    'FIXED',
    ('Completed', 'In Review', True):
    'FIXED',
    ('Deprecated', 'In Review', False):
    'FIXED',
    ...
    ('Completed', 'In Progress', False):
    'ASSIGNED',
    ('Deprecated', 'In Progress', False):
    'ASSIGNED',

    # State: ASSIGNED
    ('In Review', 'Rework Needed', False):
    'ASSIGNED',
    ('Completed', 'Rework Needed', True):
    'ASSIGNED',
    ('Deprecated', 'Rework Needed', False):
    'ASSIGNED',

    # State: FIXED (Verified)
    ('Not Started', 'Completed', True):
    'VERIFIED',
    ('In Progress', 'Completed', True):
    'VERIFIED',
    ('In Review', 'Completed', True):
    'VERIFIED',
    ('Rework Needed', 'Completed', True):
    'VERIFIED',
    ('Deprecated', 'Completed', True):
    'VERIFIED',

    # State: ASSIGNED
    ('In Review', 'Not Started', False):
    'ASSIGNED',
    ('Rework Needed', 'Not Started', False):
    'ASSIGNED',
    ('Completed', 'Not Started', True):
    'ASSIGNED',
    ('Deprecated', 'Not Started', False):
    'ASSIGNED',

    # State: WON'T FIX (OBSOLETE)
    ('Not Started', 'Deprecated', False):
    'OBSOLETE',
    ('In Progress', 'Deprecated', False):
    'OBSOLETE',
    ('In Review', 'Deprecated', False):
    'OBSOLETE',
    ('Rework Needed', 'Deprecated', False):
    'OBSOLETE',
    ('Completed', 'Deprecated', True):
    'OBSOLETE',
}
...
def _handle_assessment_tmpl_post(sender, objects=None, sources=None):
    ...
    db.session.flush()
    template_ids = {obj.id for obj in objects}
...
def _handle_audit_put_after_commit(sender, obj=None, **kwargs):
    ...
    if not initial_state:
        logger.debug('Initial state of an Audit is not provided, '
                     'skipping IssueTracker update.')
        return

    if (obj.archived == initial_state.archived
            or not obj.issue_tracker.get('enabled', False)):
        return

    start_update_issue_job(obj.id, _ARCHIVED_TMPL
                           if obj.archived else _UNARCHIVED_TMPL)
...
def _handle_issuetracker(sender, obj=None, src=None, **kwargs):
    ...
    logger.info('--> _handle_issuetracker: issue_obj=%s', issue_obj)
    ...
    logger.info('--> _handle_issuetracker: initial_info=%s', initial_info)
    ...
    logger.info('--> _handle_issuetracker: issue_tracker_info=%s',
                issue_tracker_info)
    try:
        ...
        if (issue_obj.enabled and issue_obj.issue_id
            ...
            try:
                ...
                logger.error('Unable to update a ticket ID=%s while deleting'
                             ' assessment ID=%d: %s', issue_obj.issue_id,
                             obj.id, error)
        db.session.delete(issue_obj)
...
def _get_roles(assessment):
  ...
  """
    ...
    query = db.session.query(
        ac_list.person_id, ac_role.name, all_models.Person.email).join(
            ac_role, ac_role.id == ac_list.ac_role_id).join(
                all_models.Person,
                all_models.Person.id == ac_list.person_id).filter(
...
def _build_status_comment(assessment, initial_assessment):
    ...
    if verifiers:
        ...
        status = _NO_VERIFIER_STATUSES.get((initial_assessment.status,
                                            assessment.status))
...
def _fill_current_value(issue_params, assessment, initial_info):
    ...
    if 'hotlist_ids' not in issue_params:
        ...
        issue_params['hotlist_ids'] = [current_hotlist_id
                                       ] if (current_hotlist_id) else []
...
def _build_cc_list(issue_tracker_info, initial_info):
    ...
    if cc_list is not None:
        current_cc_list = initial_info.get('cc_list')
        current_cc_list = set(current_cc_list) if (current_cc_list is
                                                   not None) else set()
...
def _get_added_comment_text(src):
    ...
    if comment_id is not None:
        comment_row = db.session.query(
            all_models.Comment.description, all_models.Person.email,
            all_models.Person.name).outerjoin(
                all_models.Person,
                all_models.Person.id == all_models.Comment.modified_by_id,
            ).filter(all_models.Comment.id == comment_id).first()
...
def _create_issuetracker_info(assessment, issue_tracker_info):
    ...
    if (issue_tracker_info.get('enabled')
            and _is_issue_tracker_enabled(audit=assessment.audit)):
        ...
        try:
            issue_id = _create_issuetracker_issue(assessment,
                                                  issue_tracker_info)
        except integrations_errors.Error as error:
            logger.error(
            ...
            assessment.add_warning('issue_tracker',
...
def _update_issuetracker_issue(assessment, issue_tracker_info,
    ...
    if comment_text is not None:
        comments.append(_COMMENT_TMPL % (comment_author, comment_text,
                                         _get_assessment_url(assessment)))
