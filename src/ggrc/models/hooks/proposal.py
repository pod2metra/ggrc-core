...
def is_status_changed_to(required_status, obj):
    return (inspect(obj).attrs.status.history.has_changes()
            and obj.status == required_status)
...
def add_comment_about(proposal, reason, txt):
    ...
    all_models.Relationship(
        source=proposal.instance, destination=created_comment)
...
def init_hook():
    ...
    for model in all_models.all_models:
        signals.Restful.model_posted.connect(
            set_permissions, model, weak=False)
        signals.Restful.model_put.connect(set_permissions, model, weak=False)
