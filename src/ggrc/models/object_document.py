...
class Documentable(object):
    ...
    _fulltext_attrs = [
        MultipleSubpropertyFullTextAttr(
            'document_evidence', 'document_evidence', ['title', 'link']),
    ...
    def documents(cls):
        ...
        return db.relationship(
            ...
            primaryjoin=
            lambda: and_(documentable_id, cls.id == documentable_id),
            ...
            # case return 2 types of values BOOL(false) and INT(id) not Null
            secondaryjoin=
            lambda: and_(document_id, Document.id == document_id, documentable_type == cls.__name__),
            viewonly=True,
        )
