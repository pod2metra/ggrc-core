...
class Context(Base, db.Model):
    ...
    _api_attrs = reflection.ApiAttributes('name', 'related_object',
                                          'description')
  ...
  """
    ...
    def build_object_context(self, context, name=None, description=None):
        ...
        if description is None:
            description = ''
        new_context = Context(
            name=name, description=description, related_object=self)
    ...
    def _filter_by_role(cls, role, predicate):
        ...
        return Person.query.join(
            UserRole, Role).filter((UserRole.context_id == cls.context_id) & (
                Role.name == role) & (predicate(Person.name)
