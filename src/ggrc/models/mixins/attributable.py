...
class Attributable(object):
    ...
    def _attributes(cls):  # pylint: disable=no-self-argument
        ...
        def join_function():
            ...
            return and_(object_id == cls.id, object_type == unicode(
                cls.__name__))
    ...
    def eager_query(cls):
        ...
        query = query.options(orm.subqueryload('_attributes'))
        return query

    @classmethod
    def indexed_query(cls):
        return super(Attributable, cls).indexed_query().options(
            orm.Load(cls).subqueryload("_attributes").load_only(
                "value_datetime",
                "value_string",
                "value_integer",
            ))
    ...
    def get_delete_ca_query_for(cls, ids):
        ...
        return attributes.Attributes.__table__.delete().where(
            attributes.Attributes.object_type == cls.__name__).where(
