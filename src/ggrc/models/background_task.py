...
class BackgroundTask(Base, Stateful, db.Model):
    ...
    VALID_STATES = ["Pending", "Running", "Success", "Failure"]
    ...
    def make_response(self, default=None):
        ...
        return app.make_response(
            (self.result['content'], self.result['status_code'],
             self.result['headers']))
...
def queued_task(func):
    ...
    """
        ...
        try:
            ...
            task.finish("Failure",
                        app.make_response((traceback.format_exc(), 200,
                                           [('Content-Type', 'text/html')])))

            # Return 200 so that the task is not retried
            return app.make_response(('failure', 200, [('Content-Type',
                                                        'text/html')]))
