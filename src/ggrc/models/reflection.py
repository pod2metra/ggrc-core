...
class AttributeInfo(object):
  ...
  """
    ...
    def gather_publish_attrs(cls, tgt_class):
        return [
            attr_name for attr_name, attr in cls.gather_attr_dicts(
                tgt_class, "_api_attrs").iteritems() if attr.read
        ]
    ...
    def gather_visible_aliases(cls, tgt_class):
        return {
            attr: props
            for attr, props in AttributeInfo.gather_aliases(tgt_class)
            .iteritems() if props is not None and not is_filter_only(props)
        }
    ...
    def gather_update_attrs(cls, tgt_class):
        return [
            attr_name for attr_name, attr in cls.gather_attr_dicts(
                tgt_class, "_api_attrs").iteritems() if attr.update
        ]
    ...
    def gather_create_attrs(cls, tgt_class):
        return [
            attr_name for attr_name, attr in cls.gather_attr_dicts(
                tgt_class, "_api_attrs").iteritems() if attr.create
        ]
    ...
    def get_acl_definitions(cls, object_class):
        ...
        return {
            ...
            for name, mandatory in flask.g.acl_role_names[
                object_class.__name__]
        }

    @classmethod
    def _generate_mapping_definition(cls, rules_set, prefix,
                                     display_name_tmpl):
    ...
    """
        ...
        definitions.update(
            cls._generate_mapping_definition(
                rules.get_snapshot_mapping_rules().get(object_class.__name__,
                                                       set()),
    ...
    """
        ...
        for attr in custom_attributes:
            ...
            if (attr.attribute_type == attr.ValidTypes.DROPDOWN
                    and attr.multi_choice_options):
                if description:
                    description += "\n\n"
            ...
            definition_ids = definitions.get(attr_name, {}).get(
                "definition_ids", [])
    ...
    def get_unique_constraints(cls, object_class):
        ...
        unique = [
            con for con in constraints if isinstance(con, UniqueConstraint)
        ]
        # we only handle single column unique constraints
        unique_columns = [
            u.columns.keys() for u in unique if len(u.columns) == 1
        ]
    ...
    """
        ...
        if object_class.__name__ not in EXCLUDE_CUSTOM_ATTRIBUTES:
            definitions.update(
                cls.get_custom_attr_definitions(
    ...
    def get_attr_definitions_array(cls, object_class, ca_cache=None):
        ...
        definitions = cls.get_object_attr_definitions(
            object_class, ca_cache=ca_cache)
    ...
    """
        ...
        custom_attrs = [
            v for v in other_attrs if not v.lower().startswith("map:")
        ]
        mapping_attrs = [
            v for v in other_attrs if v.lower().startswith("map:")
        ]
