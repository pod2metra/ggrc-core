...
class CustomAttributeValue(Base, Indexed, db.Model):
    ...
    custom_attribute_id = db.Column(db.Integer,
                                    db.ForeignKey(
    ...
    """
        ...
        def filter_by_custom(predicate):
            return cls.query.filter(
                ...
                (cls.attributable_type == obj_class.__name__) &
                (cls.attributable_id == obj_class.id
                 ) & predicate(cls.attribute_value)).exists()
            ...
            if self.attribute_value not in valid_options:
                raise ValueError(
                    ...
                    "expected one of {l}".format(
                        v=self.attribute_value, l=valid_options))
    ...
    """
        ...
        def make_flags(multi_choice_mandatory):
            ...
            return flags(
                comment_required=flags_mask &
                (cad.MultiChoiceMandatoryFlags.COMMENT_REQUIRED),
                evidence_required=flags_mask &
                (cad.MultiChoiceMandatoryFlags.EVIDENCE_REQUIRED))
