...
def task_group_object(object_type, related_type, related_ids):
    ...
    if object_type == "TaskGroup":
        return db.session.query(
            all_models.TaskGroupObject.task_group_id).filter(
...
def _assessment_object_mappings(object_type, related_type, related_ids):
    ...
    if (object_type in Types.scoped | Types.trans_scope
            and related_type in Types.all):

        source_query = db.session.query(
            Relationship.destination_id.label("result_id"), ).join(
                Snapshot,
                and_(
        ...
        destination_query = db.session.query(
            Relationship.source_id.label("result_id"), ).join(
                Snapshot,
                and_(
    ...
    elif (object_type in Types.all
          and related_type in Types.scoped | Types.trans_scope):
        source_query = db.session.query(
            Snapshot.child_id.label("result_id"), ).join(
                Relationship,
                and_(
                    Relationship.source_id == Snapshot.id,
                    Relationship.source_type == Snapshot.__name__,
        ...
        raise Exception("Snapshot relationship called with invalid types.\n"
                        "object types: '{}' - '{}'".format(
...
def _parent_object_mappings(object_type, related_type, related_ids):
    ...
    if object_type in Types.parents and related_type in Types.all:
        ...
        raise Exception("Parent relationship called with invalid types.\n"
                        "object types: '{}' - '{}'".format(
                            object_type, related_type))
...
def get_ids_related_to(object_type, related_type, related_ids=None):
  ...
  """
    ...
    if (object_type in Types.scoped and related_type in Types.all
            or related_type in Types.scoped and object_type in Types.all):
        return _assessment_object_mappings(object_type, related_type,
                                           related_ids)

    if (object_type in Types.parents and related_type in Types.all
            or related_type in Types.parents and object_type in Types.all):
        return _parent_object_mappings(object_type, related_type, related_ids)
    ...
    queries.extend(
        get_extension_mappings(object_type, related_type, related_ids))
    queries.extend(
        get_special_mappings(object_type, related_type, related_ids))

    if (object_type in Types.trans_scope and related_type in Types.all
            or object_type in Types.all and related_type in Types.trans_scope):
        queries.append(
            _assessment_object_mappings(object_type, related_type,
                                        related_ids))
