...
class AccessControlList(mixins.Base, db.Model):
  ...
  """
    ...
    _api_attrs = reflection.ApiAttributes("person",
                                          ...
                                          reflection.Attribute(
                                              "person_name",
                                              create=False,
                                              update=False), "ac_role_id")

    person_id = db.Column(
        db.Integer, db.ForeignKey('people.id'), nullable=False)
    ac_role_id = db.Column(
        db.Integer, db.ForeignKey('access_control_roles.id'), nullable=False)
    ...
    parent = db.relationship(
        lambda: AccessControlList, remote_side=lambda: AccessControlList.id)
    ...
    def _extra_table_args(_):
        return (
            db.UniqueConstraint('person_id', 'ac_role_id', 'object_id',
