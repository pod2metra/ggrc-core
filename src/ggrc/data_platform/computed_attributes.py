...
def get_computed_attributes():
    ...
    return db.session.query(models.AttributeTemplates).join(
        models.AttributeDefinitions).join(models.AttributeTypes).filter(
            models.AttributeTypes.computed == 1).all()
...
def get_aggregate_function(attribute):
    ...
    if function_name == "max":
        ...
        def max_(aggregate_values, rel_map):
            ...
            values = [(aggregate_values[aggregate_id], aggregate_id)
...
def _get_group_key(revision, aggregate_type, computed_object):
  ...
  """
    ...
    elif (revision.resource_type == "Relationship"
          and revision.source_type in related_types
          and revision.destination_type in related_types):
        key = "related_objects"
    return key
...
def group_revisions(attributes, revisions):
    ...
    for attr in attributes:
        ...
        for revision in revisions:
            ...
            if key:
                groups[attr][key].add((revision.resource_type,
...
def _objects_from_snapshots(snapshots):
    ...
    return set(
        ...
        ).filter(models.Snapshot.id.in_(
            snap[1] for snap in snapshots)).distinct())
...
def get_affected_objects(attribute_groups):
    ...
    for attr, groups in attribute_groups.iteritems():
        ...
        objects.update(
            _objects_from_snapshots(groups["destination_snapshots"]))
        objects.update(
            _get_objects_from_aggregates(groups["aggregate_objects"],
                                         attr.object_template.name))
        objects.update(
            _get_objects_from_deleted(groups["aggregate_deleted"],
...
def _get_aggregate_values(attr, aggregate_objects):
  ...
  """
    ...
    ).filter(aggregate_model.id.in_(ids))
...
def compute_values(affected_objects, all_relationships, snapshot_map):
    ...
    for attr, objects in affected_objects.iteritems():
        ...
        for obj in objects:
            source_id, value = aggregate_function(aggregate_values,
                                                  rel_map[obj])
            ...
            if field_type in ("value_datetime", "value_integer",
                              "value_string"):
            ...
            for snapshot_id in snapshot_map.get(obj, set()):
                computed_values[attr][(u"Snapshot",
                                       snapshot_id)] = computed_value_dict
...
def get_snapshot_data(affected_objects):
    ...
    query = db.session.query(models.Snapshot.child_type,
                             models.Snapshot.child_id,
                             models.Snapshot.id).filter(
                                 sa.tuple_(
                                     models.Snapshot.child_type,
                                     models.Snapshot.child_id,
                                 ).in_(all_objects))
    ...
    snapshot_tag_map = {}
    if all_ids:
        query = db.session.query(models.Snapshot.id,
                                 sa.func.concat_ws(
                                     "-",
                                     models.Snapshot.parent_type,
                                     models.Snapshot.parent_id,
                                     models.Snapshot.child_type,
                                 )).filter(models.Snapshot.id.in_(all_ids))
...
def get_index_data(computed_values, snapshot_tag_map):
    ...
    for attr, objects in computed_values.iteritems():
        for obj, computed_value in objects.iteritems():
            value = (computed_value["value_datetime"]
                     or computed_value["value_string"] or "")
            tags = u""
            if obj[0] == "Snapshot":
...
def delete_all_computed_values():
    ...
    with benchmark("Delete all computed attribute values"):
        ...
        models.Attributes.query.filter_by(
            models.Attributes.attribute_template.in_(attributes)).delete()
...
def get_revisions(revision_ids):
  ...
  """
    ...
    ).filter(models.Revision.resource_type != "Snapshot",
             models.Revision.id.in_(revision_ids)).all()
  ...
  """
    ...
    with benchmark("Compute attributes"):
        ...
        with benchmark("Get snapshot data"):
            snapshot_map, snapshot_tag_map = get_snapshot_data(
                affected_objects)
