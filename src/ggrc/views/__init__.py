...
def compute_attributes(args):
    ...
    with benchmark("Run compute_attributes background task"):
        ...
        return app.make_response(("success", 200, [("Content-Type",
                                                    "text/html")]))
...
def update_audit_issues(args):
    ...
    if not audit_id or not message:
        ...
        return app.make_response(
            ('Parameters audit_id and message are required.', 400,
    ...
    query = db.session.query(
        ...
        issue_tracker.issue_id, issue_tracker.object_id).join(
            relationships,
            relationships.source_id == issue_tracker.object_id).filter(
...
def do_reindex():
    ...
    people_query = db.session.query(
        all_models.Person.id, all_models.Person.name, all_models.Person.email)
...
def get_permissions_json():
    ...
    with benchmark("Get permission JSON"):
        ...
        return json.dumps(
            getattr(g, '_request_permissions', None), cls=SetEncoder)
...
def get_public_config():
    ...
    return {
        "external_help_url":
        getattr(settings, "EXTERNAL_HELP_URL", ""),
        "external_import_help_url":
        getattr(settings, "EXTERNAL_IMPORT_HELP_URL", ""),
        "snapshotable_objects":
        list(rules.Types.all),
        "snapshotable_ignored":
        list(rules.Types.ignore),
        "snapshotable_parents":
        list(rules.Types.parents),
        "external_services": {
            "Person": external_service
        },
    }
    ...
    with benchmark("Get access roles JSON"):
        attrs = all_models.AccessControlRole.query.options(
            sqlalchemy.orm.undefer_group("AccessControlRole_complete")).filter(
                ~all_models.AccessControlRole.internal).all()
        published = []
        for attr in attrs:
...
def get_internal_roles_json():
    ...
    with benchmark("Get access roles JSON"):
        attrs = all_models.AccessControlRole.query.options(
            sqlalchemy.orm.undefer_group("AccessControlRole_complete")).filter(
                all_models.AccessControlRole.internal == true()).all()
        published = []
        for attr in attrs:
            published.append(publish(attr))
        published = publish_representation(published)
...
def admin_reindex():
  ...
  """
    task_queue = create_task(
        name="reindex", url=url_for(reindex.__name__), queued_callback=reindex)
    return task_queue.make_response(
        app.make_response(("scheduled %s" % task_queue.name, 200,
                           [('Content-Type', 'text/html')])))
...
def admin_refresh_revisions():
    ...
    task_queue = create_task("refresh_revisions",
                             url_for(refresh_revisions.__name__),
                             refresh_revisions)
    return task_queue.make_response(
        app.make_response(("scheduled %s" % task_queue.name, 200,
                           [('Content-Type', 'text/html')])))
