...
def inclusion_filter(obj):
    return permissions.is_allowed_read(obj.__class__.__name__, obj.id,
                                       obj.context_id)
...
def update_snapshot_index(session, cache):
    ...
    get_indexer().delete_records_by_ids(
        "Snapshot", reindex_snapshots_ids, commit=False)
...
class ModelView(View):
    ...
    def _get_type_where_clause(model):
        ...
        polymorphic_on_values = list(
            val for val, m in mapper.polymorphic_map.items() if m in mappers)
    ...
    def get_resource_match_query(self, model, obj_id):
        ...
        query = db.session.query(*columns).filter(
            and_(self._get_type_where_clause(model), columns[0] == obj_id))
        return query
    ...
    def filter_query_by_request(self, query, filter_by_contexts=True):  # noqa
        ...
        if request.args:
            ...
            filter_, joinlist, _ = querybuilder.collection_filters(
                request.args)
        ...
        if "__no_role" in request.args:
            ...
            subq = db.session.query(user_roles_module.person_id).filter(
                or_(
                    ...
                    user_roles_module.context_id == 0)).subquery()
            filter_ = and_(
        ...
        if filter_by_contexts:
            ...
            for j in joinlist:
                ...
                if j_contexts is not None:
                    j_filter_expr = context_query_filter(
                        j_class.context_id, j_contexts)
                    if resources:
                        j_filter_expr = or_(j_filter_expr,
                                            self.model.id.in_(j_resources))
        ...
        if '__search' in request.args:
            ...
            search_query = db.session.query(
                indexer.record_type.key).filter(search_query)
    ...
    def get_object(self, obj_id):
        ...
        try:
            return self.get_collection(filter_by_contexts=False).filter(
                self.model.id == obj_id).one()
    ...
    """
        result = db.session.query(self.modified_attr).order_by(
            self.modified_attr.desc()).first()
...
class Resource(ModelView):
  ...
  """
        ...
        if 'If-None-Match' in self.request.headers and \
           self.request.headers['If-None-Match'] == obj_etag:
            with benchmark("Make response"):
                return current_app.make_response(('', 304, [('Etag',
                                                             obj_etag)]))
        with benchmark("Make response"):
    ...
    def validate_headers_for_put_or_delete(self, obj):
        ...
        if (request.headers["If-Match"] != object_etag
                or request.headers["If-Unmodified-Since"] != object_timestamp):
            return current_app.make_response((json.dumps({
                "message":
    ...
    def _check_put_permissions(self, obj, new_context):
        ...
        if (not permissions.is_allowed_update(self.model.__name__, obj.id,
                                              obj.context_id) and
        ...
        if (new_context != obj.context_id
                and not permissions.is_allowed_update(self.model.__name__,
                                                      obj.id, new_context) and
                not permissions.has_conditions('update', self.model.__name__)):
            raise Forbidden()
    ...
    def dump_attrs(self, obj):
        ...
        attrs = tuple(
            p.key for p in mapper.iterate_properties if p.key not in rel_keys)
        # TODO(anushovan): consider caching class definitions.
        attrs_cls = collections.namedtuple('Dumped%s' % obj_cls.__name__,
                                           attrs)
    ...
    def apply_paging(self, matches_query):
        ...
        if '__page_only' in request.args:
            ...
            if page_number == 1 and len(matches) < page_size:
                ...
                total = matches_query.count()
        page = Pagination(matches_query, page_number, page_size, total,
                          matches)
        collection_extras = {'paging': self.build_page_object_for_json(page)}
    ...
    def collection_get(self):
        ...
        with benchmark(
                "dispatch_request > collection_get > Collection matches"):
            ...
            filter_by_contexts = not (self.model.__name__ in ("Relationship",
                                                              "Revision")
                                      and _is_creator())
        ...
        with benchmark("dispatch_request > collection_get > Query Data"):
            if '__page' in request.args or '__page_only' in request.args:
                ...
                with benchmark("Query matches"):
                    ...
                    extras = {}
        with benchmark(
                "dispatch_request > collection_get > Matched resources"):
            cache_op = None
        ...
        with benchmark("dispatch_request > collection_get > Create Response"):
            ...
            if '__fields' in request.args:
                ...
                objs = [{f: o[f]
                         for f in custom_fields if f in o} for o in objs]
            with benchmark("Serialize collection"):
    ...
    def add_resources_to_cache(self, match_obj_pairs):
        ...
        for match, obj in match_obj_pairs.items():
            if obj.__class__.__name__ in cache_manager.supported_classes:
                memcache_client.add(
                    cache_utils.get_cache_key(
                        None, id_=match[0], type_=match[1]), as_json(obj))
    ...
    def _get_relationships_cache(self, body):
        ...
        if cache is not None:
            return cache
        relationships = self.model.query.filter(
            tuple_(self.model.source_id, self.model.source_type,
    ...
    """
        ...
        if self.model.__name__ == "Relationship":
            obj = self._get_relationships_cache(body).get(
                (src["source"]["id"], src["source"]["type"],
                 src["destination"]["id"], src["destination"]["type"]))
        if obj is None:
            obj = self.model()
            db.session.add(obj)
        return obj
        ...
        for class_name, ids in objects.items():
            ...
            if hasattr(class_, "query"):
                g.referenced_objects[class_] = {
                    obj.id: obj
                    for obj in class_.query.filter(class_.id.in_(ids))
                }
    ...
    """
        ...
        with benchmark("Serialize objects"):
            for obj in objects:
                object_for_json = {} if no_result else self.object_for_json(
                    obj)
        ...
        with benchmark("collection post"):
            ...
            if 'X-GGRC-BackgroundTask' in request.headers:
                if 'X-Appengine-Taskname' not in request.headers:
                    task = create_task(request.method, request.full_path, None,
                                       request.data)
            ...
            if body and 'person' in body[0]:
                ...
                if person.get('external'):
                    ...
                    obj = user_generator.find_or_create_external_user(
                        person['email'], person['name'])
                    if not obj:
                        return current_app.make_response(
                            ('application/json', 406, [('Content-Type',
                                                        'text/plain')]))
                    return self.json_success_response(
                        [(201, self.object_for_json(obj))])

            res = []
            headers = {"Content-Type": "application/json"}
            with benchmark("collection post > body loop: {}".format(
                    len(body))):
                ...
                try:
                    ...
                    logger.warning(
                        "Collection POST commit failed", exc_info=True)
                    db.session.rollback()
            ...
            with benchmark("collection post > calculate response statuses"):
                ...
                if wrap:
                    ...
                    if errors:
                        status = errors[0][0]
                        headers["X-Flash-Error"] = ' || '.join(
                            (error for _, error in errors))
                    else:
                        status = 200
            with benchmark("collection post > make response"):
                result = current_app.make_response((self.as_json(res), status,
                                                    headers))
    ...
    def get_resources_from_database(self, matches):
        ...
        with benchmark("Publish objects"):
            ...
            for obj in objs:
                resources[ids[obj.id]] = ggrc.builder.json.publish(
                    obj, includes)
    ...
    def json_success_response(self,
                              response_object,
                              last_modified=None,
                              status=200,
                              id=None,
        ...
        if last_modified:
            headers.append(('Last-Modified',
                            self.http_timestamp(last_modified)))
        if obj_etag:
        ...
        return current_app.make_response((self.as_json(response_object),
                                          status, headers))
...
class ReadOnlyResource(Resource):
    def dispatch_request(self, *args, **kwargs):
        ...
        if method == 'GET':
            return super(ReadOnlyResource, self).dispatch_request(
                *args, **kwargs)
        else:
            raise NotImplementedError()
    ...
    if isinstance(resource, (list, tuple)):
        ...
        if resource['type'] == "Relationship" and _is_creator():
            ...
            if not hasattr(ggrc.models.all_models, resource['resource_type']):
                ...
                return None
            instance = utils.referenced_objects.get(resource['resource_type'],
                                                    resource['resource_id'])
            if instance is None or\
               not user_permissions.is_allowed_read_for(instance):
                return None
        else:
            if not user_permissions.is_allowed_read(
                    resource['type'], resource['id'], context_id):
                return None
