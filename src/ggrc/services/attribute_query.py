...
class AttributeQueryBuilder(object):
    ...
    def coerce_value_for_query_param(self, attr, arg, value):
        ...
        if attr_type is Boolean:
            ...
            if value == 'true':
                ...
                raise BadRequest(
                    '{0} must be "true" or "false", not {1}.'.format(
                        arg, value))
        elif attr_type is DateTime:
            try:
                value = iso8601.parse_date(value)
            except iso8601.ParseError as e:
                raise BadRequest('Malformed DateTime {0} for parameter {0}. '
                                 'Error message was: {1}'.format(
                                     value, arg, e.message))
    ...
    def process_property_path(self, arg, value):
        ...
        if arg.endswith('__in'):
            value = value.split(',')
            value = [
                self.coerce_value_for_query_param(attr, arg, v) for v in value
            ]
    ...
    def collection_filters(self, args):
        ...
        for arg, value in args.items():
            try:
                joins, filters, options = self.process_property_path(
                    arg, value)
