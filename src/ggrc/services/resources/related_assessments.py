...
class RelatedAssessmentsResource(common.Resource):
    ...
    """
        ...
        query = models.Assessment.query.options(
            ...
            orm.Load(models.Assessment).joinedload("audit").undefer_group(
                "Audit_complete", ),
            orm.Load(
                models.Assessment).joinedload("custom_attribute_definitions")
            .undefer_group("CustomAttributeDefinitons_complete", ),
            orm.Load(models.Assessment).joinedload("custom_attribute_values")
            .undefer_group("CustomAttributeValues_complete", ),
    ...
    def _get_documents(cls, assessments):
        ...
        source_query = db.session.query(
            ...
            models.Document,
            and_(models.Relationship.source_type == models.Document.__name__,
                 models.Relationship.source_id == models.Document.id)).filter(
        ...
        destination_query = db.session.query(
            ...
            models.Relationship.destination_id.label("document_id")).join(
                models.Document,
                and_(models.Relationship.destination_type ==
                     models.Document.__name__,
        ...
        for assessment_id, document_id in source_query.union(
                destination_query):
        ...
        if all_document_ids:
            documents = models.Document.query.options(
                orm.Load(models.Document).undefer_group(
                    "Document_complete")).filter(
                        models.Document.id.in_(all_document_ids)).all()
    ...
    @staticmethod
    def _get_snapshots_json(assessment, assessment_snapshot_map,
                            snapshots_map):
    ...
    """
        ...
        for snapshot_id in assessment_snapshot_map[assessment.id]:
            ...
            snapshot_json["revision"] = {
                "content": {
                    "title": snapshot.revision.content["title"]
                }
            }
    ...
    def _get_snapshots(self, obj, assessments):
        ...
        assessment_snapshot_query = db.session.query(
            models.Relationship.source_id.label("assessment_id"),
            models.Relationship.destination_id.label("snapshot_id")).join(
                models.Snapshot,
                and_(models.Relationship.destination_type ==
                ...
                .join(
                    models.Snapshot,
                    and_(models.Relationship.source_type ==
                         models.Snapshot.__name__,
        ...
        snapshots = models.Snapshot.eager_inclusions(
            ...
            ["revision"]).filter(
                models.Snapshot.id.in_(all_snapshot_ids)).all()
