...
class AssessmentResource(common.ExtendedResource):
    ...
    """
        ids = [
            rel.source_id for rel in relationships if type_ in rel.source_type
        ]
    ...
    """
        ...
        with benchmark("Get assessment snapshot relationships"):
            documents = models.Document.eager_query().filter(
                models.Document.id.in_(relationship_ids)).all()
        urls = [
            doc.log_json() for doc in documents if doc.document_type == doc.URL
        ]
        ref_urls = [
            doc.log_json() for doc in documents
    ...
    def _get_issue_data(self, relationships):
        ...
        with benchmark("Get related issue data"):
            issues = models.Issue.eager_query().filter(
                models.Issue.id.in_(relationship_ids)).all()
    ...
    def _get_comment_data(self, relationships):
        ...
        with benchmark("Get assessment comment data"):
            comments = models.Comment.eager_query().filter(
                models.Comment.id.in_(relationship_ids)).order_by(
