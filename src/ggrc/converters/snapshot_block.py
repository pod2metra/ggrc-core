...
class SnapshotBlockConverter(object):
    ...
    def _cad_name_map(self):
        ...
        return OrderedDict(
            [(key, value["title"]) for key, value in self._cad_map.items()])
    ...
    def _attribute_name_map(self):
        ...
        if not model:
            logger.warning("Exporting invalid snapshot model: %s",
                           self.child_type)
            return {}
    ...
    def _stub_cache(self):
        ...
        for model_name, ids in stubs.iteritems():
            with benchmark(
                    "Generate snapshot cache for: {}".format(model_name)):
    ...
    def _access_control_map(self):
        ...
        for snap in self.snapshots:
            ...
            for acl in snap.content.get("access_control_list", []):
                ...
                _access_control_map[snap.content["id"]][role_name].append(
                    email)

            # Emails should be sorted in asc order
            _access_control_map[snap.content["id"]] = {
                role: sorted(emails)
                for role, emails in _access_control_map[snap.content["id"]]
                .items()
            }
    ...
    def get_value_string(self, value):
        ...
        if isinstance(value, dict) and "type" in value and "id" in value:
            return self._stub_cache.get(value["type"], {}).get(
                value["id"], u"")
        if isinstance(value, list):
    ...
    def _body_list(self):
        ...
        return [
            self._content_line_list(snapshot) for snapshot in self.snapshots
        ] or [[]]
