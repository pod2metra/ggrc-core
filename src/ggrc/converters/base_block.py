  ...
  """
    ...
    def __init__(self, converter, **options):
        ...
        if self.object_class:
            self.object_headers = get_object_column_definitions(
                self.object_class)
            all_header_names = [
                unicode(key) for key in self.get_header_names().keys()
            ]
    ...
    def check_block_restrictions(self):
        ...
        if not self.object_class:
            self.add_errors(
                errors.WRONG_OBJECT_TYPE,
                line=self.offset + 2,
                object_name=self.class_name)
            return
        if (self.operation == 'import'
                and self.object_class is CycleTaskGroupObjectTask
        ...
        if self._has_non_importable_columns:
            ...
            self.add_warning(
                errors.ONLY_IMPORTABLE_COLUMNS_WARNING,
                line=self.offset + 2,
    ...
    def _get_identifier_mappings(self, relationships):
        ...
        for object_type, ids in object_ids.items():
            ...
            if hasattr(model, "slug"):
                ...
                continue
            query = db.session.query(model.id, id_column).filter(
                model.id.in_(ids))
            ...
            with benchmark("building cache"):
                ...
                for rel in relationships:
                    if rel.source_type == self.object_class.__name__:
                        ...
                        identifier = id_map.get(rel.source_type, {}).get(
                            rel.source_id)
                        if identifier:
    ...
    def _create_ticket_tracker_cache(self):
        ...
        query = db.session.query(models.IssuetrackerIssue.object_id,
                                 models.IssuetrackerIssue.issue_url).filter(
                                     models.IssuetrackerIssue.object_id.in_(
                                         self.object_ids),
                                     models.IssuetrackerIssue.object_type ==
                                     models.Assessment.__name__)
        return dict(query)
    ...
    def get_role(self, name):
        ...
        if not self._roles_cache:
            self._roles_cache = {
                role.name: role
                for role in models.all_models.Role.query
            }
    ...
    """
        ...
        ).filter(models.all_models.UserRole.context_id.in_(context_ids)).all()
    ...
    def _create_owners_cache(self):
        ...
        if not owner_ids:
            return {}
        query = db.session.query(models.Person.id, models.Person.email).filter(
    ...
    """
        counter = Counter(
            header.strip().rstrip("*").lower() for header in raw_headers)
        duplicates = [header for header, count in counter.items() if count > 1]
        if duplicates:
            self.add_errors(
                errors.DUPLICATE_COLUMN,
                line=self.offset + 1,
    ...
    def get_header_names(self):
        ...
        header_names = {
            ...
            for k, v in self.object_headers.items()
        }
        return header_names
    ...
    """
        ...
        for index, raw_header in enumerate(raw_headers):
            ...
            if header in header_names:
                ...
                if (self.operation == 'import'
                        and hasattr(self.object_class, "IMPORTABLE_FIELDS") and
    ...
    def _store_revision_ids(self, event):
        ...
        if event:
            self.revision_ids.extend(
                revision.id for revision in event.revisions)
    ...
    def generate_unique_counts(self):
        ...
        for key, header in self.headers.items():
            ...
            for rel_index, row in enumerate(self.row_converters):
                ...
                if value:
                    self.unique_counts[key][value].add(
                        self.calc_abs_index(rel_index))
                ...
                if key == "slug":  # mark obj not to be expunged from the session
                    for index in indexes:
                        ...
                        if self.in_range(offset_index, remove_offset=False):
                            self.row_converters[
                                offset_index].set_do_not_expunge()
    ...
    """
        checker = pre_commit_checks.CHECKS.get(
