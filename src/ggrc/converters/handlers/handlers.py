  ...
  """
    ...
    def check_unique_consistency(self):
        ...
        nr_duplicates = self.row_converter.object_class.query.filter(
            and_(
                getattr(self.row_converter.object_class,
                        self.key) == self.value,
                self.row_converter.object_class.id !=
                self.row_converter.obj.id)).count()
        if nr_duplicates > 0:
            self.add_error(
...
class DeleteColumnHandler(ColumnHandler):
    ...
    def parse_item(self):
        if self.raw_value:
            self.add_error(
                u"Line {line}: Delete column is temporary disabled, "
    ...
    def set_obj_attr(self):
        ...
        if self.row_converter.is_new:
            self.add_error(
                errors.DELETE_NEW_OBJECT_ERROR,
            ...
            if self.dry_run or self.row_converter.ignore:
                ...
                if indexer is not None:
                    for o in tr.session.deleted:
                        indexer.delete_record(
                            o.id, o.__class__.__name__, commit=False)
                tr.commit()
...
class StatusColumnHandler(ColumnHandler):
    def __init__(self, row_converter, key, **options):
        ...
        super(StatusColumnHandler, self).__init__(row_converter, key,
                                                  **options)
...
class UsersColumnHandler(UserColumnHandler):
    ...
    def parse_item(self):
        ...
        if not people and self.mandatory:
            self.add_warning(
                errors.OWNER_MISSING, column_name=self.display_name)
...
class DateColumnHandler(ColumnHandler):
    ...
    def parse_item(self):
        ...
        if value and not re.match(r"[0-9]{4}-[0-9]{1,2}-[0-9]{1,2}|"
                                  ...
                                  self.raw_value.strip()):
            self.add_error(
                errors.UNKNOWN_DATE_FORMAT, column_name=self.display_name)
            return
    ...
    def check_last_asmnt_date(self, new_last_asmnt_date):
        ...
        old_last_asmnt_date = getattr(self.row_converter.obj,
...
class MappingColumnHandler(ColumnHandler):
    ...
    def __init__(self, row_converter, key, **options):
        ...
        super(MappingColumnHandler, self).__init__(row_converter, key,
                                                   **options)
  ...
  """
    ...
    def parse_item(self):
        ...
        prefixed_key = "{}_{}".format(
            self.row_converter.object_class._inflector.table_singular,
            self.key)
...
class ParentColumnHandler(ColumnHandler):
    ...
    def __init__(self, row_converter, key, **options):
        super(ParentColumnHandler, self).__init__(row_converter, key,
                                                  **options)
...
class ProgramColumnHandler(ParentColumnHandler):
    ...
    def __init__(self, row_converter, key, **options):
        self.parent = Program
        super(ProgramColumnHandler, self).__init__(row_converter, key,
                                                   **options)
...
class AuditColumnHandler(MappingColumnHandler):
    ...
    """
        ...
        if isinstance(audit, Audit):
            old_slug = None
            if (hasattr(self.row_converter.obj, "audit")
                    and self.row_converter.obj.audit):
                old_slug = self.row_converter.obj.audit.slug
            else:
                rel_audits = self.row_converter.obj.related_objects(
                    _types="Audit")
                if rel_audits:
    ...
    def insert_object(self):
        ...
        for person in self.value:
            ...
            if context and user_role:
                ...
                user_role.query.filter_by(
                    person=person, context=context).delete()
        self.dry_run = True
    ...
    def parse_item(self):
        ...
        categories = CategoryBase.query.filter(
            and_(
                ...
                CategoryBase.type == self.category_base_type)).all()
        category_names = set([c.name.strip() for c in categories])
        for name in names:
            if name not in category_names:
                self.add_warning(
                    errors.WRONG_MULTI_VALUE,
                    column_name=self.display_name,
                    value=name)
        return categories
...
class DocumentsColumnHandler(ColumnHandler):
    ...
    def parse_item(self):
        ...
        for line in lines:
            if len(line) != 2:
                self.add_warning(
                    errors.WRONG_VALUE, column_name=self.display_name)
                continue
            title, link = line
            documents.append(
