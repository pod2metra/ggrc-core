...
class DocumentLinkHandler(handlers.ColumnHandler):
    ...
    """
        ...
        if duplicate_new_links:
            ...
            self.add_warning(
                errors.DUPLICATE_IN_MULTI_VALUE,
    ...
    """
        ...
        for new_link, new_doc in new_link_map.iteritems():
            if new_link in old_link_map:
                ...
                models.Relationship(
                    source=self.row_converter.obj, destination=new_doc)
...
class DocumentUrlHandler(DocumentLinkHandler):
    ...
    """
        return "\n".join(
            doc.link for doc in self.row_converter.obj.document_url)
...
class ReferenceUrlHandler(DocumentLinkHandler):
    ...
    """
        return "\n".join(
            doc.link for doc in self.row_converter.obj.reference_url)
