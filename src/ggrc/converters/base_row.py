...
class RowConverter(object):
        ...
        for i, (attr_name, header_dict) in enumerate(self.headers.items()):
            ...
            if not self.ignore:
                if attr_name == "status" and hasattr(self.obj, "DEPRECATED"):
                    self.is_deprecated = (self.obj.DEPRECATED == item.value !=
                                          self.obj.status)
    ...
    def check_mandatory_fields(self):
        ...
        mandatory = [
            key for key, header in headers.items() if header["mandatory"]
        ]
        ...
        # custom attributes.
        missing = [
            headers[key]["display_name"] for key in missing_keys
            if headers[key]["type"] != AttributeInfo.Type.OBJECT_CUSTOM
        ]
        if missing:
            self.add_error(
