...
EmailProposalContext = collections.namedtuple("EmailProposalContext", [
...
def get_fields_list_values(proposal, acr_dict, person_dict):
  ...
  """
    ...
    for field_name, value in proposal.content[
            "mapping_list_fields"].iteritems():
        propose_dict = {}
        for action, items in value.iteritems():
...
def get_object_url(obj):
    return urlparse.urljoin(utils.get_url_root(), "{}/{}".format(
...
def _get_presented_attrs(object_type):
    ...
    if object_type not in flask.g.object_field_value_mapper:
        ...
        flask.g.object_field_value_mapper[object_type] = {
            a.alias: a.value_map
            for a in all_attrs if isinstance(a, ft_attrs.ValueMapFullTextAttr)
        }
...
def field_value_converter(values_dict, object_type):
    ...
    for field in values_dict:
        if field in attrs:
            value = getattr(object_type,
                            field).property.columns[0].type.python_type(
                                values_dict[field])
...
def addressee_body_generator(proposals):
    ...
    for proposal in proposals:
        if not isinstance(proposal.instance, roleable.Roleable):
            continue
        single_values = get_field_single_values(proposal, person_dict,
                                                cads_dict)
