...
def _generate_mapping_field_cache(mapping_fields, mapping_list_field):
    ...
    all_items = itertools.chain(*[
        itertools.chain(v['added'], v['deleted'])
...
def apply_mapping(instance, content):
    ...
    rel_names = [
        r.key for r in inspect(instance.__class__).relationships
        if not r.uselist
    ]
    ...
    for key, value in mapping_fields.iteritems():
        if key in rel_names:
            setattr(instance, key,
                    field_cache.get((value["type"], value["id"]))
                    if value else None)
