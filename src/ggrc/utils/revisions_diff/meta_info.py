...
class MetaInfo(object):
    ...
    def _updateable_attributes(self):
        return {
            k
            for k, v in reflection.AttributeInfo.gather_attr_dicts(
                self.instance.__class__, "_api_attrs").iteritems() if v.update
        }

    @cached_property.cached_property
    def _relationship_dict(self):
        """Generates relationships dict for self.instance, grouped by use_list."""
        relations = sa.inspection.inspect(
            self.instance.__class__).relationships
        ...
        descriptors = sa.inspection.inspect(
            self.instance.__class__).all_orm_descriptors
    ...
    def cads(self):
        ...
        if not isinstance(self.instance,
                          customattributable.CustomAttributable):
            return set()
    ...
    def fields(self):
        ...
        return [
            Field(c.name, not c.nullable
                  and c.name not in self._mandatory_attributes)
    ...
    def mapping_list_fields(self):
        ...
        return [
            Field(f, f in self._mandatory_attributes)
