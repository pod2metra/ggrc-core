...
class ImmutableDict(collections.Mapping):
    ...
    def __repr__(self):
        return "{cls}(**{arg})".format(
            cls=self.__class__.__name__, arg=dict(self._dict))
...
class MappingRule(BasicRule):
    ...
    VALUE = {Labels.MAP: True, Labels.UNMAP: True}
...
class IssueMappingRule(MappingRule):
    ...
    VALUE = {Labels.MAP: True, Labels.UNMAP: True, Labels.MAP_SNAPSHOT: True}
...
def _all_rules():
  ...
  """
    ...
    all_rules = {
        ...
        "Contract":
        all_models - {'Policy', 'Regulation', 'Contract', 'Standard'},
        "Control": all_models,
    ...
    all_rules.update({
        ...
        | {StaticSnapshotMappingRule(type_)
           for type_ in snapshots},
        "Assessment":
        {"Issue"} | {StaticSnapshotMappingRule(type_)
                     for type_ in snapshots},
        "Issue": {
            "Assessment", "Audit", "CycleTaskGroupObjectTask", "Issue",
            "Person", "Program", "Project", "RiskAssessment"
        } | {IssueMappingRule(type_)
             for type_ in snapshots},
    })
...
def _filter_rules(rules, flag):
    ...
    return {
        key: {rule[Labels.TYPE]
              for rule in value if rule[flag]}
