...
def publish(obj,
  ...
  """
    ...
    if publisher and getattr(publisher, '_publish_attrs', []):
        ...
        ret.update(
            publisher.publish_contribution(obj, inclusions, inclusion_filter,
                                           attribute_whitelist))
        return ret
...
def update(obj, json_obj):
  ...
  """
    ...
    if updater:
        ...
        logger.warning(
            "No updater available. Obj might not be updated correctly.")
...
class UpdateAttrHandler(object):
  ...
  """
    ...
    """
        ...
        if (isinstance(value, (set, list)) and not update_raw and
            (not hasattr(class_attr, 'property')
             or not hasattr(class_attr.property, 'columns') or
             not isinstance(class_attr.property.columns[0].type, JsonType))):
            cls._do_update_collection(obj, value, attr_name)
        else:
            try:
                setattr(obj, attr_name, value)
            except AttributeError as error:
                logger.error('Unable to set attribute %s: %s', attr_name,
                             error)
                raise
    ...
    def ColumnProperty(cls, obj, json_obj, attr_name, class_attr):
        ...
        method = getattr(
            cls, class_attr.property.expression.type.__class__.__name__,
    ...
    def DateTime(cls, obj, json_obj, attr_name, class_attr):
        ...
        try:
            ...
            raise BadRequest('Malformed DateTime {0} for parameter {1}. '
                             'Error message was: {2}'.format(
                                 value, attr_name, error.message))
    ...
    def Date(cls, obj, json_obj, attr_name, class_attr):
        ...
        try:
            ...
            try:
                ...
                raise BadRequest('Malformed Date {0} for parameter {1}. '
                                 'Error message was: {2}'.format(
                                     value, attr_name, error.message))
            ...
            if rel_obj:
                try:
                    ...
                    raise TypeError(''.join(
                        ['Failed to convert attribute ', attr_name]))
            return None
    ...
    def RelationshipProperty(cls, obj, json_obj, attr_name, class_attr):
        ...
        return cls.query_for(rel_class, json_obj, attr_name,
                             class_attr.property.uselist)
...
def build_stub_union_query(queries):
    ...
    for (type_, conditions) in queries:
        if isinstance(conditions, (int, long, str, unicode)):
            ...
            results.setdefault(type_, {}).setdefault(keys, {}).setdefault(
                vals, [])
...
class LazyStubRepresentation(object):
    def __init__(self, type_, conditions):
        ...
        self.condition_key, self.condition_val = zip(*sorted(
            conditions.items()))
...
class Builder(AttributeInfo):
    ...
    def publish_link_collection(self, join_objects, inclusions, include,
                                inclusion_filter):
    ...
    """
        ...
        if attr_value:
            return self.generate_link_object_for(attr_value, inclusions,
                                                 include, inclusion_filter)

        return None

    def publish_association_proxy(self, obj, attr_name, class_attr, inclusions,
                                  include, inclusion_filter):
        ...
        if include:
            ...
            return self.publish_link_collection(target_objects, inclusions,
                                                include, inclusion_filter)
        else:
            if isinstance(class_attr.remote_attr,
                          (property, PolymorphicRelationship)):
                ...
                if len(list(target_mapper.self_and_descendants)) > 1:
                    ...
                    return [
                        self.generate_link_object_for(
                            getattr(o, target_attr), inclusions, include,
                            inclusion_filter) for o in join_objects
                    ]
                else:
                    target_name = list(
                    ...
                    return [
                        LazyStubRepresentation(target_type,
                                               getattr(o, target_name))
                        for o in join_objects
                    ]

    def publish_relationship(self, obj, attr_name, class_attr, inclusions,
                             include, inclusion_filter):
        uselist = class_attr.property.uselist
        if uselist:
            join_objects = getattr(obj, attr_name)
            return self.publish_link_collection(join_objects, inclusions,
                                                include, inclusion_filter)
        ...
        if isinstance(class_attr, AssociationProxy):
            if getattr(class_attr, 'publish_raw', False):
                ...
                result = self.publish_association_proxy(
                    ...
                    inclusion_filter)
        elif (isinstance(class_attr, InstrumentedAttribute)
              and isinstance(class_attr.property, RelationshipProperty)):
            result = self.publish_relationship(obj, attr_name, class_attr,
                                               inclusions, include,
                                               inclusion_filter)
            ...
            if not inclusions or include:
                ...
                result = self.publish_link(obj, attr_name, inclusions, include,
                                           inclusion_filter)
        else:
    ...
    def _publish_attrs_for(self,
        ...
        for attr in attrs:
            ...
            json_obj[attr_name] = self.publish_attr(
                obj, attr_name, local_inclusion[1:],
                len(local_inclusion) > 0, inclusion_filter)
    ...
    """
        ...
        return self._publish_attrs_for(obj, self._publish_attrs, json_obj,
                                       inclusions, inclusion_filter,
                                       attribute_whitelist)
