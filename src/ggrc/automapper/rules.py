...
def _check_rule_type_order(type_levels, *type_sets):
  ...
  """
    try:
        levels = [{(type_levels[type_])
                   for type_ in set_} for set_ in type_sets]
    except KeyError as e:
        raise AutomappingRuleConfigError("Unknown level for {}".format(
            e.args[0]))

    for i, level in enumerate(levels[1:], 1):
        if max(level) < min(levels[i - 1]):
            raise AutomappingRuleConfigError(
                "All types {} must be higher than all types {}".format(
                    type_sets[i - 1], type_sets[i]))
...
class Types(object):
    ...
    all = {
        'Program', 'Regulation', 'Policy', 'Standard', 'Contract', 'Section',
        'Clause', 'Objective', 'Control'
    }
