...
class SnapshotGenerator(object):
    ...
    def _get_snapshottable_objects(self, obj):
        ...
        with benchmark("Snapshot._get_snapshotable_objects"):
            ...
            with benchmark(
                    "Snapshot._get_snapshotable_objects.related_mappings"):
                relatable_rules = {
                    rule
            ...
            with benchmark(
                    "Snapshot._get_snapshotable_objects.direct mappings"):
                direct_mappings = {
                    getattr(obj, rule.name)
                    for rule in object_rules["fst"] if isinstance(rule, Attr)
                }

            related_objects = {
                Stub.from_object(obj)
                for obj in related_mappings | direct_mappings
            }

            with benchmark(
                    "Snapshot._get_snapshotable_objects.fetch neighborhood"):
    ...
    def analyze(self):
        ...
        query = set(
            db.session.query(
                ...
                tuple_(models.Snapshot.parent_type,
                       models.Snapshot.parent_id).in_(self.parents)))
    ...
    def _remove_lost_snapshot_mappings(self):
        ...
        lost_rel_ids = db.session.query(all_models.Relationship.id).join(
            ...
            dest_snap,
            dest_snap.id == all_models.Relationship.destination_id).outerjoin(
                source_rel,
                sa.and_(
                    ...
                    sa.and_(
                        ...
                        source_rel.id.is_(None), dest_rel.id.is_(None),
                        sa.tuple_(
    ...
    with benchmark("Snapshot.create_snapshots"):
        ...
        with benchmark("Snapshot.create_snapshots.create"):
            return generator.create(
                event=event, revisions=revisions, _filter=_filter)
  ...
  """
    ...
    with benchmark("clone_scope.clone audit scope"):
        source_snapshots = db.session.query(
            models.Snapshot.child_type,
            models.Snapshot.child_id, models.Snapshot.revision_id).filter(
