...
def _get_custom_attribute_dict():
  ...
  """
    ...
    cadef_klass_names = {
        getattr(all_models, c)._inflector.table_singular: c
        for c in Types.all
    }
...
def delete_records(snapshot_ids):
  ...
  """
    db.session.query(Record).filter(
        ...
        Record.key.in_(snapshot_ids)).delete(synchronize_session=False)
    db.session.commit()
...
def get_access_control_role_data(rec, ac_list_item):
  ...
  """
    ...
    if ac_role_name:
        for key, val in builder.build_person_subprops({
                "id": person_id
        }).items():
            newrec = rec.copy()
            newrec.update({
                "property": ac_role_name,
                "subproperty": key,
                "content": val
            })
            yield newrec
...
def get_access_control_sort_subprop(rec, access_control_list):
  ...
  """
    ...
    for ac_role_name, people in collection.iteritems():
        for prop in get_person_sort_subprop({
                "property": ac_role_name
        }, people):
            newrec = rec.copy()
            newrec.update(prop)
...
def get_category_sort_subprop(rec, category_list):
  ...
  """
    ...
    newrec.update({
        "content":
        ":".join(sorted([c.get("display_name", None) for c in category_list])),
        "subproperty":
        "__sort__",
    })
    yield newrec
...
def get_record_value(prop, val, rec):
    ...
    if isinstance(val, dict) and val.get("type") == "Person":
        return itertools.chain(
            get_person_data(rec, val), get_person_sort_subprop(rec, [val]))
    ...
    if isinstance(rec["content"], basestring):
        return [rec]
    logger.warning(u"Unsupported value for %s #%s in %s %s: %r", rec["type"],
                   rec["key"], rec["property"], rec["subproperty"],
                   rec["content"])
    return []
