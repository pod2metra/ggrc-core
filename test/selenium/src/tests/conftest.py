    ...
    if driver is not None and capture_debug and pytest_html is not None:
        ...
        if "screenshot" not in exclude:
            try:
                ...
                extra.append(
                    pytest_html.extras.image(screenshot, "Screenshot"))
...
def chrome_options(chrome_options, create_tmp_dir):
    ...
    prefs = {
        "download.default_directory": create_tmp_dir,
...
def _common_request_param(request):
  ...
  """
    ...
    if hasattr(request, "param") and request.param:
        ...
        return (({param: _common_fixtures(param)
                  for param in params}
                 if isinstance(params, list) else _common_fixtures(params))
                if params else None)
