...
class TestAdminDashboardPage(base.Test):
    ...
    def test_events_widget_tree_view_has_data(self, admin_dashboard):
        ...
        items_with_incorrect_format = [
            getattr(item, 'text') for item in list_items
            if not re.compile(self._event_el.TREE_VIEW_ROW_REGEXP).match(
                getattr(item, 'text'))
        ]
    ...
    @pytest.mark.parametrize(
        "ca_type, def_type",
        [(ca_type_item,
          objects.get_normal_form(random.choice(objects.ALL_CA_OBJS)))
