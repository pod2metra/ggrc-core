...
class TestAuditPage(base.Test):
            ...
            if isinstance(request.param, tuple):
                ...
                if fixture in request.fixturenames and fixture.startswith(
                    ...
                    (get_cls_rest_service(objects.get_plural(
                        fixture.type))().update_obj(
                            obj=fixture, **params_to_update))
    ...
    @pytest.mark.parametrize(
        ...
        indirect=True)
    def test_asmt_creation_with_mapping(self, new_program_rest,
                                        dynamic_objects, dynamic_relationships,
    ...
    @pytest.mark.parametrize(
        ...
        ids=[
            "Assessments generation without Assessment Template",
    ...
    """
        expected_asmts = (entities_factory.AssessmentsFactory().generate(
            mapped_objects=new_controls_rest,
            audit=new_audit_rest,
    ...
    @pytest.mark.parametrize(
        ...
        [("new_audit_rest", {
            "status": AuditStates.PLANNED
        }), ("new_audit_rest", {
            "status": AuditStates.IN_PROGRESS
        }), ("new_audit_rest", {
            "status": AuditStates.MANAGER_REVIEW
        }), ("new_audit_rest", {
            "status": AuditStates.READY_FOR_EXT_REVIEW
        }), ("new_audit_rest", {
            "status": AuditStates.COMPLETED
        })],
        ids=[
            "Audit statuses: 'Planned' - 'Planned'",
    ...
    """
        ...
        self.general_equal_assert(expected_audit, actual_audit, "created_at",
                                  "updated_at", "slug", "program")
    ...
    @pytest.mark.parametrize(
        "create_and_clone_audit_w_params_to_update",
        [("new_assessment_template_rest", {
            "status": ObjectStates.DRAFT
        }), ("new_assessment_template_rest", {
            "status": ObjectStates.DEPRECATED
        }), ("new_assessment_template_rest", {
            "status": ObjectStates.ACTIVE
        })],
        ids=[
            "Assessment Template's statuses: 'Draft' - 'Draft'",
    ...
    """
        ...
        exclude_attrs = (Representation.tree_view_attrs_to_exclude +
                         ("slug", "modified_by", "audit", "assignees",
                          "verifiers", "template_object_type"))
        self.general_equal_assert(
            expected_asmt_tmpl, actual_asmt_tmpls,
            *(exclude_attrs
              if not is_expect_ggrc_3423 else exclude_attrs + ("status", )))
