...
class TestObjectivePage(base.Test):
    ...
    @pytest.mark.parametrize(
        ...
        ids=[
            "Check if Objective can be mapped to Control via REST",
    ...
    def test_mapping_objectives_to_control_via_rest(
        ...
        dynamic_relationships = help_utils.convert_to_list(
        ...
        assert ((rest_service.RelationshipsService().map_objs(
            ...
            dest_objs=dynamic_objects))[0].status_code == expected_status_code
                ) if is_map_again else True
        actual_objectives_tab_count = (webui_service.ObjectivesService(
            selenium).get_count_objs_from_tab(src_obj=new_control_rest))
