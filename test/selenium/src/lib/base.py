...
class InstanceRepresentation(object):
    def __repr__(self):
        return str({
            key: value
            for key, value in self.__dict__.items() if "__" not in key
        })
...
class Test(InstanceRepresentation):
    ...
    """
        ...
        actual_objs_wo_excluded_attrs = (
            Representation.extract_objs_wo_excluded_attrs(
                help_utils.convert_to_list(actual_objs), *exclude_attrs))
        assert (expected_obj_wo_excluded_attrs in actual_objs_wo_excluded_attrs
    ...
    """
        ...
        Test.check_xfail_or_fail(
            is_condition=is_list_excluded_attrs_equal,
            issue_msg=issue_msg,
...
class Element(InstanceRepresentation):
  ...
  """
    ...
    """
        ...
        return selenium_utils.get_when_visible(self._driver,
...
class MultiInputField(Element):
    ...
    def add_values(self, *values):
        ...
        for value in values:
            self.add_btn.click()
            TextInputField(self.element,
                           self._locators.TXT_CSS).enter_text(value)
...
class Widget(AbstractPage):
    ...
    """
        ...
        self.widget_name_from_url = (url_parts["widget_name_from_url"].replace(
            "!", ""))
    ...
    checking existing of element by locator and URL's logic."""
        ...
        if selenium_utils.is_element_exist(
                self._driver,
            (By.XPATH, constants.locator.Common.INFO_PAGE_XPATH)):
            if ((self.widget_name_from_url in url.Widget.INFO) or
                ((objects.get_singular(
                    self.source_obj_from_url) == self.mapped_obj_from_url) and
                 (self.source_obj_id_from_url == self.mapped_obj_id_from_url))
                    or (self.widget_name_from_url == self.mapped_obj_from_url
    ...
    def is_snapshoted_panel(self):
        ...
        return (not self.is_info_page
                and (self.source_obj_from_url in (objects.AUDITS,
                                                  objects.ASSESSMENTS,
                                                  objects.ISSUES))
                and (objects.get_plural(self.widget_name_from_url.lower()) in
                     objects.ALL_SNAPSHOTABLE_OBJS))
...
class TreeView(Component):
    ...
    def __init__(self, driver, obj_name=None):
        ...
        if self.obj_name is not None:
            ...
            self.fields_to_set = factory.get_fields_to_set(
    ...
    """
        ...
        if self._init_tree_view_items():
            list_headers = [
                _item.text.splitlines()[:len(self.fields_to_set)]
                for _item in self.tree_view_headers()
            ]
            # u'Ex' to u'Ex', u'Ex1, Ex2' to [u'Ex1', u'Ex2']
            list_lists_items = [[
                _.split(", ") if len(_.split(", ")) >= 2 else _
                for _ in _item.cell_values[:len(self.fields_to_set)]
            ] for _item in self.tree_view_items()]
            list_scopes = [
...
class SetVisibleFieldsModal(Modal):
  ...
  Tree View."""
    ...
    according to titles of fields to set."""
        ...
        _locator_modal_fields = (By.CSS_SELECTOR, self._locators.MODAL)
        _locator_fields_titles = (By.CSS_SELECTOR,
    ...
    def confirm_set_visible_fields(self):
        ...
        Button(self._driver,
               _locator_set_fields).click_via_js()  # issue GGRC-1854
...
class TreeViewItem(Element):
    ...
    def cell_values(self):
        ...
        return [
            cell.text for cell in selenium_utils.get_when_all_visible(
...
class CommentsPanel(Element):
    ...
    def __init__(self, driver, locator_or_element):
        ...
        self._items = []
        self.header_lbl = Label(self.element, self._locators.HEADER_LBL_CSS)
        self.input_txt = RichTextInputField(self.element,
                                            self._locators.INPUT_TXT_CSS)
        self.add_btn = Button(self.element, self._locators.ADD_BTN_CSS)
    ...
    """
        self._items = [
            CommentItem(self._driver,
                        element) for element in self.element.find_elements(
                            *self._locators.ITEMS_CSS)
        ]
        return [{
            "modified_by": item.author.text,
            "created_at": item.datetime.text,
    ...
    def add_comments(self, *comments):
        ...
        for comment in list(*comments):
            ...
            try:
                selenium_utils.wait_until_condition(
                    self._driver,
                    lambda items: len(self.scopes) == count_of_comments)
...
class ListCheckboxes(Component):
    ...
    elements) and list of titles."""
        unselect = [
            obj[1].click() for obj in objs if obj[0].text not in list_titles
            if obj[1].is_selected()
        ]
        return unselect
    ...
    elements) and list of titles."""
        select = [
            obj[1].click() for obj in objs if obj[0].text in list_titles
            if not obj[1].is_selected()
        ]
        return select
    ...
    def get_mapping_statuses(self):
        ...
        objs_checkboxes = self._driver.find_elements(*self.locator_checkboxes)
        objs = [
            MappingStatusAttrs(obj[0], obj[1][0], obj[1][1])
            for obj in zip([obj.text for obj in objs_titles],
                           [[obj.is_selected(), not obj.is_enabled()]
...
class ElementsList(Component):
  ...
  """
    ...
    """
        element = (element_or_locactor
                   if isinstance(element_or_locactor,
                                 webdriver.remote.webelement.WebElement)
                   else selenium_utils.get_when_visible(
                       self._driver, element_or_locactor))
    ...
    """
        if not self._items:
            self._items = [
                self.item_class(self._driver, el)
                for el in self.list_element.find_elements(
    ...
    def get_item(self, text):
        ...
        return next(
            elem_val for elem_val in self.get_items() if text in elem_val.text)
