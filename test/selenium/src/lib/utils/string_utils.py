...
class StringMethods(object):
    ...
    @classmethod
    def random_list_strings(cls,
                            list_len=RANDOM_STR_LISTS_COUNT,
    ...
    """
        ...
        if isinstance(arg, (str, unicode)):
            ...
            converted_value = (True if arg.title() in [
                alias.YES_VAL, alias.TRUE_VAL, "1"
            ] else False if arg.title() in [
                alias.NO_VAL, alias.FALSE_VAL, "0"
            ] else arg)
    ...
    """
        ...
        for arg in args:
            if isinstance(arg, (str, unicode)):
                if (str(arg).isdigit() or all(
                        char in string.punctuation for char in str(arg))):
    ...
    @staticmethod
    def exchange_dicts_items(transform_dict, dicts, is_keys_not_values=True):
    ...
    """
        ...
        def replace_dict_values(dic):
            ...
            return {
                k: (transform_dict.get(k) if k in transform_dict.keys() else v)
                for k, v in dic.iteritems()
            }

        if isinstance(dicts, dict):
            dicts = (replace_dict_keys(dicts)
                     if is_keys_not_values else replace_dict_values(dicts))
        if isinstance(dicts, list) and all(
                isinstance(dic, dict) for dic in dicts):
            dicts = [
                replace_dict_keys(dic)
                if is_keys_not_values else replace_dict_values(dic)
                for dic in dicts
            ]
        return dicts
    ...
    """
        return [{(dict_of_transform_keys[k]
                  if dict_of_transform_keys.get(k) else k): v
    ...
    """
        return all(
            item in dest_dict.iteritems() for item in src_dict.iteritems())
