...
class FilterUtils(object):
    ...
    """
        ...
        return FilterUtils.get_filter_exp(
            field="title", operator=alias.EQUAL_OP, list_values=list_values)
    ...
    """
        cas = [(cas_def["title"], cas_val, cas_def["attribute_type"])
               for cas_id, cas_val in cas_ids_with_val.iteritems()
               for cas_def in cas_defs if cas_def["id"] == cas_id]
        return ([
            exprs for ca in cas
