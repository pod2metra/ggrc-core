...
def get_single_obj(obj):
  ...
  accordingly.
  """
    return (obj[0] if (not is_multiple_objs(obj)
                       and isinstance(obj, (list, tuple))) else obj)


def execute_method_according_to_plurality(objs,
                                          method_name,
                                          types=None,
                                          **method_kwargs):
  ...
  """
    ...
    return ([method_name(obj, **method_kwargs) for obj in objs]
            if is_multiple_objs(objs, types) else method_name(
                get_single_obj(objs), **method_kwargs))
...
def convert_to_list(items):
  ...
  - if are not list items then convert to list items."""
    list_items = items if isinstance(items, list) else [
        items,
    ]
    return list_items
