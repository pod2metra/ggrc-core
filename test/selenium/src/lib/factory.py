...
def _factory(cls_name, parent_cls, search_nested_subclasses=False):
 ...
 """
    ...
    members = (_all_subclasses(parent_cls)
               if search_nested_subclasses else parent_cls.__subclasses__())
    for member_cls in members:
        if member_cls.__name__.lower() == subcls_name:
            break
    else:
        raise exception.NoClassFound("%s for parent %s" % (subcls_name,
                                                           parent_cls))
    return member_cls
...
def get_cls_entity_factory(object_name):
    ...
    return _factory(
        cls_name=cls_name, parent_cls=base_cls, search_nested_subclasses=True)


def get_cls_obj_entity(object_name):
    """Get and return class of object factory."""
    from lib.entities import entity
    cls_name = (objects.get_singular(object_name, title=True) +
...
def get_locator_add_widget(widget_name):
 ...
 dropdown in widget bar"""
    # todo: unittests
    return getattr(constants.locator.WidgetBarButtonAddDropdown,
                   widget_name.upper())
