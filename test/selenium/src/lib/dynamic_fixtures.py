  ...
  executed fixtures."""
    ...
    if fixture.endswith("_snapshot"):
        ...
        dict_executed_fixtures.update({
            fixture:
            Representation.convert_repr_to_snapshot(
                objs=origin_obj, parent_obj=parent_obj)
        })
    return {
        k: v
        for k, v in dict_executed_fixtures.iteritems() if k == fixture
    }[fixture]
  ...
  """
                ...
                return factory.get_cls_rest_service(
                    object_name=name)().create_objs(
                        ...
                        generate_ca_defenitions_for_asmt_tmpls(
                            list_ca_definitions=extra_attrs[:len(
                                _list_cas_types)]),
    ...
    if obj_name == objects.AUDITS:
        parent_obj_name = (objects.get_singular(objects.PROGRAMS)
                           if obj_count == 1 else objects.PROGRAMS)
  ...
  """
    ...
    """
        if "new_cas_for_" in fixture:
            fixture_params = fixture.replace("new_cas_for_", "").replace(
                "_rest", "")
    ...
    """
        ...
        if (objects.get_plural(obj_name) in objects.ALL_OBJS
            ...
            obj_count = 1
            objs_info_pages = [
                conftest_utils.create_obj_via_lhn(web_driver,
                                                  getattr(
                                                      element.Lhn,
                                                      obj_name.upper()))
    ...
    for fixture in fixtures:
        ...
        if isinstance(fixture, str):
            if (fixture.startswith("new_") and fixture.endswith("_ui")
                    and dict_executed_fixtures.get("selenium")):
                new_objs = new_ui_fixture(
                    web_driver=dict_executed_fixtures["selenium"],
                    fixture=fixture)
                dict_executed_fixtures.update({fixture: new_objs})
            elif fixture.endswith(("_rest", "_snapshot")):
                if fixture.startswith("new_"):
                    new_objs = new_rest_fixture(
                        fixture=fixture, factory_params=fixture_params)
                    dict_executed_fixtures.update({fixture: new_objs})
                    ...
                    updated_objs = update_rest_fixture(
                        fixture=fixture, factory_params=fixture_params)
    ...
    executed_fixtures_copy = (copy.deepcopy(dict_executed_fixtures)
                              if not dict_executed_fixtures.get("selenium")
                              else copy.copy(dict_executed_fixtures))
