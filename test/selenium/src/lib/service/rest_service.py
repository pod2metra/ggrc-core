...
class BaseRestService(object):
    ...
    """
        ...
        return [
            self.set_obj_attrs(
                obj=factory_obj, attrs=attrs, **attrs_for_template)
    ...
    """
        ...
        return [
            self.set_obj_attrs(
                obj=new_obj, attrs=new_attrs, **new_obj.__dict__)
    ...
    def update_obj(self, obj, **attrs):
        ...
        return self.set_obj_attrs(
            obj=obj,
            attrs=self.get_items_from_resp(
    ...
    items {key: value} from it."""
        ...
        def get_extra_items(resp_dict):
            ...
            if resp_dict.get("viewLink"):
                extra.update({
                    "url":
                    environment.APP_URL + resp_dict.get("viewLink")[1:]
                })
            return extra
            ...
            try:
                ...
                raise requests.exceptions.ContentDecodingError(
                    messages.ExceptionsMessages.err_server_req_resp.format(
                        resp.request.body, resp.status_code + unicode_err,
                        resp.text))
    ...
    """
        obj.__dict__.update(
            {k: v
             for k, v in attrs.iteritems() if v and k not in [
                 "type",
             ]})
        if kwargs:
            # for Audit objects
    ...
    """
        return [
            self.client.create_object(
                type=objects.get_singular(self.endpoint),
    ...
    def create_assessments(self, audit, template, control_snapshots):
        ...
        for control_snapshot in control_snapshots:
            ...
            assessment.update_attrs(
                audit=audit, template=template, object=control_snapshot)
...
class ObjectsInfoService(HelpRestService):
    ...
    def get_obj(self, obj):
        ...
        obj_dict = (BaseRestService.get_items_from_resp(
            self.client.create_object(
                type=self.endpoint,
                object_name=unicode(obj.type),
                filters=query.Query.expression_get_obj_by_id(
                    obj.id))).get("values")[0])
    ...
    created datetime.
    """
        comment_obj_dict = (BaseRestService.get_items_from_resp(
            self.client.create_object(
                type=self.endpoint,
