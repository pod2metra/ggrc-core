...
class BaseWebUiService(object):
                ...
                if val:
                    ...
                    if key in ["updated_at", "created_at"]:
                        ...
                        if str(datetime_val.time()) != "00:00:00":
                            ...
                            datetime_val = (
                                datetime_val.astimezone(tz=tz.tzutc())
                                if datetime_val.tzinfo else
                                datetime_val.replace(tzinfo=tz.tzutc()))
    ...
    """
        ...
        selenium_utils.open_url(
            self.driver, generic_widget_url, is_via_js=True)
    ...
    def get_list_objs_from_mapper(self, src_obj, dest_objs):
     ...
     list of MappingStatusAttrs - namedtuples for mapping representation."""
        ...
        list_objs_scopes, mapping_statuses = (self._search_objs_via_tree_view(
            src_obj, dest_objs))
    ...
    """
        ...
        return ([get_obj_from_info_panel(src_obj, obj) for obj in objs]
                if isinstance(objs, list) else get_obj_from_info_panel(
                    src_obj, objs))
    ...
    """
        ...
        if self.obj_name == obj_name_from_dict:
            ...
            raise ValueError(
                messages.ExceptionsMessages.err_csv_format.format(
                    dict_list_objs_scopes))
    ...
    """
        ...
        objs_widget = self.open_widget_of_mapped_objs(src_obj)
        datetime_of_export_clicking = (objs_widget.tree_view.open_3bbs(
        ).select_export().export_objs_to_csv_and_return_datetime())
        # Control Snapshot_, Control_
        obj_part = "{obj_type}{snapshot_obj_type}_".format(
            obj_type=self.obj_type,
            snapshot_obj_type=(" " + self.snapshot_obj_type
                               if self.snapshot_obj_type else ""))
    ...
    """
        ...
        (dest_objs_widget.tree_view.open_map().map_dest_objs(
    ...
    """
        # pylint: disable=invalid-name
        mapper = self._get_unified_mapper(src_obj)
        mapper.tree_view.open_set_visible_fields(
        ).select_and_set_visible_fields()
    ...
    """
        dropdown_on_info_panel = (self.open_info_panel_of_obj_by_title(
            src_obj, obj).open_info_3bbs())
...
class SnapshotsWebUiService(BaseWebUiService):
    ...
    """
        objs_widget = self.open_widget_of_mapped_objs(src_obj)
        obj_info_panel = (objs_widget.tree_view.select_member_by_title(
            title=obj.title).panel)
    ...
    """
        obj_info_panel = (self.open_info_panel_of_obj_by_title(src_obj,
                                                               obj).panel)
...
class AuditsService(BaseWebUiService):
    ...
    """
        ...
        actual_cloned_audit_obj = self.get_obj_from_info_page(
            obj=cloned_audit_obj)
        self.driver.refresh()
...
class AssessmentTemplatesService(BaseWebUiService):
    ...
    Tree View, fill data according to object attributes and create new object.
    """
        (self._open_create_modal_and_fill_data(src_obj, obj).select_assignee(
            obj.assignees).save_and_close())
...
class AssessmentsService(BaseWebUiService):
    ...
    def __init__(self, driver):
        super(AssessmentsService, self).__init__(driver, objects.ASSESSMENTS)
    ...
    """
        ...
        obj_info_panel = self.open_info_panel_of_obj_by_title(src_obj, obj)
        return obj_info_panel.comments_panel.add_comments(
            comments_descriptions)

    def generate_objs_via_tree_view(self,
                                    src_obj,
                                    objs_under_asmt,
                                    asmt_tmpl_obj=None):
    ...
    """
        objs_under_asmt_titles = [
            obj_under.title for obj_under in objs_under_asmt
        ]
        objs_widget = self.open_widget_of_mapped_objs(src_obj)
        asmt_tmpl_title = asmt_tmpl_obj.title if asmt_tmpl_obj else None
        (objs_widget.tree_view.open_3bbs().select_generate().generate_asmts(
            asmt_tmpl_title=asmt_tmpl_title,
    ...
    """
        ...
        return [
            issue[element.RelatedIssuesTab.TITLE.upper()]
    ...
    """
        # pylint: disable=invalid-name
        modal_edit = (
            self.open_info_page_of_obj(src_obj).open_info_3bbs().select_edit())
