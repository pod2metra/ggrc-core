...
class Common(object):
    ...
    INFO_PAGE_XPATH = ("//section[contains(@class,'{}')]".format(_INFO_WIDGET)
                       + XPATH_NOT_HIDDEN)
    # import / export pages
    CONTENT = ".content"
    OPTION = "option"
...
class CommonAssessment(object):
    ...
    MAPPED_SNAPSHOT_TITLE_CSS = (By.CSS_SELECTOR,
                                 MAPPED_SNAPSHOTS + Common.TITLE)
    MAPPED_SNAPSHOT_DESCRIPTION_CSS = (By.CSS_SELECTOR,
                                       MAPPED_SNAPSHOTS + Common.DESCRIPTION)
...
class PageHeader(object):
    ...
    BUTTON_DASHBOARD = (By.CSS_SELECTOR,
                        _CONTENT + ' .to-my-work[href="/dashboard"]')
    BUTTON_SEARCH = (By.CSS_SELECTOR,
                     _CONTENT + ' [data-toggle="unified-search"]')
    BUTTON_MY_TASKS = (By.CSS_SELECTOR,
                       _CONTENT + ' [href="/dashboard#!task_widget"]')
    BUTTON_ALL_OBJECTS = (By.CSS_SELECTOR,
                          _CONTENT + ' [href^="/objectBrowser"]')
    TOGGLE_USER_DROPDOWN = (By.CSS_SELECTOR, _CONTENT + " .dropdown-toggle")
    GENERIC_SUCCESS_ALERT = (By.CSS_SELECTOR, ".alert-success")
    BUTTON_DATA_IMPORT = (By.CSS_SELECTOR, _CONTENT + ' [href="/import"]')
    BUTTON_DATA_EXPORT = (By.CSS_SELECTOR, _CONTENT + ' [href="/export"]')
    # dropdown toggle
    USER_MENU = ".menu " + Common.DROPDOWN_MENU
    BUTTON_HELP = (By.CSS_SELECTOR, Common.DROPDOWN_MENU + ' #page-help')
    BUTTON_LOGOUT = (By.CSS_SELECTOR,
                     Common.DROPDOWN_MENU + ' [href="/logout"]')
...
class Dashboard(object):
    ...
    START_NEW_AUDIT_BTN_CSS = (By.CSS_SELECTOR,
                               _GET_LIST + ' [data-object-singular="Audit"]')
    START_NEW_WORKFLOW_BTN_CSS = (
        By.CSS_SELECTOR, _GET_LIST + ' [data-object-singular="Workflow"]')
    CREATE_TASK_BTN_CSS = (
        By.CSS_SELECTOR,
        _GET_LIST + ' [data-object-singular="CycleTaskGroupObjectTask"]')
    CREATE_OBJECT_BTN_CSS = (By.CSS_SELECTOR, _GET_LIST + ' [href="#"]')
    ALL_OBJECTS_BTN_CSS = (By.CSS_SELECTOR,
                           _GET_LIST + ' [href="/objectBrowser"]')
...
class LhnMenu(object):
    ...
    class _Locator(object):
        ...
        def get_create_new_button(label):
            return (By.CSS_SELECTOR, '[data-model-name="{}"] [data-test-id='
                    '"button_lhn_create_new_program_522c563f"]'.format(label))

        @staticmethod
        def get_accordion_count(label):
            return (By.CSS_SELECTOR,
                    '[data-model-name="{}"] .item-count'.format(label))

        @staticmethod
        def get_accordion_members(object_name):
            return (By.CSS_SELECTOR,
                    '[data-model-name="{}"]>.content>.sub-level>li'.format(
                        object_name))
        ...
        def __init__(cls, *args):
            for object_singular, object_plural in (zip(objects.ALL_SINGULAR,
                ...
                if "_" in capitalized_name:
                    capitalized_name = capitalized_name.title().replace(
                        "_", "")
                # set lhn items
    ...
    FILTER_SUBMIT_BUTTON = (By.CSS_SELECTOR,
                            ".lhs-search>.widgetsearch-submit")
    FILTER_CLEAR_BUTTON = (By.CSS_SELECTOR,
                           '.lhs-search [data-title="Clear filters"]')
    ...
    # workflows labels
    BUTTON_WORKFLOWS_ACTIVE = (By.CSS_SELECTOR,
                               '[data-for="Workflow"]>[data-value="Active"]')
    BUTTON_WORKFLOWS_DRAFT = (By.CSS_SELECTOR,
                              '[data-for="Workflow"]>[data-value="Draft"]')
...
class ExportPage(object):
    ...
    # user input elements
    EXPORT_FORMAT_DD_CSS = (By.CSS_SELECTOR, ".export-format")
...
class ExtendedInfo(object):
 ...
 member object."""
    ...
    # user input elements
    BUTTON_MAP_TO = (By.CSS_SELECTOR,
                     '[data-test-id="extended_info_button_map"]')
    ALREADY_MAPPED = (By.CSS_SELECTOR,
...
class CommonModalUnifiedMapper(object):
    ...
    FILTER_ATTRIBUTE_COMPARE = (By.CSS_SELECTOR,
                                ".filter-attribute__operator select")
    FILTER_ATTRIBUTE_VALUE = (By.CSS_SELECTOR,
                              ".filter-attribute__value input")
    ...
    BUTTON_MAP_SELECTED = (By.CSS_SELECTOR,
                           MODAL + Common.MODAL_FOOTER + " .btn-map")
    RESULT_TOGGLE_CSS = (
        By.CSS_SELECTOR,
        MODAL + Common.MODAL_FOOTER + " collapse-panel-click-area")
...
class ModalCloneOrCreateAssessmentTemplates(CommonModalUnifiedMapper):
    ...
    RESULT_TOGGLE_CSS = (
        By.CSS_SELECTOR,
        MODAL + " .modal-footer--trailed collapse-panel-click-area")
...
class ModalCreateNewObject(BaseModalCreateNew):
    ...
    BUTTON_SAVE_AND_ADD_ANOTHER = (
        By.CSS_SELECTOR,
        '{} [data-toggle="modal-submit-addmore"]'.format(MODAL))
...
class ModalCreateNewRisk(BaseModalCreateNew):
    ...
    UI_DESCRIPTION = (By.CSS_SELECTOR,
                      '.modal-body form>div:nth-child(3) iframe')
...
class ModalCreateNewControl(BaseModalCreateNew):
    ...
    TEST_PLAN = (By.CSS_SELECTOR,
                 '[data-test-id="control_test_plan_d8b5a2f4"] label')
    UI_TEST_PLAN = (By.CSS_SELECTOR,
    ...
    CATEGORIES = (By.CSS_SELECTOR,
                  '[data-test-id="control_categories_1eb33246"] label')
    SELECTABLE_CATEGORIES = (
        By.CSS_SELECTOR, '[data-test-id="control_categories_1eb33246"] select')
    STATE = (By.CSS_SELECTOR, '[data-test-id="control_state_5d184456"] label')
    DROPDOWN_STATE = (By.CSS_SELECTOR,
    ...
    DATEPICKER_EFFECTIVE_DATE = (
        By.CSS_SELECTOR, '[test-id="control_effective_dates_0376cf90"] '
        '[data-id="effective_date_hidden"] [data-handler="selectDay"]')
...
class ModalCreateNewAsmt(BaseModalCreateNew, CommonAssessment):
    ...
    MAPPED_SNAPSHOTS_CSS = (
        By.CSS_SELECTOR,
        BaseModalCreateNew.MODAL + CommonAssessment.MAPPED_SNAPSHOTS)
    MAP_OBJS_BTN_CSS = (
        By.CSS_SELECTOR,
...
class ModalCreateNewAsmtTmpl(BaseModalCreateNew):
    ...
    ASSIGNEE_DROPDOWN = (
        By.CSS_SELECTOR,
        'select[can-value="instance.default_people.assignees"]')


class ModalEditObject(BaseModalCreateNew):
    """Locators for Edit object modals."""
    BUTTON_DELETE = (By.CSS_SELECTOR,
                     '.deny-buttons [data-toggle="modal-ajax-deleteform"]')
...
class ModalCustomAttribute(object):
    ...
    ATTR_TITLE_UI_CSS = (By.CSS_SELECTOR,
                         Common.MODAL_BODY + ' [name="title"]')
    SAVE_AND_CLOSE_BTN_CSS = (
        By.CSS_SELECTOR,
        Common.MODAL_FOOTER + ' .confirm-buttons [data-toggle="modal-submit"]')
    ATTR_TITLE_LBL_CSS = (By.CSS_SELECTOR,
                          Common.MODAL_BODY + " div:nth-child(1)>label")
    ATTR_TYPE_CSS = (By.CSS_SELECTOR, Common.MODAL_HEADER + " h2")
    ATTR_TYPE_SELECTOR_DD_CSS = (By.CSS_SELECTOR,
                                 Common.MODAL_BODY + " dropdown select")
    MANDATORY_LBL_CSS = (By.CSS_SELECTOR, Common.MODAL_BODY + " .span2 label")
    MANDATORY_CB_CSS = (By.CSS_SELECTOR,
                        Common.MODAL_BODY + ' [type="checkbox"]')
    INLINE_HELP_LBL_CSS = (By.CSS_SELECTOR,
                           Common.MODAL_BODY + " div:nth-child(2)>label")
    INLINE_HELP_UI_CSS = (By.CSS_SELECTOR,
                          Common.MODAL_BODY + ' [name="helptext"]')
    PLACEHOLDER_LBL_CSS = (By.CSS_SELECTOR,
                           Common.MODAL_BODY + " .span6 label")
    PLACEHOLDER_UI_CSS = (By.CSS_SELECTOR,
                          Common.MODAL_BODY + ' [name="placeholder"]')
    POSSIBLE_VALUES_UI_CSS = (
        By.CSS_SELECTOR, Common.MODAL_BODY + ' [name="multi_choice_options"]')
    ADD_ANOTHER_BTN_CSS = (
        By.CSS_SELECTOR, Common.MODAL_FOOTER +
...
class ObjectWidget(object):
    ...
    HEADER_OWNER = (By.CSS_SELECTOR, _HEADER + ' [data-field="owners"]')
    ...
    HEADER_STATE_READY_FOR_REVIEW = (By.CSS_SELECTOR,
                                     _STATE + '.state-readyforreview')
    ...
    MEMBERS_TITLE_LIST = (By.CSS_SELECTOR,
...
class CommonWidgetInfo(object):
    ...
    _HEADERS_AND_VALUES = (
        _INFO_WIDGET_XPATH + '//div[starts-with(./@class, "span")]//h6/..')
    WIDGET = Common.INFO
    ...
    CAS_CHECKBOXES = (By.XPATH,
                      _INFO_WIDGET_XPATH + "//inline-edit-control//input["
                      "@type='checkbox']")
    # labels
    ...
    TXT_OBJECT_REVIEWED = (By.CSS_SELECTOR,
                           "{} .object-approved".format(WIDGET))
    ...
    PEOPLE_HEADER_CSS = (By.CSS_SELECTOR,
                         _PEOPLE_ITEM + " editable-people-group-header")
    PEOPLE_VALUE_CSS = (By.CSS_SELECTOR, _PEOPLE_ITEM + " object-list")
    # user input elements
    BUTTON_3BBS = (By.XPATH,
                   _INFO_WIDGET_XPATH + "//*[@data-toggle='dropdown']")
    # controllers
...
class WidgetInfoPanel(CommonWidgetInfo):
    ...
    BUTTON_CLOSE = (By.CSS_SELECTOR, _PIN_ACTION + ' [can-click="close"]')
    # People section
...
class WidgetInfoProgram(WidgetInfoPanel):
    ...
    TOGGLE_SHOW_ADVANCED = (By.CSS_SELECTOR,
                            "{} .show-hidden-fields".format(WIDGET))
...
class WidgetInfoAssessment(WidgetInfoPanel, CommonAssessment):
    ...
    # Base
    LCAS_HEADERS_AND_VALUES = (By.CSS_SELECTOR,
...
class WidgetAdminEvents(object):
    ...
    TREE_VIEW_ITEMS = (By.CSS_SELECTOR, "{0} {1}".format(
        _BASE_CSS_SELECTOR, _TREE_ITEMS_SELECTOR))
    TREE_VIEW_HEADER = (By.CSS_SELECTOR,
                        "{} header".format(_BASE_CSS_SELECTOR))
    FIRST_TREE_VIEW_ITEM = (By.CSS_SELECTOR, "{0} {1}:first-child".format(
        _BASE_CSS_SELECTOR, _TREE_ITEMS_SELECTOR))
...
class CommonDropdown3bbsTreeView(CommonDropdownMenu):
    ...
    BTN_3BBS_EXPORT_CSS = (By.CSS_SELECTOR,
                           TREE_VIEW_3BBS_DD + " .fa-download")
...
class TreeView(object):
    ...
    NO_RESULTS_MSG_CSS = (By.CSS_SELECTOR,
                          Common.WDG_NOT_HIDDEN + " .tree-no-results-message")
    SHOW_FIELDS_BTN_CSS = (
        By.CSS_SELECTOR,
    ...
    ITEM_DD_BTN_CSS = (By.CSS_SELECTOR,
                       Common.WDG_NOT_HIDDEN + ITEM_EXPAND_BTN)
...
class AdminCustomAttributes(object):
    ...
    class _Locator(object):
        ...
        def get_toggle(child_id):
            return (By.CSS_SELECTOR,
                    '#custom_attribute_widget li:nth-child({}) '
    ...
    class __metaclass__(type):
        def __init__(cls, *args):
            items = (objects.WORKFLOWS, objects.RISK_ASSESSMENTS,
                     objects.THREATS, objects.RISKS, objects.PROGRAMS,
                     objects.AUDITS, objects.OBJECTIVES, objects.SECTIONS,
                     objects.CONTROLS, objects.ISSUES, objects.ASSESSMENTS,
                     objects.STANDARDS, objects.REGULATIONS, objects.POLICIES,
                     objects.CONTRACTS, objects.CLAUSES, objects.VENDORS,
                     objects.PEOPLE, objects.ACCESS_GROUPS, objects.ORG_GROUPS,
                     objects.PRODUCTS, objects.MARKETS, objects.PROCESSES,
                     objects.FACILITIES, objects.PROJECTS, objects.DATA_ASSETS,
                     objects.SYSTEMS)
            for id_, name in enumerate(items, start=1):
                setattr(cls, Common.TOGGLE + name.upper(),
    ...
    LISTED_MEMBERS = (By.CSS_SELECTOR,
