...
class Representation(object):
    ...
    tree_view_attrs_to_exclude = ("created_at", "updated_at",
                                  "custom_attributes")
    people_attrs_names = [
    ...
    def all_attrs_names(cls):
        ...
        return list(
            set(cls.get_attrs_names() + [
                "access_control_list", "recipients", "people_values",
                "default_people", "modal_title", "assignee_type", "user_roles"
            ]))
    ...
    """
        all_entities_cls = (help_utils.convert_to_list(entity)
                            if entity else list(Entity.all_entities_classes()))
        ...
        return str(
            dict(
                zip(self.attrs_names_to_repr, [
                    getattr(self, attr_name_to_repr)
                    for attr_name_to_repr in self.attrs_names_to_repr
                ])))
    ...
    """
        if objs or isinstance(objs, bool):
            if isinstance(objs, list):
                if (all(not isinstance(_, dict)
                        and not isinstance(_, (str, unicode, int)) and _
                        for _ in objs)):
                    objs = [_.__dict__ for _ in objs]
            else:
                if (not isinstance(objs, dict)
                        and not isinstance(objs, (str, unicode, int))):
                    objs = objs.__dict__
            return objs
    ...
    """
        ...
        """
                if isinstance(attr_value, dict):
                    ...
                    if attr_name in [
                            "managers", "assignees", "creators", "verifiers",
                    ...
                    if obj_attr_name == "comments":
                        converted_attr_value = {
                            k: (parser.parse(v).replace(tzinfo=tz.tzutc())
                                if k == "created_at"
            ...
            if set(obj_cas_attrs_names).issubset(obj.__dict__.keys()):
                ...
                if (cas_def and
                    ...
                    cas_val_dicts_keys = ([_.keys()[0] for _ in cas_val]
                                          if isinstance(cas_val, list) else
                                          [None])
                    _cas_val = [
                        {
                            k: v
                ...
                cas_def_dict = (dict([
                    _def.iteritems().next() for _def in cas_def
                ]) if (isinstance(cas_def, list)
                       and all(isinstance(_def, dict) for _def in cas_def))
                                else None)
                cas_val_dict = (dict([
                    _val.iteritems().next() for _val in cas_val
                ]) if (isinstance(cas_def, list)
                       and all(isinstance(_def, dict) for _def in cas_def))
                                else None)
        ...
        return help_utils.execute_method_according_to_plurality(
            objs=objs,
            types=Entity.all_entities_classes(),
    ...
    """
        return {"type": getattr(self, "type"), "id": getattr(self, "id")}
    ...
    """
        ...
        def convert_repr_to_snapshot(origin_obj, parent_obj):
            ...
            origin_obj.__dict__.update(
                {k: v
                 for k, v in snapshoted_obj.__dict__.iteritems()})
            return origin_obj

        return help_utils.execute_method_according_to_plurality(
            objs=objs,
            types=Entity.all_entities_classes(),
            method_name=convert_repr_to_snapshot,
            parent_obj=parent_obj)

    def update_attrs(self,
                     is_replace_attrs=True,
                     is_allow_none=True,
                     is_replace_dicts_values=False,
                     **attrs):
    ...
    """
        return (self.update_objs_attrs_values(
            objs=self,
            is_replace_attrs_values=is_replace_attrs,
            is_allow_none_values=is_allow_none,
            is_replace_values_of_dicts=is_replace_dicts_values,
            **attrs))
    ...
    def delete_attrs(self, *attrs_names):
        ...
        # pylint: disable=expression-not-assigned
        [
            delattr(self, attr_name) for attr_name in attrs_names
    ...
    @classmethod
    def update_objs_attrs_values(cls,
                                 objs,
    ...
    """
        ...
        return help_utils.execute_method_according_to_plurality(
            ...
            is_allow_none_values=is_allow_none_values,
            **attrs)
    ...
    """
        ...
        return ([filter_obj_attrs(obj, attrs_to_include) for obj in objs]
                if isinstance(objs, list) else filter_obj_attrs(
                    objs, attrs_to_include))
    ...
    def attrs_values_types_error(self_attr, other_attr, expected_types):
        raise ValueError("'{}' have to be isinstance of classes: {}\n".format(
            (self_attr, other_attr), expected_types))
    ...
    """
        return (all(
            all((self_k == other_k and cls.is_attrs_equal(
                attr_name=self_k,
                self_attr_value=self_attr[self_k],
                other_attr_value=other_attr[self_k])) for self_k, other_k in
                zip(self_attr.keys(), other_attr.keys())) for self_attr,
            other_attr in zip(self_list_attrs, other_list_attrs)))

    @staticmethod
    def compare_cas(self_cas, other_cas):
        """Compare entities' 'custom_attributes' attributes."""
        if (isinstance(self_cas, (dict, type(None)))
                and isinstance(other_cas, (dict, type(None)))):
            is_equal = False
            if (isinstance(self_cas, dict) and isinstance(other_cas, dict)):
                is_equal = StringMethods.is_subset_of_dicts(
                    self_cas, other_cas)
            else:
    ...
    def compare_datetime(self_datetime, other_datetime):
        ...
        if (isinstance(self_datetime, (datetime, type(None)))
                and isinstance(other_datetime, (datetime, type(None)))):
            return self_datetime == other_datetime
        else:
            Representation.attrs_values_types_error(
                self_attr=self_datetime,
                other_attr=other_datetime,
        ...
        if (isinstance(self, other.__class__)
            ...
            for attr_name in self.attrs_names_to_repr:
                ...
                if is_equal:
                    ...
                    self_diff[attr_name] = self_attr_value, type(
                        self_attr_value)
                    other_diff[attr_name] = other_attr_value, type(
                        other_attr_value)
    ...
    """
        ...
        expected_excluded_attrs, actual_excluded_attrs = (
            cls.extract_simple_collections(exclude_attrs, actual_objs,
                                           *expected_objs))
    ...
    """
        return [
            expected_obj.update_attrs(**dict([(attr, None)
    ...
    def extract_simple_collections(expected_objs, actual_objs, *exclude_attrs):
        ...
        return [
            Representation.extract_excluded_attrs_collection(
...
class Entity(Representation):
    ...
    def __init__(self, **attrs):
        self.set_attrs("type", "slug", "id", "title", "href", "url", "admins",
                       "primary_contacts", "secondary_contacts", "status",
                       "os_state", "comments", "custom_attribute_definitions",
    ...
    def all_entities_classes():
        ...
        return (PersonEntity, CustomAttributeDefinitionEntity, ProgramEntity,
                ControlEntity, AuditEntity, AssessmentEntity,
                AssessmentTemplateEntity, IssueEntity, CommentEntity,
                ObjectiveEntity)
...
class CommentEntity(Representation):
    ...
    def __init__(self, **attrs):
        self.set_attrs("type", "id", "href", "description", "created_at",
                       "modified_by", **attrs)
...
class PersonEntity(Entity):
    ...
    def __init__(self, **attrs):
        ...
        self.delete_attrs("slug", "title", "admins", "primary_contacts",
                          "secondary_contacts", "status", "os_state",
                          "comments")
        self.set_attrs("name", "email", "company", "system_wide_role", **attrs)
...
class CustomAttributeDefinitionEntity(Representation):
    ...
    def __init__(self, **attrs):
        ...
        self.set_attrs("title", "id", "href", "type", "definition_type",
                       "attribute_type", "helptext", "placeholder",
                       "mandatory", "multi_choice_options", "created_at",
                       "updated_at", "modified_by", **attrs)
...
class ProgramEntity(Entity):
    ...
    def __init__(self, **attrs):
        ...
        self.set_attrs("managers", "editors", "readers", "primary_contacts",
...
class ControlEntity(Entity):
    ...
    def __init__(self, **attrs):
        ...
        self.set_attrs("principal_assignees", "secondary_assignees", "program",
                       **attrs)
...
class AuditEntity(Entity):
    ...
    def __init__(self, **attrs):
        ...
        self.delete_attrs("admins", "primary_contacts", "secondary_contacts",
                          "os_state", "comments")
        self.set_attrs("audit_captains", "auditors", "program", **attrs)
...
class AssessmentTemplateEntity(Entity):
    ...
    def __init__(self, **attrs):
        ...
        self.delete_attrs("admins", "primary_contacts", "secondary_contacts",
                          "os_state", "comments")
        self.set_attrs("assignees", "verifiers", "template_object_type",
                       "audit", **attrs)
...
class AssessmentEntity(Entity):
    ...
    def __init__(self, **attrs):
        ...
        self.set_attrs("creators", "assignees", "verifiers", "assessment_type",
                       "verified", "mapped_objects", "audit", "template",
                       "object", **attrs)

    def cads_from_template(self):
        return [
            definition for definition in self.custom_attribute_definitions
