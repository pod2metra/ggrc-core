...
from lib.constants import (element, objects, roles, value_aliases, url as
                           const_url, messages)
from lib.constants.element import AdminWidgetCustomAttributes
from lib.entities.entity import (PersonEntity, CustomAttributeDefinitionEntity,
                                 CommentEntity)
...
class EntitiesFactory(object):
    ...
    def __init__(self, obj_name):
        ...
        self.obj_entity_cls = factory.get_cls_obj_entity(
            object_name=self.obj_name)
        if self.__class__ not in [
                CustomAttributeDefinitionEntity, PersonEntity, CommentEntity
        ]:
    ...
    """
        ...
        def get_acl_member(role_id, person):
            ...
            if isinstance(person, PersonEntity):
                ...
                raise ValueError(
                    messages.CommonMessages.err_common.format(
                        PersonEntity, person))

        return [
            get_acl_member(role_id, person)
...
class CommentsFactory(EntitiesFactory):
    ...
    'is_add_rest_attrs' then add attributes for REST."""
        ...
        if is_add_rest_attrs:
            comment_obj.update_attrs(assignee_type=",".join((
...
class CustomAttributeDefinitionsFactory(EntitiesFactory):
    ...
    """
      ...
      """
        ...
        return {
            k: v
            for _ in [generate_ca_value(cls, ca) for ca in list_ca_def_objs]
            for k, v in _.items()
        }
...
class AuditsFactory(EntitiesFactory):
    ...
    """
        ...
        return [
            copy.deepcopy(audit).update_attrs(
                title=unicode(audit.title + " - copy " + str(num)),
                slug=None,
                created_at=None,
                updated_at=None,
                href=None,
...
class AssessmentTemplatesFactory(EntitiesFactory):
    ...
    """
        return [
            copy.deepcopy(asmt_tmpl).update_attrs(
        ...
        if is_add_rest_attrs:
            ...
            asmt_tmpl_obj.update_attrs(
                ...
                people_values=[{
                    "value": v,
                    "title": t
                } for v, t in [(roles.ADMIN, elements.OBJECT_ADMINS), (
...
class AssessmentsFactory(EntitiesFactory):
    ...
    """
        ...
        asmts_type = (mapped_objects[0].type if all(
            getattr(mapped_obj, "type") for mapped_obj in mapped_objects) else
                      None)
        if asmt_tmpl:
        ...
        asmts_objs = [
            empty_asmt.update_attrs(
                ...
                creators=asmts_creators,
                assignees=asmts_assignees,
                verifiers=asmts_verifiers,
                assessment_type=asmts_type,
                custom_attribute_definitions=asmts_cas_def)
            for empty_asmt, mapped_object in zip(
                [self.obj_inst()
                 for _ in xrange(len(mapped_objects))], mapped_objects)
        ]
        return asmts_objs
    ...
    """
        ...
        if getattr(asmt_obj, "verifiers"):
            asmt_obj.update_attrs(
                verifiers=PeopleFactory.extract_people_emails(
                    attrs["verifiers"]),
                access_control_list=string_utils.StringMethods.
                convert_list_elements_to_list(asmt_obj.access_control_list + [
                    PeopleFactory.get_acl_members(
    ...
    'is_add_rest_attrs' then add attributes for REST."""
        asmt_obj = self.obj_inst().update_attrs(
            ...
            assessment_type=objects.get_obj_type(objects.CONTROLS),
            verified=False)
        if is_add_rest_attrs:
            asmt_obj.update_attrs(
                recipients=",".join((unicode(roles.ASSIGNEES),
                                     unicode(roles.CREATORS),
