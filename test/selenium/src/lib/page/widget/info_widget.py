...
class InfoWidget(base.Widget):
  ...
  """
    ...
    """
        return (element.ReviewStates.REVIEWED
                if selenium_utils.is_element_exist(
                    self.info_widget_elem, self._locators.TXT_OBJECT_REVIEWED)
    ...
    def get_header_and_value_txt_from_custom_scopes(
            self, header_text, custom_scopes_locator=None):
    ...
    """
        ...
        all_headers_and_values = self.info_widget_elem.find_elements(
            *custom_scopes_locator
            if custom_scopes_locator else self._locators.HEADERS_AND_VALUES)
        return next((scope.text.splitlines() + [None] if len(
            scope.text.splitlines()) == 1 else scope.text.splitlines()[:2]
    ...
    """
        ...
        return (people_header_txt, None
                if people_value_txt == ["None"] else people_value_txt)

    def get_headers_and_values_dict_from_cas_scopes(
            self, is_gcas_not_lcas=True):  # noqa: ignore=C901
...
class SnapshotedInfoPanel(InfoPanel):
    ...
    def snapshot_obj_version(self):
        ...
        return base.Label(self.info_widget_elem,
                          self._locators.SNAPSHOT_OBJ_VER)
...
class Programs(InfoWidget):
    ...
    def __init__(self, driver):
        ...
        self.ref_url = base.MultiInputField(self.info_widget_elem.parent,
...
class Audits(InfoWidget):
    ...
    def __init__(self, driver):
        ...
        self._extend_list_all_scopes(self.audit_captain_lbl_txt,
...
class Assessments(InfoWidget):
    ...
    """
        if (self.tab_container.tab_controller.active_tab.text ==
                element.AssessmentTabContainer.OTHER_ATTRS_TAB):
            code_elem = base.Label(self.info_widget_elem,
                                   self._locators.CODE_CSS)
...
class Dashboard(base.Widget):
    ...
    def __init__(self, driver):
        ...
        self.create_task_btn = base.Button(self._driver,
                                           self._locators.CREATE_TASK_BTN_CSS)
        self.create_object_btn = base.Button(
            self._driver, self._locators.CREATE_OBJECT_BTN_CSS)
        self.all_objects_btn = base.Button(self._driver,
                                           self._locators.ALL_OBJECTS_BTN_CSS)
