    ...
    """
        ...
        if pagination_elements:
            if len(pagination_elements) == exp_pagination_count:
                ...
                raise ValueError(
                    messages.ExceptionsMessages.err_pagination_count.format(
                        exp_pagination_count))
        else:
            raise exceptions.NoSuchElementException(
                messages.ExceptionsMessages.err_pagination_elements.format(
                    ",".join([
...
class TreeView(base.TreeView):
    ...
    """
        ...
        return (Widget(self._driver,
                       self.obj_name).select_member_by_num(item_num))

    def _get_item_num_by_title(self, title):
        """Return number of item by title"""
        list_items = [
            item.text.splitlines() for item in self.tree_view_items()
        ]
        item_num = next(
            num for num, item in enumerate(list_items) if title in item)
        return item_num
    ...
    """
        ...
        selenium_utils.hover_over_element(
            self._driver,
            self.tree_view_items()[button_num].element)
