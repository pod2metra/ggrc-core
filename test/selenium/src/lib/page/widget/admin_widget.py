...
class Events(Widget):
    ...
    def __init__(self, driver):
        ...
        self.widget_header = base.Label(driver,
                                        self._locators.TREE_VIEW_HEADER)

    def get_events(self):
        """Get list of elements that displayed in Tree View on Event widget."""
        selenium_utils.get_when_clickable(self._driver,
                                          self._locators.FIRST_TREE_VIEW_ITEM)
        ...
        for i in items:
            if i.text == item_title:
                if expand:
                    ...
                    return i.collapse()
        raise exception.ElementNotFound('{} in list {}'.format(
            item_title, items))
