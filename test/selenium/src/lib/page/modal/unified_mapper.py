...
class CommonUnifiedMapperModal(base.Modal):
    ...
    def add_filter_attr(self,
                        attr_name,
                        value,
                        operator=None,
                        compare_op=alias.EQUAL_OP):
    ...
    def _get_latest_filter_elements(self):
        ...
        latest_filter = {
            "name":
            base.DropdownStatic(latest_filter_elem[-1],
                                self._locators.FILTER_ATTRIBUTE_NAME),
            "compare_op":
            base.DropdownStatic(latest_filter_elem[-1],
                                self._locators.FILTER_ATTRIBUTE_COMPARE),
            "value":
            base.TextInputField(latest_filter_elem[-1],
                                self._locators.FILTER_ATTRIBUTE_VALUE),
            "operator":
            None
        }
    ...
    source object.
    """
        base.Button(self.modal_elem,
                    self._locators.BUTTON_MAP_SELECTED).click()
        selenium_utils.get_when_invisible(self.modal_elem,
    ...
    """
        ...
        self._select_dest_obj_type(
            obj_name=dest_objs_type, is_asmts_generation=is_asmts_generation)
        for enum, title in enumerate(dest_objs_titles):
            if enum == 0:
                operator = None
            else:
                operator = alias.OR_OP
            self.add_filter_attr(
                self._elements.ATTRIBUTE_TITLE, title, operator=operator)
        self._select_search_dest_objs()
        return self.tree_view.get_list_members_as_list_scopes()

    def map_dest_objs(self,
                      dest_objs_type,
                      dest_objs_titles,
                      is_asmts_generation=False):
