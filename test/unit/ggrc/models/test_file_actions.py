...
class TestFileActions(unittest.TestCase):
    ...
    @ddt.data(
        ...
        (u'嘗試使用這個名字', '_ggrc_control-1', u'嘗試使用這個名字_ggrc_control-1'),
    ...
    @ddt.unpack
    def test_generate_file_name(self, original_file_name, postfix, expected,
                                _):
    ...
    """
        ...
        result = generate_file_name(
            original_file_name, postfix, separator='_ggrc')
        self.assertEquals(expected, result)

    @mock.patch('ggrc.app.init_views')
    @ddt.data(
        # folder_id, file_name, expected
        ('some_folder_id', 'name1', {
            'name': 'name1',
