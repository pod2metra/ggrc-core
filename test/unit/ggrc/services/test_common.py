...
class TestGetRevisionsList(TestCase):
    ...
    def new_simple_object(self):
        return collections.namedtuple("SimpleObject",
                                      ["id", "log_json", "type"])(next(
                                          self.pool_of_ids), lambda: "{}",
                                                                  "simple")

    @property
    def new_ownable_obj(self):
        return collections.namedtuple(
            "OwnableObject", ["id", "log_json", "type", "ownable"])(
                next(self.pool_of_ids), lambda: "{}", "ObjectOwner",
                self.new_simple_object)
    ...
    @unpack
    def test_with_extra_modified_obj(self, created_count, modified_count,
                                     deleted_count):
        ...
        with self.mock_get_cache(new, deleted, dirty):
            self.assertEqual(expected_results, [
                r.action
                for r in self.get_log_revisions(self.new_simple_object)
            ])
...
class TestFilterResource(TestCase):
    ...
    def test_filter_revision(self, _):
        ...
        res = common.filter_resource(
            resource=resource, depth=1, user_permissions=object())
