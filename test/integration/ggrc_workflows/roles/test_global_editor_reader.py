...
class GlobalEditorReaderGetTest(WorkflowRolesTestCase):
  ...
  """
    ...
    """
        ...
        task_group_object_res = self.api.get(TaskGroupObject,
                                             self.first_task_group_object.id)
        self.assert200_helper(task_group_object_res)

        task_group_task_res = self.api.get(TaskGroupTask,
    ...
    """
        ...
        cycle_obj = self.session.query(Cycle)\
            ...
            .first()
        cycle_res = self.api.get(Cycle, cycle_obj.id)
        ...
        cycle_task_group_obj = self.session.query(CycleTaskGroup)\
            ...
            .first()
        cycle_task_group_res = self.api.get(CycleTaskGroup,
        ...
        cycle_object_obj =\
            ...
            .filter(
                ...
                cycle_task_group_obj.id)\
            .first()
        cycle_object_res = self.api.get(CycleTaskGroupObjectTask,
                                        cycle_object_obj.id)
    ...
    @ddt.data(("reader", "Workflow Member", False),
              ...
              ("admin2", "Workflow Member", True), ("reader", "Admin", True),
              ("creator", "Admin", True), ("editor", "Admin", True),
              ("admin2", "Admin", True))
    @ddt.unpack
