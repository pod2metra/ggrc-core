...
class TestRecurringCyclesEvent(TestCase):
  ...
  """
    ...
    def setUp(self):
        ...
        wf_data = {'unit': 'week', 'repeat_every': 1}

        tg_data = {'start_date': '2018-01-17', 'end_date': '2018-01-17'}

        with freeze_time('2018-01-17'):
            _, workflow_1 = wf_generator.generate_workflow(data=wf_data)
            _, task_group_1 = wf_generator.generate_task_group(
                workflow=workflow_1)
            wf_generator.generate_task_group_task(
                task_group=task_group_1, data=tg_data)
            wf_generator.activate_workflow(workflow_1)

            _, workflow_2 = wf_generator.generate_workflow(data=wf_data)
            _, task_group_2 = wf_generator.generate_task_group(
                workflow=workflow_2)
            wf_generator.generate_task_group_task(
                task_group=task_group_2, data=tg_data)
