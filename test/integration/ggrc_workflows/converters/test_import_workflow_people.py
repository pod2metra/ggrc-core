...
class TestWorkflowPeopleImport(TestCase):
    ...
    def setUp(self):
        ...
        self.wf_import_params = collections.OrderedDict(
            [("object_type", "Workflow"), ("code", self.wf_slug),
             ("title", "SomeTitle"), ("Need Verification", 'True')])
    ...
    """
        ...
        exst_admins = [
            acl.person.email for acl in workflow.access_control_list
            if acl.ac_role.name == 'Admin'
        ]
        expected_admins = [
            self.user_emails[idx] for idx in expected_data['admins']
        ]
        ...
        expected_members = [
            self.user_emails[idx] for idx in expected_data['members']
        ]
    ...
    """
        ...
        actual_admins = [
            a.person.email for a in acl if a.ac_role.name == "Admin Mapped"
        ]
    ...
    @ddt.unpack
    def test_create_workflow_with_acl(self,
                                      import_data,
                                      expected_data=None,
