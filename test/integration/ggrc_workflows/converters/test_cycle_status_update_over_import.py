...
class TestCycleTaskStatusUpdate(ggrc_test.TestCase):
    ...
    def _update_structure(self, structure=None):
        ...
        self.cycle.is_verification_needed = structure.get(
            "cycle_is_verification_needed", self.cycle.is_verification_needed)
    ...
    def build_import_data(self, task_statuses):
        return [
            OrderedDict([("object_type", self.ALIAS),
                         ("Code*", self.tasks[idx].slug), ("State", status)])
    ...
    @ddt.data(
        ...
        # verified few tasks
        {
            "update_structure": {
                "task_statuses":
                [all_models.CycleTaskGroupObjectTask.VERIFIED] * 2,
                "group_status":
                all_models.CycleTaskGroup.IN_PROGRESS,
                "cycle_status":
                all_models.Cycle.IN_PROGRESS,
                "workflow_status":
    ...
    def test_update_status(self, update_structure, start_structure=None):
        ...
        self.tasks = all_models.CycleTaskGroupObjectTask.query.filter(
            all_models.CycleTaskGroupObjectTask.id.in_(self.task_ids)).all()
        ...
        self.assertEqual(
            len(all_models.CycleTaskGroupObjectTask.query.all()), task_count)
        self.assertEqual(cycle_status, group.cycle.status)
        self.assertEqual(workflow_status, group.cycle.workflow.status)

    @ddt.data(ASSIGNED_STRUCTURE, IN_PROGRESS_STRUCTURE, DECLINED_STRUCTURE)
    def test_update_to_verified(self, start_structure):
        ...
        line_error_tmpl_2 = errors.INVALID_STATUS_DATE_CORRELATION.format(
            ...
            line="{}")
        verfied_date_errors = {
            line_error_tmpl_1.format(i + 3)
            for i in xrange(len(self.tasks))
        }
        finished_date_errors = {
            line_error_tmpl_2.format(i + 3)
            for i in xrange(len(self.tasks))
        }
    ...
    def test_finished_to_verified(self, number_of_tasks):
        ...
        start_statuses = self._get_start_tasks_statuses(
            self.FINISHED_STRUCTURE)
        with ggrc_factories.single_commit():
        ...
        self.assertEqual([None] * len(self.tasks),
                         [t.verified_date for t in self.tasks])
        task_statuses = [all_models.CycleTaskGroupObjectTask.VERIFIED
                         ] * number_of_tasks
        response = self.import_data(*self.build_import_data(task_statuses))
        self._check_csv_response(response, {})
        self.tasks = all_models.CycleTaskGroupObjectTask.query.filter(
        ...
        self.assertEqual([finished_date] * len(self.tasks),
                         [t.finished_date for t in self.tasks])
        verified_dates = [t.verified_date
                          for t in self.tasks][:number_of_tasks]
        self.assertNotEqual([None] * number_of_tasks, verified_dates)
        verified_dates += [t.verified_date
                           for t in self.tasks][number_of_tasks:]
        self.assertEqual([None] * (len(self.tasks) - number_of_tasks), [
            t.verified_date for t in self.tasks
        ][number_of_tasks:])
        self.assertEqual(
            len(self.tasks) != number_of_tasks, self.tasks[0].cycle.is_current)
        # wrong rollback  case (try to import from verified to finished)
        response = self.import_data(*self.build_import_data(start_statuses))
    ...
    def __build_status_error_resp(self, key, error_tmpl, exception_statuses):
        ...
        error_tmpl = self.__build_error_tmpl(
            error_tmpl, column_name="State", message="Invalid state '{}'")
        return {
            self.ALIAS: {
                key: {
                    error_tmpl.format(3 + idx, status)
    ...
    def test_validation_error(self, exception_statuses):
        ...
        error_msgs = self.__build_status_error_resp(
            "row_errors", errors.VALIDATION_ERROR, exception_statuses)
    ...
    def test_simple_send_invalid_status(self,
        ...
        self._update_structure(start_structure)
        warn_msgs = self.__build_status_error_resp(
            "row_warnings", errors.WRONG_VALUE_CURRENT, exception_statuses)
        send_data = self.build_import_data(exception_statuses)
        self.__assert_error_message(
            send_data, warn_msgs, start_structure=start_structure)
