...
class TestExportTasks(TestCase):
    ...
    def assertCycles(self, field, value, cycle_slugs):
        ...
        self.assertEqual(
            sorted(cycle_slugs), sorted([i["Code*"] for i in parsed_data]))
    ...
    def test_filter_group_title(self, cycle_count, task_count):
        ...
        for task_id, slug in task_cycle_filter.iteritems():
            task = all_models.CycleTaskGroupObjectTask.query.filter(
                all_models.CycleTaskGroupObjectTask.id == task_id).one()
            self.assertCycles("group title", task.cycle_task_group.title,
                              [slug])
    ...
    def test_filter_by_group_assignee(self, cycle_count, task_count):
        ...
        for task_id, slug in task_cycle_filter.iteritems():
            task = all_models.CycleTaskGroupObjectTask.query.filter(
                all_models.CycleTaskGroupObjectTask.id == task_id).one()
            self.assertCycles("group assignee",
                              task.cycle_task_group.contact.email, [slug])
            self.assertCycles("group assignee",
                              task.cycle_task_group.contact.name, [slug])
    ...
    def test_filter_by_task_assignee(self, cycle_count, task_count):
        ...
        for task_id, slug in task_cycle_filter.iteritems():
            task = all_models.CycleTaskGroupObjectTask.query.filter(
                all_models.CycleTaskGroupObjectTask.id == task_id).one()
            assignees = list(
                self.get_persons_for_role_name(task, "Task Assignees"))
    ...
    def test_filter_by_task_secondary_assignee(self, cycle_count, task_count):
        ...
        for task_id, slug in task_cycle_filter.iteritems():
            task = all_models.CycleTaskGroupObjectTask.query.filter(
                all_models.CycleTaskGroupObjectTask.id == task_id).one()
            s_assignees = list(
                self.get_persons_for_role_name(task,
                                               "Task Secondary Assignees"))
            self.assertEqual(1, len(s_assignees))
            self.assertCycles("task secondary assignees", s_assignees[0].email,
                              [slug])
            self.assertCycles("task secondary assignees", s_assignees[0].name,
                              [slug])
    ...
    def test_filter_by_task_due_date_year(self, cycle_count, task_count):
        ...
        for due_date, cycle_slugs in due_date_dict.iteritems():
            self.assertCycles("task due date", "{}-{}-{}".format(*due_date),
                              list(cycle_slugs))
    ...
    def test_filter_by_cycle_assignee(self, cycle_count, task_count):
        ...
        for task_id, slug in task_cycle_filter.iteritems():
            task = all_models.CycleTaskGroupObjectTask.query.filter(
                all_models.CycleTaskGroupObjectTask.id == task_id).one()
            self.assertCycles("cycle assignee", task.cycle.contact.email,
                              [slug])
            self.assertCycles("cycle assignee", task.cycle.contact.name,
                              [slug])
