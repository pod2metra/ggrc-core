  ...
  """
    ...
    reason of error. Only admin can update cycle tasks via import."""
        ...
        with freeze_time(self.ftime_active):
            ...
            response = self.import_file(
                "cycle_task_correct.csv", person=creator)
    ...
    def _generate_cycle_tasks(self):
        ...
        self._activate_workflow(
            self.ftime_active, self.workflow_active, self.task_group_active,
            self.task_group_tasks_active, self.random_objects[0],
            self.generated_cycle_tasks_active)
        cycle = self._activate_workflow(
            self.ftime_historical, self.workflow_historical,
            self.task_group_historical, self.task_group_tasks_historical,
            self.random_objects[1], self.generated_cycle_tasks_historical)
    ...
    expected data for checks"""
        ...
        self.expected_cycle_task_correct = {
            "CYCLETASK-1": {
                "title":
                self.task_group_tasks_active[0]["title"] + " one",
                "description":
            ...
            "CYCLETASK-2": {
                "title":
                self.task_group_tasks_active[1]["title"] + " two",
                "description":
            ...
            "CYCLETASK-3": {
                "title":
                self.task_group_tasks_active[2]["title"] + " three",
                "description":
            ...
            "CYCLETASK-4": {
                "title":
                self.task_group_tasks_active[3]["title"] + " four",
                "description":
            ...
            "CYCLETASK-5": {
                "title":
                self.task_group_tasks_active[4]["title"] + " five",
                "description":
...
class TestCycleTaskImportUpdateAssignee(BaseTestCycleTaskImportUpdate):
    ...
    def test_update_assignee(self, alias):
        ...
        assignees = list(
            self.get_persons_for_role_name(self.query.first(),
                                           "Task Assignees"))
        self.assertFalse(assignees)
        response = self.import_data(
            OrderedDict([("object_type", alias), ("Code*", self.instance.slug),
        ...
        s_assignees = list(
            self.get_persons_for_role_name(self.query.first(),
                                           "Task Secondary Assignees"))
        self.assertEqual([self.s_assignee.email],
                         [u.email for u in s_assignees])
    ...
    def test_update_assignee_with_non_importable(self, alias):
        ...
        self.assertEqual([self.s_assignee.email],
                         [u.email for u in s_assignees])
        self._check_csv_response(response,
                                 self.generate_expected_warning(
                                     'Task Assignees',
                                     'Task Secondary Assignees'))
