...
class TestAutomappings(TestCase):
    ...
    def create_ac_roles(cls, obj, person_id):
        ...
        ac_role = models.AccessControlRole.query.filter_by(
            object_type=obj.type, name="Admin").first()
        factories.AccessControlListFactory(
            ac_role=ac_role, object=obj, person_id=person_id)
    ...
    def assert_mapping(self, obj1, obj2, missing=False):
        ...
        if not missing:
            self.assertIsNotNone(
                rel, msg='%s not mapped to %s' % (obj1.type, obj2.type))
            ...
            self.assertIsNone(
                rel, msg='%s mapped to %s' % (obj1.type, obj2.type))
    ...
    def test_mapping_to_sections(self):
        ...
        regulation = self.create_object(
            models.Regulation, {'title': make_name('Test Regulation')})
    ...
    def test_mapping_to_objective(self):
        ...
        regulation = self.create_object(
            models.Regulation, {'title': make_name('Test PD Regulation')})
        section = self.create_object(
            models.Section, {
                'title': make_name('Test section'),
                'directive': {
                    'id': regulation.id
        ...
        objective = self.create_object(models.Objective,
                                       {'title': make_name('Test control')})
        self.assert_mapping_implication(
            to_create=[(regulation, section), (section, objective), (objective,
                                                                     control)],
            implied=[
    ...
    def test_mapping_between_objectives(self):
        ...
        objective2 = self.create_object(models.Objective,
                                        {'title': make_name('Test Objective')})
        self.assert_mapping_implication(
            to_create=[(regulation, section), (section, objective1),
    ...
    def test_mapping_nested_controls(self):
        ...
        control2 = self.create_object(models.Control,
                                      {'title': make_name('Test control')})
        self.assert_mapping_implication(
            to_create=[(objective, control_p), (control_p, control1),
                       (control_p, control2)],
            implied=[])
    ...
    def test_automapping_permissions(self):
        ...
        self.api.set_user(creator)
        self.assert_mapping_implication(
            to_create=[(section, objective), (control, objective)],
            implied=[(program, regulation), (program, section),
                     (section, regulation), (control, section)],
        )

    def test_program_role_propagation(self):
        """Test if automappings also propagate program roles"""
        roles = {"Program Managers", "Program Editors", "Program Readers"}
        propagated_roles = {
            "Program Managers Mapped", "Program Editors Mapped",
        ...
        db_roles = all_models.AccessControlRole.query.filter(
            all_models.AccessControlRole.name.in_(roles
                                                  | propagated_roles)).options(
                                                      load_only("id",
                                                                "name")).all()

        role_map = {role.name: role.id for role in db_roles}

        program = self.create_object(
            models.Program, {
                'title':
                make_name('Program'),
                'access_control_list': [{
        ...
        self.assert_mapping_implication(
            to_create=[(program, regulation), (source_obj, regulation),
    ...
    """
        ...
        auto = Automapping.query.filter_by(
            ...
            destination_type=rel1.destination_type).one()
        assert auto is not None

        # Check if the correct parent id is set:
        rel2 = models.Relationship.query.filter_by(parent_id=rel1.id).one()
...
class TestIssueAutomappings(TestCase):
    ...
    def setUp(self):
        ...
        with factories.single_commit():
            ...
            factories.RelationshipFactory(
                source=self.issue_audit, destination=self.audit)

            # to map an Issue to a Snapshot, you first should map it to Audit
            factories.RelationshipFactory(
                source=self.issue_snapshot, destination=self.audit)
            factories.RelationshipFactory(
                source=self.issue_snapshot, destination=self.snapshot)
    ...
    def _ordered_pairs_from_relationships(relationships):
        ...
        def order(src, dst):
            return ((src, dst)
                    if (src.type, src.id) < (dst.type, dst.id) else (dst, src))
    ...
    def _get_automapped_relationships(cls):
        ...
        automapped = all_models.Relationship.query.filter(
            all_models.Relationship.automapping_id == automapping.id, ).all()
    ...
    def test_issue_assessment_automapping(self):
        ...
        factories.RelationshipFactory(source=self.issue, destination=self.asmt)

        automapped = self._get_automapped_relationships()

        self.assertItemsEqual(automapped, [(self.audit, self.issue),
    ...
    def test_issue_assessment_automapping_no_audit(self):
        ...
        factories.RelationshipFactory(
            source=self.issue_audit, destination=self.asmt)

        automapped = self._get_automapped_relationships()

        self.assertItemsEqual(automapped, [(self.issue_audit, self.snapshot)])

    def test_issue_assessment_automapping_all_mapped(self):
        """Issue is not automapped to Snapshot and Audit if already mapped."""
        factories.RelationshipFactory(
            source=self.issue_snapshot, destination=self.asmt)
