...
class Api(object):
    ...
    def send_request(self,
        ...
        if response.status_code == 401:
            self.set_user()
            response = request(
                api_link, data=json_data, headers=headers.items())
    ...
    def patch(self, model, data):
        return self.send_request(
            self.client.patch, data=data, api_link=self.api_link(model))

    def get(self, obj, id_):
        return self.data_to_json(self.client.get(self.api_link(obj, id_)))

    def get_collection(self, obj, ids):
        return self.data_to_json(
            self.client.get("{}?ids={}".format(self.api_link(obj), ids)))

    def get_query(self, obj, query):
        return self.data_to_json(
            self.client.get("{}?{}".format(self.api_link(obj), query)))
    ...
    def search(self, types, query="", counts=False, relevant_objects=None):
        """Api search call."""
        link = '/search?q={}&types={}&counts_only={}'.format(
            query, types, counts)
