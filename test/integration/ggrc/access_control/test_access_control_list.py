...
class TestAccessControlList(TestCase):
    ...
    def setUp(self):
        ...
        self.acl = factories.AccessControlListFactory(
            object=self.control, ac_role_id=self.acr.id, person=self.person)
    ...
    def _post_control(self, id_, person_id, collection=False):
        ...
        response = self.api.post(all_models.Control, [control]
                                 if collection else control)
    ...
    def test_acl_revision_count(self):
        ...
        control["access_control_list"] = []
        self.api.put(
            all_models.Control.query.get(control["id"]), {"control": control})
        self.assertEqual(
            all_models.Revision.query.filter_by(
                resource_type="AccessControlList").count(),
            3)
