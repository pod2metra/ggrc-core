...
class TestCase(BaseTestCase, object):
    ...
    def get_role_id_for_obj(obj, role_name):
        ...
        for role_id, name in ac_role.get_custom_roles_for(
                obj.type).iteritems():
            if name == role_name:
                return role_id
    ...
    """
        ...
        messages = ("block_errors", "block_warnings", "row_errors",
                    "row_warnings")
        ...
        response_str = json.dumps(
            responses, indent=4, sort_keys=True, cls=SetEncoder)
        expected_str = json.dumps(
            expected_messages, indent=4, sort_keys=True, cls=SetEncoder)

        self.assertEqual(
            responses, expected_messages,
    ...
    """
        ...
        with tempfile.NamedTemporaryFile(
                dir=cls.CSV_DIR, suffix=".csv") as tmp:
            ...
            return cls._import_file(
                os.path.basename(tmp.name), dry_run, person)
    ...
    def send_import_request(data, dry_run=False, person=None):
        ...
        response = api.client.post(
            "/_service/import_csv", data=data, headers=headers)
        return json.loads(response.data)

    @classmethod
    @patch("ggrc.gdrive.file_actions.get_gdrive_file", new=read_imported_file)
    def _import_file(cls, filename, dry_run=False, person=None):
        ...
        response = cls.send_import_request(
            data, dry_run=dry_run, person=person)
        return response
    ...
    """
        ...
        request_body = {"export_to": "csv", "objects": data}
    ...
    def assert_slugs(self, field, value, slugs, operator=None):
        ...
        parsed_data = self.export_parsed_csv(search_request)[
            self.model._inflector.title_singular.title()]
        self.assertEqual(
            sorted(slugs), sorted([i["Code*"] for i in parsed_data]))
    ...
    def assert_filter_by_datetime(self,
                                  alias,
                                  datetime_value,
                                  slugs,
                                  formats=None,
    ...
    def assert_roles(self, obj, **roles):
        ...
        acl_person_roles = [(acl.ac_role.name, acl.person)
                            for acl in obj.access_control_list]
            ...
            for person, roles in persons:
                ...
                for role in roles.split(","):
                    factories.AccessControlListFactory(
                        person=person, ac_role_id=ac_roles[role], object=obj)
