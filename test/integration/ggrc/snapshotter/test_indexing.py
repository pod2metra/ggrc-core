...
class TestSnapshotIndexing(SnapshotterBaseTestCase):
    ...
    def test_create_indexing(self):
        ...
        control = db.session.query(
            models.Control).filter(models.Control.slug == "control-3").one()
        access_group = db.session.query(models.AccessGroup).filter(
            models.AccessGroup.slug == "ag-2").one()
        objective = db.session.query(
            models.Objective).filter(models.Objective.slug == "obj-1").one()
        process = db.session.query(
            models.Process).filter(models.Process.slug == "proc-2").one()
        ...
        records = db.session.query(Record).filter(
            tuple_(Record.type, Record.key, Record.property,
                   Record.content).in_({("Snapshot", s.id, "parent",
                                         "Audit-{}".format(audit.id))
                                        for s in snapshots}))
        ...
        custom_attributes = [(control, "control text field 1",
                              "control value 1"), (objective,
        ...
        for obj, definition, value in custom_attributes:
            ...
            _cav = db.session.query(Record).filter(
                Record.key == snapshot.id, Record.type == "Snapshot",
                Record.property == definition, Record.content == value)
            _title = db.session.query(Record).filter(
                Record.key == snapshot.id, Record.type == "Snapshot",
                Record.property == "title", Record.content == obj.title)
    ...
    def test_update_indexing(self):
        ...
        access_group = db.session.query(models.AccessGroup).filter(
            models.AccessGroup.title == "ag-2").one()
        objective = db.session.query(
            models.Objective).filter(models.Objective.title == "obj-1").one()
        custom_attribute_defs = self.create_custom_attribute_definitions()
        custom_attribute_values = [
            {
                "custom_attribute": custom_attribute_defs["objective"],
        ...
        objective_cav = db.session.query(models.CustomAttributeValue).filter(
            ...
            "objective value 1").one()

        access_group_cav = db.session.query(
            models.CustomAttributeValue).filter(
                models.CustomAttributeValue.attribute_value ==
                "access_group text value 1").one()
        ...
        obj_edits = [(objective, "objective title edited after initial index"),
                     (access_group,
                      "access group title edited after initial index")]

        for obj, title in obj_edits:
            obj = self.refresh_object(obj)
            self.api.modify_object(obj, {"title": title})
        ...
        for obj, title, definition, value in custom_attributes:
            ...
            _title = db.session.query(Record).filter(
                Record.key == snapshot.id, Record.type == "Snapshot",
                Record.property == "title", Record.content == title)
            _desc = db.session.query(Record).filter(
                Record.key == snapshot.id, Record.type == "Snapshot",
                Record.property == "description",
                Record.content == obj.description)
    ...
    def test_index_by_acr(self):
        ...
        with factories.single_commit():
            acr = factories.AccessControlRoleFactory(
                name=role_name, object_type="Control")
            person = factories.PersonFactory(
                email="test@example.com", name='test')
            control = factories.ControlFactory()
            factories.AccessControlListFactory(
                ac_role=acr, person=person, object=control)
        revision = all_models.Revision.query.filter(
            all_models.Revision.resource_id == control.id,
            all_models.Revision.resource_type == control.type,
        ).one()
