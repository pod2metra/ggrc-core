...
class TestSnapshoting(SnapshotterBaseTestCase):
    ...
    def test_snapshot_create(self):
        ...
        ac_roles = db.session.query(models.AccessControlRole.id,
        ...
        self.create_object(
            models.Audit, {
                ...
                [{
                    "ac_role_id": ac_roles["Audit Captains"],
                    "person": {
                        "id": captain,
                        "type": "Person"
                    }
        ...
        self.assertEqual(snapshot_obj.revision.content["title"],
                         "Test Control Snapshot 1 EDIT 1")

        snapshot_revision = db.session.query(
            models.Revision.resource_type, models.Revision.resource_id,
            models.Revision._content).filter(
        ...
        self.assertEqual(
            db.session.query(models.AccessControlList.id).filter(
                ...
                models.AccessControlList.person_id.in_(
                    people["Auditors"] + people["Audit Captains"])).count(), 7)
    ...
    """
        ...
        self.assertEqual(
            db.session.query(models.Snapshot).filter(
                models.Snapshot.parent_type == "Audit",
                models.Snapshot.parent_id == audit.id).count(), 2)
        ...
        for obj, expected_title in expected:
            snapshot = db.session.query(models.Snapshot).filter(
                models.Snapshot.child_type == obj.__class__.__name__,
                models.Snapshot.child_id == obj.id).first()
            self.assertEquals(snapshot.revision.content["title"],
                              expected_title)
    ...
    """
        ...
        for _type in Types.all:
            self.assertEqual(
                db.session.query(getattr(models.all_models, _type)).count(), 3)
    ...
    def test_snapshot_relations_remove(self):
        ...
        with factories.single_commit():
            ...
            for _ in xrange(total_reg_count):
                ...
                factories.RelationshipFactory(
                    source=control, destination=regulation)
                factories.RelationshipFactory(
                    source=regulation, destination=program)
        ...
        for num, rel in enumerate(rels):
            if num < unmapped_reg_count:
                ...
                removed_reg_rels.append((rel.destination_type,
                                         rel.destination_id))
                # Unmap Regulation object from Control
                db.session.delete(rel)
            else:
                keep_reg_rels.append((rel.destination_type,
                                      rel.destination_id))
        db.session.commit()
    ...
    def test_audit_creation_if_nothing_in_program_scope(self):
        ...
        program = db.session.query(models.Program).filter(
            models.Program.title == "empty program").one()
        ...
        audit = db.session.query(
            models.Audit).filter(models.Audit.title == audit_title).one()
    ...
    def test_snapshot_post_api(self):
        ...
        self.create_mapping(program2,
                            objective)  # different direction than above

        audit = db.session.query(
            models.Audit).filter(models.Audit.program_id == program.id).one()
        audit2 = db.session.query(
            models.Audit).filter(models.Audit.program_id == program2.id).one()
        ...
        objective_revision = db.session.query(models.Revision).filter(
            ...
            models.Revision.resource_id == objective.id).all()[-1]
        ...
        self.assertSetEqual({s.revision_id
                             for s in control_snapshots},
                            {control_revision.id})
        ...
        def _assert_exactly_one_relationship(src, dst):
            ...
            rels = db.session.query(models.Relationship).filter(
                ...
                db.session.query(models.Relationship).filter(
    ...
    def test_relationship_post_api(self):
        ...
        self.assertIsNotNone(
            models.Relationship.find_related(program, objective))
        self.assertIsNotNone(
            models.Relationship.find_related(program, control))
