...
class TestUserGenerator(TestCase):
    ...
    def test_person_import(self):
        ...
        with mock.patch.multiple(PersonClient, _post=self._mock_post):
            ...
            audit_slug = 'Audit1'
            self.import_data(
                OrderedDict([("object_type",
                              "Audit"), ("Code*", audit_slug), ("Program*",
                                                                program.slug),
                             ("Auditors", "cbabbage@example.com"), ("Title",
                                                                    "Title"),
                             ("State", "Planned"), ("Audit Captains",
                                                    "aturing@example.com")]))
            audit = Audit.query.filter(Audit.slug == audit_slug).first()
            auditors = [
                acl.person.email for acl in audit.access_control_list
            ...
            self.import_data(
                OrderedDict([("object_type", "Assessment"), ("Code*",
                                                             assessment_slug),
                             ("Audit*", audit.slug), ("Creators*",
                                                      "aturing@example.com"),
                             ("Assignees*", "aturing@example.com"),
                             ("Secondary Contacts",
                              "cbabbage@example.com"), ("Title", "Title")]))
            assessment = Assessment.query.filter(
                Assessment.slug == assessment_slug).first()
            acl_roles = {
                acl.ac_role.name: acl
                for acl in assessment.access_control_list
            }
            self.assertIn("aturing@example.com",
                          acl_roles["Creators"].person.email)
    ...
    def test_persons_import(self):
        ...
        with mock.patch.multiple(PersonClient, _post=self._mock_post):
            ...
            self.assertEqual(
                len(assessment_template.default_people['verifiers']), 2)
            self.assertEqual(
