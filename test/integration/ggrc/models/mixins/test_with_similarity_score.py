...
class TestWithSimilarityScore(TestCase):
    ...
    """
        ...
        for all_mappings in assessment_mappings:
            audit = [
                x for x in all_mappings
                if hasattr(x, "type") and x.type == "Audit"
            ][0]
            if with_types:
    ...
    """
        ...
        audit_1 = self.obj_gen.generate_object(
            models.Audit, {
                "title": "Audit 1",
                "program": {
                    "id": program_1.id
        ...
        audit_2 = self.obj_gen.generate_object(
            models.Audit, {
                "title": "Audit 2",
                "program": {
                    "id": program_2.id
        ...
        audit_3 = self.obj_gen.generate_object(
            models.Audit, {
                "title": "Audit 3",
                "program": {
                    "id": program_3.id
        ...
        assessments = self.make_assessments(
            assessment_mappings, with_types=True)

        risk_asmnt_ids = {
            asmnt.id
            for asmnt in assessments if asmnt.assessment_type == "Risk"
        }
    ...
    """
        ...
        assessment_mappings = [
            [
                audit_1, "Objective", objective_1_program_1,
                objective_2_program_1
            ],
            [
                audit_2, "Objective", objective_1_program_1,
                objective_2_program_1
            ],
            [audit_3, "Objective"],
        ]
    ...
    @ddt.unpack
    def test_asmt_issue_similarity(self, asmnt_type, obj_factory,
                                   issue_exists):
        ...
        with factories.single_commit():
            ...
            factories.RelationshipFactory(
                source=audit, destination=assessment1)
            factories.RelationshipFactory(
                source=audit, destination=assessment2)
            factories.RelationshipFactory(source=audit, destination=issue)
            factories.RelationshipFactory(
                source=snapshot, destination=assessment1)
        ...
        if issue_exists:
            self.assertListEqual(response.json[0]["Issue"]["ids"],
                                 expected_ids)
        else:
            self.assertListEqual(response.json[0]["Issue"]["ids"], [])
    ...
    @ddt.unpack
    def test_asmt_issue_related(self, asmnt_types, asmnt_related,
                                issues_exist):
        ...
        with factories.single_commit():
            ...
            for asmnt_type in asmnt_types:
                ...
                factories.RelationshipFactory(
                    source=snapshot, destination=assessment)
            ...
            if asmnt_related:
                factories.RelationshipFactory(
                    source=issue, destination=assessment)
        ...
        for num, data in enumerate(response.json):
            if issues_exist[num]:
                self.assertListEqual(data["Issue"]["ids"], expected_ids)
            else:
                self.assertListEqual(data["Issue"]["ids"], [])
    ...
    def test_asmnt_asmnt_related(self, asmnt_types, object_type, expect_asmnt):
        ...
        expected_ids = [
            assessment_ids[num]
            for num, id_expected in enumerate(expect_asmnt, 1) if id_expected
        ]
        self.assertListEqual(response.json[0]["Assessment"]["ids"],
                             expected_ids)
    ...
    def test_asmnt_mapped_similar(self, obj_types, asmnt_types, similar):
        ...
        with factories.single_commit():
            ...
            factories.RelationshipFactory(
                source=audit, destination=assessment2)
            factories.RelationshipFactory(
                source=audit, destination=snapshots[0])
            factories.RelationshipFactory(
                source=audit, destination=snapshots[1])
            factories.RelationshipFactory(
