...
class TestMixinAutoStatusChangeableBase(test_assessment.TestAssessmentBase):
    ...
    def change_status(self,
        ...
        self.api.modify_object(obj, {"status": status})
    ...
    def create_simple_assessment(self):
        ...
        self.api.modify_object(
            assessment, {"title": assessment.title + " modified, change #1"})
...
class TestFirstClassAttributes(TestMixinAutoStatusChangeableBase):
    ...
    @ddt.data(models.Assessment.DONE_STATE, models.Assessment.FINAL_STATE,
    ...
    def test_update_label_not_change_status(self, from_status):
        ...
        self.api.modify_object(assessment, {'label': 'Followup'})
        assessment = self.refresh_object(assessment)
    ...
    def test_update_field_change_status(self, field_name, new_value,
        ...
        self.api.modify_object(assessment, {field_name: new_value})
        assessment = self.refresh_object(assessment)
...
class TestSnapshots(TestMixinAutoStatusChangeableBase):
    ...
    """
        with factories.single_commit():
            ...
            assessment = factories.AssessmentFactory(
                audit=audit, status=from_status)
    ...
    @ddt.unpack
    def test_unmapping_snapshot_status_check(self, from_status,
                                             expected_status):
    ...
    """
        with factories.single_commit():
            audit = factories.AuditFactory()
            assessment = factories.AssessmentFactory(
                audit=audit, assessment_type='Contract')
            factories.RelationshipFactory(source=audit, destination=assessment)
            control = factories.ControlFactory(title='test control')
        assessment_id = assessment.id
        snapshot = self._create_snapshots(audit, [control])[0]
...
class TestDocuments(TestMixinAutoStatusChangeableBase):
    ...
    @ddt.unpack
    def test_document_added_status_check(self, document_type, from_status,
                                         expected_status):
    ...
    def test_document_remove_related(self, document_type, from_status,
        ...
        with factories.single_commit():
            ...
            factories.RelationshipFactory(
                destination=assessment, source=document)
...
class TestCustomAttributes(TestMixinAutoStatusChangeableBase):
    ...
    @ddt.unpack
    def test_global_ca_update_empty_value_not_change_status(
    ...
    @ddt.unpack
    def test_global_ca_update_change_status(self, from_status,
                                            expected_status):
...
class TestOther(TestMixinAutoStatusChangeableBase):
   ...
   map/unmap issue, assignees Handlers"""
    ...
    """
        ...
        self.modify_assignee(assessment, "creator@example.com",
                             ["Creators", "Assignees"])
        assessment = self.refresh_object(assessment)
        self.assertEqual(assessment.status,
                         getattr(models.Assessment, test_state))
    ...
    """
        ...
        self.assertEqual(
            assessment.title.endswith("modified, change #1"), True)

        self.assertEqual(assessment.status, models.Assessment.DONE_STATE)

        self.api.modify_object(
            assessment, {"title": assessment.title + " modified, change #2"})
        assessment = self.refresh_object(assessment)
        self.assertEqual(
            assessment.title.endswith("modified, change #2"), True)
        self.assertEqual(assessment.status, models.Assessment.PROGRESS_STATE)

        assessment = self.change_status(assessment, assessment.VERIFIED_STATE,
        ...
        self.api.modify_object(
            assessment, {"title": assessment.title + "modified, change #3"})
        assessment = self.refresh_object(assessment)
        self.assertEqual(
            assessment.title.endswith("modified, change #3"), True)
        self.assertEqual(assessment.status, models.Assessment.PROGRESS_STATE)
    ...
    def test_modifying_person_custom_attribute_changes_status(self):
        ...
        self.api.modify_object(
            assessment, {"custom_attribute_values": custom_attribute_values})
        ...
        custom_attribute_values = [{
            ...
            ca_def.id,
            "attribute_value":
            "Person:" + str(another_person.id),  # make a change
        }]
        self.api.modify_object(
            assessment, {"custom_attribute_values": custom_attribute_values})
        ...
        custom_attribute_values = [{
            ...
            ca_def.id,
            "attribute_value":
            "Person:" + str(person_id),  # make a change
        }]
        self.api.modify_object(
            assessment, {"custom_attribute_values": custom_attribute_values})
