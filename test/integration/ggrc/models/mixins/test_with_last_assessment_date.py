...
class TestWithLastAssessmentDate(TestCase, WithQueryApi):
    ...
    def setUp(self):
        ...
        for objective in objectives:
            factories.RelationshipFactory(
                source=program, destination=objective)
        ...
        self.finished_dates = [
            datetime.datetime(2017, 2, 20, 13, 40, 0),
    ...
    def test_last_asmt_date_values(self):
        ...
        objectives = self._get_first_result_set(
            self._make_query_dict("Objective"), "Objective", "values")
    ...
    def test_last_asmt_date_filter(self):
        ...
        self.assertSetEqual({c["id"]
                             for c in controls_result["values"]},
        ...
        self.assertSetEqual({c["id"]
                             for c in objectives_result["values"]},
    ...
    def test_last_asmt_date_order(self):
        ...
        controls_sorted_manually = sorted(
            sorted(controls_unsorted, key=itemgetter("id")),
        ...
        objectives_sorted_manually = sorted(
            sorted(objectives_unsorted, key=itemgetter("id")),
            key=itemgetter("last_assessment_date"),
        )
