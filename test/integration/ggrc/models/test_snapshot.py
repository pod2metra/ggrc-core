...
class TestSnapshot(TestCase):
    ...
    def _create_cas():
        ...
        ca_model_names = [
            "facility", "control", "market", "section", "threat",
            "access_group", "data_asset"
        ]
        ca_args = [
            {
                "title": "CA text",
    ...
    def test_revision_content(self):
        ...
        self.assertNotEqual(
            facility_revision.content["custom_attribute_values"], [])

    def _get_object(self, obj):
        return self.client.get("/api/{}/{}".format(obj._inflector.table_plural,
                                                   obj.id)  # noqa # pylint: disable=protected-access
