...
class TestAssessment(TestAssessmentBase):
    ...
    def test_enabling_comment_notifications_by_default(self):
        ...
        self.assertEqual(
            sorted(recipients), ["Assignees", "Creators", "Verifiers"])
    ...
    def test_post_mapped_roles(self):
        ...
        for role in self.assignee_roles:
            ...
            self.assert_mapped_role("{} Mapped".format(role), person_email,
                                    audit)
    ...
    def test_import_mapped_roles(self):
        ...
        for role in [
                "Assignees Mapped", "Creators Mapped", "Verifiers Mapped"
        ]:
            for user in users:
    ...
    def test_document_mapped_roles(self):
        ...
        with factories.single_commit():
            ...
            factories.RelationshipFactory(
                source=assessment, destination=document)

        db.session.add(document)
        for role in [
                "Assignees Document Mapped", "Creators Document Mapped",
    ...
    def test_deletion_mapped_roles(self):
        ...
        response = self.api.put(
            assessment, {
                "access_control_list": [
                    acl_helper.get_acl_json(self.assignee_roles["Creators"],
                                            person.id)
                ]
            })
        self.assertEqual(response.status_code, 200)
    ...
    def test_deletion_multiple_assignee(self):
        ...
        for ac_role in self.assignee_roles.keys():
            ...
            self.assert_mapped_role("{} Mapped".format(ac_role), person_email,
                                    audit)
        ...
        with factories.single_commit():
            ...
            for ac_role_id in self.assignee_roles.values():
                factories.AccessControlListFactory(
                    ac_role_id=ac_role_id, person=person, object=assessment)
            factories.RelationshipFactory(source=audit, destination=assessment)
            snapshot = self._create_snapshots(audit,
                                              [factories.ControlFactory()])[0]
...
class TestAssessmentUpdates(ggrc.TestCase):
    ...
    def test_update_assessment_and_get_list(self):
        ...
        all_models.Assessment.query.filter(
            all_models.Assessment.id == self.assessment_id).update({
                all_models.Assessment.status:
                old_state,
            })
        db.session.commit()
        # required for populate cache
        content = self.api.client.get("/api/assessments?id__in={}".format(
            self.assessment_id))
        self.assertEqual(
            old_state,
        ...
        new_state = "In Review"
        self.api.put(
            all_models.Assessment.query.get(self.assessment_id),
            {"status": new_state})
        content = self.api.client.get("/api/assessments?id__in={}".format(
            self.assessment_id))
        self.assertEqual(
    ...
    @ddt.data(
        ...
        ("", "Control", "Plan - {}", "Plan - 0<br>Plan - 1<br>Plan - 2"),
