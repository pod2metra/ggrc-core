...
class TestAssessmentGeneration(TestAssessmentBase):
    ...
    def assert_assignees(self, role, response, *users):
        ...
        acl_people = all_models.Person.query.filter(
            all_models.Person.id.in_([
                a.get("person", {}).get("id") for a in acls
                if asmnt_roles.get(a.get("ac_role_id")) == role
            ]))
    ...
    def generate_acls(self, users, role):
        ...
        for person in people:
            factories.AccessControlListFactory(
                ac_role=role, object=self.audit, person=person)
    ...
    def test_mapped_roles_template(self):
        ...
        for obj in [self.audit, self.snapshot]:
            self.assert_mapped_role("Verifiers Mapped", "user1@example.com",
                                    obj)
            self.assert_mapped_role("Verifiers Mapped", "user2@example.com",
                                    obj)
            self.assert_mapped_role("Assignees Mapped", "user3@example.com",
                                    obj)
            self.assert_mapped_role("Assignees Mapped", "user4@example.com",
                                    obj)
            self.assert_mapped_role("Creators Mapped", "user@example.com", obj)
    ...
    def test_control_test_plan(self):
        ...
        response = self.assessment_post(template)
        self.assertEqual(response.json["assessment"]["test_plan"], "<br>".join(
            [template.procedure_description, test_plan]))
    ...
    def test_autogenerated_no_tmpl(self):
        ...
        with factories.single_commit():
            ...
            users = [
                factories.PersonFactory(email=e) for e in prince_assignees
            ]
            for user in users:
                factories.AccessControlListFactory(
                    person=user,
    ...
    def test_remap_doc_from_assessment(self, test_asmt_num):
        ...
        for num, asmt in asmts.items():
            ...
            update_data.append(
                collections.OrderedDict([
                    ...
                    ("Evidence File", ""
                     if num == test_asmt_num else evidences_str),
                ]))
        ...
        for num, asmt in asmts.items():
            ...
            if num == test_asmt_num:
                ...
                self.assertEqual([ev.link for ev in asmt.document_evidence],
                                 evidences)
    ...
    def test_generated_assessment_type(self, templ_type, obj_type, exp_type):
        ...
        response = self.assessment_post(
            template=template, extra_data=assessment_type)
        if exp_type:
    ...
    """
        ...
        self.assertEqual(asmt.status, getattr(all_models.Assessment,
                                              test_state))
        response = self.assessment_post(extra_data={
            "id": asmt.id,
            "design": "",
            "operationally": "",
            "notes": ""
