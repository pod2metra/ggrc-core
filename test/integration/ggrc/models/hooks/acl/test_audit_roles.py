...
class TestAuditRoleProgation(TestCase):
    ...
    def setup_people(self):
        ...
        for person in [
            ...
            self.people[person[0]] = factories.PersonFactory(email=person[1])
            rbac_factories.UserRoleFactory(
                role=creator_role, person=self.people[person[0]])
    ...
    def setup_objects(self):
        ...
        objects['audit'] = audit = factories.AuditFactory(
            ...
            access_control_list=[{
                ...
                "ac_role":
                self.audit_roles['Audit Captains'],
                "person":
                self.people['created_captain']
            }])
        # Assessment template
    ...
    def setup_mappings(self):
        ...
        for control in objects['controls']:
            factories.RelationshipFactory(
                source=objects['program'], destination=control)

        # Audit mappings:
        for obj in [
                objects['assessment_template'], objects['assessment'],
                objects['issue']
        ]:
            factories.RelationshipFactory(source=audit, destination=obj)

        # Assessment mappings:
        for obj in [objects['comment'], objects['document']]:
            factories.RelationshipFactory(
                source=objects['assessment'], destination=obj)

        # Issue mappings:
        for obj in [objects['issue_comment'], objects['issue_document']]:
            factories.RelationshipFactory(
                source=objects['issue'], destination=obj)
    ...
    def setUp(self):
        ...
        self.issue_roles = {
            ...
            for role in all_models.AccessControlRole.query.filter(
                all_models.AccessControlRole.object_type == "Issue").all()
        }
        self.people = {}
        self.objects = {}
    ...
    """
        ...
        roles_type_map = {
            self.audit_roles['Audit Captains']:
            defaultdict(lambda: self.audit_roles['Audit Captains Mapped']),
            self.audit_roles['Auditors']:
            defaultdict(
                lambda: self.audit_roles['Auditors Mapped'], {
                    "Assessment":
                    self.audit_roles['Auditors Assessment Mapped'],
            ...
            self.audit_roles['Program Readers Mapped']:
            defaultdict(lambda: self.audit_roles['Program Readers Mapped']),
            self.audit_roles['Program Editors Mapped']:
            defaultdict(lambda: self.audit_roles['Program Editors Mapped']),
            self.audit_roles['Program Managers Mapped']:
            defaultdict(lambda: self.audit_roles['Program Managers Mapped']),
        }
        ...
        for acl in audit.access_control_list:
            self.assertNotEqual(
                len(all_acls), 0, "No propagated acls created for {}".format(
                    acl.ac_role.name))
