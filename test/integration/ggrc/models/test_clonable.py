...
class TestClonable(SnapshotterBaseTestCase):
    ...
    """
        check_fields = [
            "template_object_type", "test_plan_procedure",
            "procedure_description", "default_people", "title"
        ]
        ...
        for s, d in zip(source.custom_attribute_definitions,
    ...
    def test_audit_clone(self):
        ...
        factories.RelationshipFactory(
            source=audit, destination=assessment_template_1)
        factories.RelationshipFactory(
            source=audit, destination=assessment_template_2)
        ...
        assessment_templates = audit_copy.related_objects(
            {"AssessmentTemplate"})

        self.assertEqual(len(assessment_templates), 2)

        assessment_template_1 = db.session.query(
            models.AssessmentTemplate).filter(
                models.AssessmentTemplate.title ==
                "test_audit_clone assessment_template_1").first()
        ...
        self.assertEqual(
            db.session.query(models.CustomAttributeDefinition).filter(
                ...
                assessment_template_1.id).count(),
            len(assessment_template_attributes_def))
    ...
    def test_asmnt_template_clone(self):
        ...
        with factories.single_commit():
            ...
            factories.RelationshipFactory(
                source=audit1, destination=assessment_template)
            for cad_type in [
        ...
        with factories.single_commit():
            for _ in range(3):
                ...
                for _ in range(3):
                    ...
                    factories.RelationshipFactory(
                        source=template, destination=control)
    ...
    def test_multiple_templates_clone(self):
        ...
        template_copies = models.AssessmentTemplate.query.filter(
            ~models.AssessmentTemplate.id.in_(template_ids)).order_by(
                models.AssessmentTemplate.title).all()
    ...
    def test_audit_clone_with_issue_tracker(self, mock_update_issue):
        ...
        with patch.object(
                issue_tracker, '_is_issue_tracker_enabled', return_value=True):
            self.api.modify_object(
                asmt, {
                    "issue_tracker": {
                        "enabled": True,
    ...
    def test_audit_clone_auditors(self):
        ...
        for auditor in auditors:
            factories.AccessControlListFactory(
                ac_role=auditor_role, object=audit, person=auditor)
        ...
        self.assertEqual(
            AccessControlList.query.filter_by(
                ac_role_id=auditor_role.id,
                object_type="Audit",
                object_id=audit_copy.id).count(), 3,
        ...
        factories.AccessControlListFactory(
            ac_role=auditor_role, object=audit, person=another_user_4)
    ...
    def test_audit_clone_custom_attributes(self):
        ...
        self.assertEqual(
            models.CustomAttributeValue.query.filter_by(
                ...
                attributable_id=audit_copy.id).count(), 1,
            "Custom Attribute weren't copied.")
    ...
    def test_audit_snapshot_scope_cloning(self):
        ...
        # snapshotted
        new_control = self.create_object(
            models.Control, {"title": "Test New Control On Program"})
        ...
        original_revisions = {(snapshot.child_type, snapshot.child_id):
                              snapshot.revision_id
                              for snapshot in snapshots}

        clone_revisions = {(snapshot.child_type, snapshot.child_id):
                           snapshot.revision_id
        ...
        self.assertEqual(
            db.session.query(models.Snapshot).filter(
                ...
                models.Snapshot.child_id == new_control.id).count(), 0,
            "No snapshots should exist for new control.")
