...
class TestIssueAuditMapping(TestCase):
    ...
    def setUp(self):
        ...
        with factories.single_commit():
            ...
            self.snapshot = factories.SnapshotFactory(
                parent=self.audit, revision=revision)
    ...
    def test_post_issue_with_audit_set(self):
        ...
        self.assertEqual(
            rel.query.filter(source_tuple == audit_tuple
                             and destination_tuple == issue_tuple
                             or destination_tuple == audit_tuple
                             and source_tuple == issue_tuple).count(), 0)
    ...
    def test_deny_mapping_to_two_audits(self):
        ...
        audit_stubs = [
            self.generator.create_stub(a) for a in self.other_audits
        ]
    ...
    def test_delete_audit_with_issue(self):
        ...
        self.issue_mapped = self.refresh_object(
            self.issue_mapped, id_=issue_id)
...
class TestIssueUnmap(TestCase):
    ...
    def setUp(self):
        ...
        with factories.single_commit():
            ...
            factories.RelationshipFactory(
                source=audit, destination=assessments[0])
            factories.RelationshipFactory(
                source=audit, destination=assessments[1])
            factories.RelationshipFactory(
    ...
    def test_issue_cascade_unmap(self):
        ...
        self.assertEqual(
            all_models.Relationship.query.filter_by(
                id=self.unmap_rel_id1).count(),
            0)
        ...
        audit_issue_rel = self.get_relationships(self.audit_id, "Audit",
                                                 self.issue_id, "Issue")
    ...
    def test_cascade_unmap_automapped(self):
        ...
        db.session.query(all_models.Relationship).update({
            "automapping_id": None
        })
        db.session.commit()
    ...
    def test_cascade_unmap_man_audit(self):
        ...
        all_models.Relationship.query.filter(
            all_models.Relationship.id.in_(audit_issue_rel.subquery())).update(
                {
                    "automapping_id": None
