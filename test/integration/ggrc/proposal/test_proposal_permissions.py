...
class TestPermissions(TestCase):
        ...
        with factories.single_commit():
            ...
            acrs = {
                ...
                factories.AccessControlRoleFactory(
                    name="ACL_Reader", object_type="Control", update=0),
                "ACL_Editor":
                factories.AccessControlRoleFactory(
                    name="ACL_Editor", object_type="Control"),
                "ACL_Nobody":
                factories.AccessControlRoleFactory(
                    name="ACL_Nobody",
                    object_type="Control",
            ...
            for role_name in ["Creator", "Reader", "Editor", "Administrator"]:
                rbac_factories.UserRoleFactory(
                    role=roles[role_name], person=self.people[role_name])
            for role_name in [
                    "Program Editors", "Program Managers", "Program Readers"
            ]:
                person = self.people[role_name]
                rbac_factories.UserRoleFactory(
                    role=roles["Creator"], person=person)
    ...
    def api_proposal_status_change(self, proposal_id, status):
        return self.api.put(
            all_models.Proposal.query.get(proposal_id),
            {"proposal": {
                "status": status
            }})
    ...
    """
        ...
        resp = self.api.client.post(
            "/query", data=json.dumps(data), headers=headers).json
