...
class TestProposalEmail(TestCase):
    ...
    def test_email_presentation(self, is_admin):
        ...
        with mock.patch(
                "ggrc.rbac.permissions.is_admin", return_value=is_admin):
    ...
    def test_email_sending(self):
        ...
        with factories.single_commit():
            ...
            role_1 = factories.AccessControlRoleFactory(
                object_type=control.type, notify_about_proposal=True)
            role_2 = factories.AccessControlRoleFactory(
                object_type=control.type, notify_about_proposal=True)
            role_3 = factories.AccessControlRoleFactory(
                object_type=control.type, notify_about_proposal=False)
            factories.AccessControlListFactory(
                ac_role=role_1, object=control, person=person_1)
            factories.AccessControlListFactory(
                ac_role=role_1, object=control, person=person_3)
            factories.AccessControlListFactory(
                ac_role=role_2, object=control, person=person_3)
            factories.AccessControlListFactory(
                ac_role=role_3, object=control, person=person_2)
