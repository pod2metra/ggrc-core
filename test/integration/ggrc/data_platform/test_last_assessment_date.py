  ...
  """
        ...
        with factories.single_commit():
            controls = [
                factories.ControlFactory(
                    slug="Control_{}".format(i), title="Control_{}".format(i))
                for i in range(5)
            ]
            objectives = [
                factories.ObjectiveFactory(
                    slug="Objective_{}".format(i),
                    title="Objective_{}".format(i)) for i in range(2)
            ]
            ...
            factories.RelationshipFactory(
                source=assessment_0, destination=audit_0)
            factories.RelationshipFactory(
                source=audit_1, destination=assessment_1)
    ...
    def test_snapshot_lad_on_new_audits(self):
        ...
        with freezegun.freeze_time(finish_date):
            ...
            for control in models.Control.query.all():
                factories.RelationshipFactory(
                    source=program, destination=control)
