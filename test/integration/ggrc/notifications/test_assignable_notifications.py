...
class TestAssignableNotificationUsingImports(TestAssignableNotification):
    ...
    def test_attaching_assessment_evidence_notifications(self, send_email):
        ...
        self.import_data(
            OrderedDict([
                ...
                (u"Code*", asmt.slug),
                (u"Evidence File",
                 u"https://gdrive.com/qwerty1/view evidence.txt"),
            ]))
    ...
    def test_assessment_state_change_notifications(self, send_email):
        ...
        self.assertRegexpMatches(content,
                                 ur"Assessment\s+has\s+been\s+started")
        ...
        query = self._get_notifications(
            notif_type="assessment_ready_for_review")
        self.assertEqual(query.count(), 1)
        ...
        for i, new_state in enumerate(Assessment.DONE_STATES):
            ...
            self.import_data(
                OrderedDict([
                    ...
                    (u"Code*", asmt_slug),
                    (u"Evidence File",
                     u"https://gdrive.com/qwerty{0}/view evidence-{0}.txt".
                     format(i)),
                ]))
    ...
    @unittest.skip("An issue needs to be fixed.")
    @patch("ggrc.notifications.common.send_email")
    def test_assessment_reopen_notifications_on_person_change(
            self, send_email):
...
class TestAssignableNotificationUsingAPI(TestAssignableNotification):
    ...
    """
        with freeze_time("2015-04-01"):
            ...
            revisions = Revision.query.filter(
                Revision.resource_type == 'Notification',
                Revision.resource_id.in_(
                    [notif.id for notif in notifications])).count()
            self.assertEqual(revisions, 6)
            ...
            self.api_helper.modify_object(
                asmt1, {"status": Assessment.PROGRESS_STATE})
            ...
            self.api_helper.modify_object(
                asmt6, {"status": Assessment.PROGRESS_STATE})
            ...
            self.assertEqual(self._get_notifications().count(), 1)
            self.api_helper.modify_object(
                asmt6, {"status": Assessment.VERIFIED_STATE})
            self.assertEqual(self._get_notifications().count(), 1)
            self.api_helper.modify_object(
                asmt6, {"status": Assessment.PROGRESS_STATE})
            self.assertEqual(self._get_notifications().count(), 1)
    ...
    """
        ...
        self.api_helper.modify_object(asmt, {"status": Assessment.DONE_STATE})
        asmt = Assessment.query.get(asmts["A 4"].id)
    ...
    """
        ...
        val2 = CustomAttributeValue(
            attribute_value="1a2b3", custom_attribute=cad2)
        ...
        self.assertEqual(
            self._get_notifications(notif_type="assessment_updated").count(),
            1)
        ...
        cad2 = CustomAttributeDefinition.query.filter(
            CustomAttributeDefinition.title == "CA 2").one()
        val2 = CustomAttributeValue(
            attribute_value="NEW", custom_attribute=cad2)
    ...
    """
        ...
        asmt4 = Assessment.query.get(asmts["A 4"].id)
        self.api_helper.modify_object(asmt4, {"status": Assessment.DONE_STATE})
        self.assertEqual(self._get_notifications().count(), 1)
    ...
    """
        ...
        self.api_helper.modify_object(relationship2,
                                      {"AssigneeType": "Assignees,Verifiers"})
        ...
        self.api_helper.modify_object(
            relationship2,
            {"AssigneeType": "Assignees"})  # not Verifier anymore
