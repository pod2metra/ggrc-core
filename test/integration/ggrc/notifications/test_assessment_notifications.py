...
class TestAssessmentNotification(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.primary_role_id = AccessControlRole.query.filter_by(
            object_type="Assessment", name="Primary Contacts").first().id

        cls.secondary_role_id = AccessControlRole.query.filter_by(
            object_type="Assessment", name="Secondary Contacts").first().id
    ...
    def setUp(self):
        ...
        factories.CustomAttributeValueFactory(
            custom_attribute=self.cad1, attributable=self.assessment)
        ...
        factories.CustomAttributeValueFactory(
            custom_attribute=self.cad2, attributable=self.assessment)
        ...
        factories.CustomAttributeValueFactory(
            custom_attribute=self.cad3, attributable=self.assessment)
    ...
    def test_multiply_updates(self):
        ...
        self.assertEqual(len(notifs), 1)
        self.assertEqual(
            sorted(updated[self.assessment.id]["updated_fields"]),
    ...
    def test_multiply_mapping(self):
        ...
        def get_relation_dict(destination_obj):
            return {
                "relationship": {
                    "context": {
                        "id": self.assessment.audit.context.id,
