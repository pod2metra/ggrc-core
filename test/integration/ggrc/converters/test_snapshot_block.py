...
class TestSnapshotBlockConverter(TestCase):
    ...
    def test_attribute_name_map(self):
        ...
        ac_roles = db.session.query(all_models.AccessControlRole.name).filter(
            all_models.AccessControlRole.object_type == "Control").all()
        expected_attrs += [("__acl__:{}".format(role[0]), role[0])
        ...
        self.assertEqual(block._attribute_name_map.items(), expected_attrs)
