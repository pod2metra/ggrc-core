...
class TestExportEmptyTemplate(TestCase):
    ...
    def test_basic_policy_template(self):
        ...
        response = self.client.post(
            "/_service/export_csv", data=dumps(data), headers=self.headers)
...
class TestExportSingleObject(TestCase):
    ...
    def test_program_audit_relevant_query(self):
        response = self._import_file(
            "data_for_export_testing_program_audit.csv")
        self._check_csv_response(response, {})
    ...
    def test_section_policy_relevant_query(self):
        ...
        for title in titles:
            if title in expected:
                self.assertIn(title, response.data,
                              "'{}' not found".format(title))
            else:
                self.assertNotIn(title, response.data,
                                 "'{}' was found".format(title))
    ...
    def test_query_all_aliases(self):
        def rhs(model, attr):
            ...
            if attr is not None and hasattr(attr, "_query_clause_element"):
                class_name = attr._query_clause_element(
        ...
        for model in set(get_importables().values()):
            for attr, field in AttributeInfo(model)._aliases.items():
                ...
                try:
                    field = field[
                        "display_name"] if type(field) is dict else field
...
class TestExportMultipleObjects(TestCase):
    ...
    def test_simple_multi_export(self):
        ...
        with factories.single_commit():
            ...
            regulations = [
                factories.RegulationFactory().title for i in range(3)
            ]
    ...
    def test_relevant_to_previous_export(self):
        ...
        res = self._import_file(
            "data_for_export_testing_relevant_previous.csv")
        self._check_csv_response(res, {})
        data = [
            {
            ...
            {
                "object_name":
                "Contract",  # contract-25, contract-27, contract-47
                "filters": {
                    "expression": {
                        "op": {
                            "name": "relevant"
        ...
        # policies
        for i in range(5, 25):
            if i in (7, 8, 9, 10, 19, 20) + (11, 12, 13, 14, 15, 16, 17, 18,
                                             19):
                self.assertIn(",Cheese ipsum ch {},".format(i), response.data)
            else:
                self.assertNotIn(",Cheese ipsum ch {},".format(i),
                                 response.data)
