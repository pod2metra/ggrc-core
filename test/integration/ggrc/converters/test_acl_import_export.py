...
class TestACLImportExport(TestCase):
    ...
    def test_single_acl_entry(self):
        ...
        acl = models.AccessControlList.query.filter_by(
            ac_role_id=role_id, object_id=market.id, object_type="Market")
    ...
    def test_acl_export(self):
        ...
        parsed_data = self.export_parsed_csv(search_request)["Market"]
        self.assertEqual(set(parsed_data[0][role_name].splitlines()), emails)
    ...
    @ddt.data({
        "Market": {
            "role name": set(_random_emails[:2])
        },
        "Facility": {
            "role name": set(_random_emails[2:])
        },
        ...
        "Control": {
            "role name": set(_random_emails[:3])
        },
        "Market": {
            "role name": set(_random_emails[2:])
        },
        "System": {
            "role name": set(_random_emails)
        },
        "Objective": {
            "role name": set()
        },
        "Product": {
            "role name": set(_random_emails[:1])
        },
        "Policy": {
            "other role name": set(_random_emails[:1])
        },
    })
        ...
        for object_type, roles in model_dict.items():
            ...
            for role_name in roles.keys():
                stored_roles[object_type][role_name] = {
                    ...
                    if acl.ac_role.name == role_name
                    and acl.ac_role.object_type == object_type
                }
