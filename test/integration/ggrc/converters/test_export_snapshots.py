...
class TestExportSnapshots(TestCase):
    ...
    def _create_cads(type_):
        ...
        with factories.single_commit():
            ...
            return [
                cad(title="date", definition_type=type_,
                    attribute_type="Date"),
                cad(title="checkbox",
    ...
    def test_full_control_export(self):
        ...
        control_dicts = {
            control.slug: {
                ...
                u"*" + control.slug,
                "Revision Date":
                unicode(snapshot.revision.created_at),
                "Description":
                control.description,
                "Effective Date":
                control.start_date.strftime("%m/%d/%Y"),
                "Fraud Related":
                u"yes" if control.fraud_related else u"no",
                "Frequency":
                control.verify_frequency.display_name,
                "Kind/Nature":
                control.kind.display_name,
                "Notes":
                control.notes,
                "Review State":
                control.os_state,
                "Significance":
                u"key" if control.key_control else u"non-key",
                "State":
                control.status,
                ...
                # Custom attributes
                "RT":
                self._get_cav(control, "RT"),
                "checkbox":
                self._get_cav(control, "checkbox"),
                "date":
                self._get_cav(control, "date"),
                "dropdown":
                self._get_cav(control, "dropdown"),
                "person":
                self._get_cav(control, "person"),
                ...
                u"creator@example.com",
                'Created Date':
                control.created_at.strftime("%Y-%m-%dT%H:%M:%S"),
                'Last Updated Date':
                control.updated_at.strftime("%Y-%m-%dT%H:%M:%S"),
                'Last Updated By':
                control.modified_by.email,
            }
    ...
    def test_snapshot_mappings_export(self):
        ...
        with factories.single_commit():
            ...
            for assessment, issue in zip(assessments, issues):
                ...
                factories.RelationshipFactory(
                    source=issue, destination=snapshot)
    ...
    """
        ...
        with QueryCounter() as counter:
            ...
            self.assertEqual(
                len(
                    self.export_parsed_csv(search_request)[
                        "Control Snapshot"]),
                1,
            )
