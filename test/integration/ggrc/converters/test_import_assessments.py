...
class TestAssessmentImport(TestCase):
  ...
  """
    ...
    def test_import_assessments_with_templates(self):
        ...
        values = set(
            v.attribute_value for v in assessment.custom_attribute_values)
    ...
    def _test_assessment_users(self, asmt, users):
        ...
        for user_name, expected_types in users.items():
            for role in expected_types:
                try:
                    user = models.Person.query.filter_by(
                    ...
                    self.assertEqual(acl_len, 1,
                                     "User {} is not mapped to {}".format(
    ...
    def _test_assigned_user(self, assessment, user_id, role):
        ...
        self.assertEqual([user_id] if user_id else [],
    ...
    """
        ...
        users = {"user 1": {"Assignees"}, "user 2": {"Assignees", "Creators"}}
        ...
        audit = [
            obj for obj in asmt_1.related_objects() if obj.type == "Audit"
        ][0]
        self.assertEqual(audit.context, asmt_1.context)

        evidence = models.Document.query.filter_by(
    ...
    """
        ...
        self.assertEqual({"a.b.com", "c d com"},
                         {i.title
                          for i in asmt1.document_url})
        self.assertEqual({u'evidence title 1'},
                         {i.title
                          for i in asmt1.document_evidence})
            ...
            "Assessment": {
                ...
                "block_warnings": {
                    errors.UNKNOWN_COLUMN.format(
                        line=2,
                        column_name=
                        "error description - non existing column will "
                        "be ignored"),
                    errors.UNKNOWN_COLUMN.format(
                        line=2, column_name="actual error message"),
                    errors.UNKNOWN_COLUMN.format(
                        line=2, column_name="map:project"),
                },
                "row_errors": {
                    errors.MISSING_VALUE_ERROR.format(
                        line=19, column_name="Audit"),
                ...
                "row_warnings": {
                    errors.UNKNOWN_OBJECT.format(
                        line=19, object_type="Audit", slug="not existing"),
    ...
    def test_import_view_only_field(self, value, is_valid):
        ...
        if not is_valid:
            row_warnings.append(
                u"Line 3: Field 'Archived' contains invalid data. "
    ...
    @ddt.data((False, "no", 0, 1, []), (True, "yes", 1, 0, [
        u'Line 3: Importing archived instance is '
    ...
    def test_import_archived_assessment(self, is_archived, value, ignored,
        ...
        resp = self.import_data(
            OrderedDict([("object_type", "Assessment"),
                         ("Code*", assessment.slug), ("archived", value),
    ...
    def test_update_non_changeable_field(self, field, value_creator):
        ...
        with factories.single_commit():
            ...
            factories.AssessmentFactory(
                slug=slug,
                modified_by=factories.PersonFactory(
        ...
        with freezegun.freeze_time("2017-9-10"):
            self.import_data(
                OrderedDict([("object_type", "Assessment"), ("Code*", slug),
                             (field, value)]))
