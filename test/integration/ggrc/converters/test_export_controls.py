...
class TestExportControls(TestCase):
    ...
    def test_search_by_owner_email(self):
        self.assert_slugs("Admin", self.basic_owner.email, [self.control.slug])

    def test_search_by_owner_name(self):
        self.assert_slugs("Admin", self.basic_owner.name, [self.control.slug])
    ...
    def test_search_by_new_owner(self):
        ...
        with factories.single_commit():
            ...
            factories.AccessControlListFactory(
                person=new_owner, object=self.control, ac_role_id=self.acr_id)

        self.assert_slugs("Admin", new_owner.email, [self.control.slug])
        self.assert_slugs("Admin", new_owner.name, [self.control.slug])
        self.assert_slugs("Admin", basic_email, [self.control.slug])
        self.assert_slugs("Admin", basic_name, [self.control.slug])
