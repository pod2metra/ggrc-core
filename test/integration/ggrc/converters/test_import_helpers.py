...
class TestCustomAttributesDefinitions(TestCase):
    ...
    def test_program_definitions(self):
        ...
        self.generator.generate_custom_attribute(
            "program", title="My Attribute")
        self.generator.generate_custom_attribute(
            "program", title="Mandatory Attribute", mandatory=True)
        self.generator.generate_custom_attribute(
            "program",
            title="Choose",
            mandatory=True,
...
class TestGetObjectColumnDefinitions(TestCase):
  ...
  """
    ...
    """
        ...
        definition_fields = {
            val["display_name"]
            for val in definitions.itervalues() if val.get(field_name)
        }
        self.assertEqual(definition_fields, expected)

    def _test_single_object(self,
                            obj_class,
                            names,
                            expected_fields,
                            has_mappings=True):
    ...
    def test_audit_definitions(self):
        ...
        names = {
            "Program", "Code", "Title", "Description", "Audit Captains",
            "State", "Planned Start Date", "Planned End Date",
            "Planned Report Period from", "Planned Report Period to",
            "Auditors", "Archived", "Delete", "Evidence URL", "Evidence File",
            'Created Date', 'Last Updated Date', 'Last Updated By', 'Folder',
            'Last Deprecated Date'
        }
        expected_fields = {
            "mandatory": {
    ...
    def test_assessment_definitions(self):
        ...
        expected_fields = {
            "mandatory": {"Title", "Audit", "Creators", "Assignees", "Code"},
            "unique": {
                "Code",
            },
            "ignore_on_update": {
