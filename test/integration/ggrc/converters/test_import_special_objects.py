...
class TestSpecialObjects(TestCase):
  ...
  """
    ...
    def test_program_roles_imports(self):
        ...
        manager_ids = [
            r.person_id for r in p1_roles
            if r.ac_role.name == "Program Managers"
        ]
        editor_ids = [
            r.person_id for r in p1_roles
            if r.ac_role.name == "Program Editors"
        ]
        reader_ids = [
            r.person_id for r in p1_roles
            if r.ac_role.name == "Program Readers"
        ]
        manager_emails = [
            p.email
            for p in Person.query.filter(Person.id.in_(manager_ids)).all()
        ]
        editor_emails = [
            p.email
            for p in Person.query.filter(Person.id.in_(editor_ids)).all()
        ]
        reader_emails = [
            p.email
            for p in Person.query.filter(Person.id.in_(reader_ids)).all()
        ]
