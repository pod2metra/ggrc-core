...
class TestAuditSnapshotQueries(TestCase, WithQueryApi):
    ...
    def test_snapshot_attribute_filter(self):
        ...
        content = result.json[0]["Snapshot"]["values"][0]["revision"][
            "content"]
        self.assertEqual(
            content["title"],
            market_title,
...
class TestIssueRelevantFilter(TestCase, WithQueryApi):
    ...
    def setUp(self):
        ...
        with factories.single_commit():
            ...
            factories.RelationshipFactory(
                source=self.issue, destination=self.control)
            factories.RelationshipFactory(
                source=self.issue, destination=self.audit)
            factories.RelationshipFactory(
                source=self.issue, destination=self.snapshot)
    ...
    @data(("Issue", "Control"), ("Control", "Issue"), ("Issue", "Snapshot"),
          ("Snapshot", "Issue"), ("Issue", "Snapshottable"),
          ("Snapshottable", "Issue"))
    @unpack
...
class TestSnapshotIndexing(TestCase, WithQueryApi):
    ...
    def test_process_network_zone(self):
        ...
        factories.RelationshipFactory(
            source=program, destination=process_nz_core)
        factories.RelationshipFactory(
            source=program, destination=process_nz_prod)
    ...
    def _add_owner(self, ownable_type, ownable_id, person_id):
        ...
        acr = all_models.AccessControlRole.query.filter_by(
            object_type=ownable_type, name="Admin").first()
    ...
    def _test_contact_field(self, contact_name_field, search_field):
        ...
        control_kwargs = {
            contact_name_field:
            factories.PersonFactory(name=person_name, email=person_email),
        }
        ...
        self.assert_rows_number_in_search(search_field,
                                          "negative_" + person_name, 0)
        self.assert_rows_number_in_search(search_field,
                                          "negative_" + person_email, 0)

    def test_control_owners(self):
        """Control Snapshots are filtered and sorted by Owners."""
        with single_commit():
            program = factories.ProgramFactory()
            controls = [factories.ControlFactory(), factories.ControlFactory()]
            people = [
                factories.PersonFactory(
                    name="Ann", email="email1@example.com"),
                factories.PersonFactory(
                    name="Bob", email="email2@example.com"),
                factories.PersonFactory(
                    name="Carl", email="email3@example.com")
            ]
            program_id = program.id
            ...
            factories.RelationshipFactory(
                source=program, destination=controls[0])
            factories.RelationshipFactory(
                source=program, destination=controls[1])
        ...
        control_revisions = models.Revision.query.filter_by(
            resource_type="Control").all()
        for c_rev in control_revisions:
            c_rev.content = models.Control.query.get(
                c_rev.resource_id).log_json()
        ...
        def owner_ids(snapshot):
            ...
            admin_role_id = models.AccessControlRole.query.filter_by(
                name="Admin",
                object_type=snapshot["revision"]["resource_type"]).first().id
        ...
        self.assertListEqual(
            [snap["child_id"] for snap in order_by_owners_result["values"]],
