...
class TestAdvancedQueryAPI(WithQueryApi, TestCase):
    ...
    def test_basic_query_in(self):
        ...
        programs = self._get_first_result_set(
            self._make_query_dict(
                "Program", expression=["title", "~", title_pattern]),
            "Program",
        )
        ...
        self.assertEqual(len(programs["values"]), programs["count"])
        self.assertTrue(
            all(title_pattern in program["title"]
    ...
    def test_basic_query_ne(self):
        ...
        programs = self._get_first_result_set(
            self._make_query_dict(
                "Program", expression=["title", "!=", title]),
            "Program",
        )
        ...
        self.assertEqual(len(programs["values"]), programs["count"])
        self.assertTrue(
            all(program["title"] != title for program in programs["values"]))
    ...
    def test_basic_query_not_in(self):
        ...
        self.assertEqual(len(programs["values"]), programs["count"])
        self.assertTrue(
            all(title_pattern not in program["title"]
                for program in programs["values"]))
    ...
    def test_basic_query_lt(self):
        ...
        self.assertTrue(
            all(
                datetime.strptime(program["start_date"], DATE_FORMAT_RESPONSE)
                < date for program in programs["values"]), )
    ...
    def test_basic_query_text_search(self):
        ...
        self.assertTrue(
            all((regulation["description"]
                 and text_pattern in regulation["description"]) or (
                     regulation["notes"]
                     and text_pattern in regulation.get("notes", ""))
    ...
    def test_basic_query_total(self):
        ...
        self.assertEqual(programs_no_limit["count"],
                         programs_no_limit["total"])
    ...
    def test_query_order_by(self):
        ...
        programs_default = self._get_first_result_set(
            self._make_query_dict("Program", order_by=[{
                "name": "title"
            }]),
            "Program",
            "values",
    ...
    def test_order_by_several_fields(self):
        ...
        expected = sorted(
            sorted(regulations_unsorted, key=itemgetter("title")),
    ...
    def test_order_by_related_titled(self):
        ...
        expected = sorted(
            sorted(audits_unsorted, key=itemgetter("id")),
            key=lambda a: program_id_title[a["program"]["id"]])

        self.assertListEqual(
            self._sort_sublists(audits_title), self._sort_sublists(expected))
    ...
    def test_query_order_by_owners(self):
        ...
        owner_role = all_models.AccessControlRole.query.filter(
            ...
            all_models.AccessControlRole.object_type == "Policy").one()
        policy_id_owner = {
            policy["id"]: person_id_name[[
                acl for acl in policy["access_control_list"]
                if acl["ac_role_id"] == owner_role.id
            ][0]["person_id"]]
            for policy in policies_unsorted
        }

        expected = sorted(
            sorted(policies_unsorted, key=itemgetter("id")),
    ...
    def test_filter_control_by_frequency(self):
        ...
        controls = self._get_first_result_set(
            self._make_query_dict(
                "Control", expression=["frequency", "=", "yearly"]),
            "Control",
        )
    ...
    def test_order_control_by_option(self, order_key, val_key):
        ...
        controls_unordered = self._get_first_result_set(
            self._make_query_dict("Control", ), "Control", "values")
    ...
    def test_filter_control_by_kind(self):
        ...
        controls = self._get_first_result_set(
            self._make_query_dict(
                "Control", expression=["kind/nature", "=", "Corrective"]),
            "Control",
        )
    ...
    def test_filter_control_by_means(self):
        ...
        controls = self._get_first_result_set(
            self._make_query_dict(
                "Control", expression=["type/means", "=", "Physical"]),
            "Control",
        )
        self.assertEqual(controls["count"], 3)
    ...
    def test_filter_control_by_key_control(self):
        ...
        controls = self._get_first_result_set(
            self._make_query_dict(
                "Control", expression=["significance", "=", "non-key"]),
            "Control",
        )
    ...
    def test_order_control_by_key_control(self):
        ...
        controls_ordered_2 = sorted(
            controls_unordered,
            key=lambda ctrl: (ctrl["key_control"] is None, ctrl["key_control"]),
            reverse=True)
        self.assertListEqual(
            self._sort_sublists(controls_ordered_1),
            self._sort_sublists(controls_ordered_2),
    ...
    def test_filter_control_by_fraud_related(self):
        ...
        controls = self._get_first_result_set(
            self._make_query_dict(
                "Control", expression=["fraud related", "=", "yes"]),
            "Control",
        )
        self.assertEqual(controls["count"], 2)
    ...
    def test_order_control_by_fraud_related(self):
        ...
        controls_ordered_2 = sorted(
            controls_unordered, key=lambda ctrl: ctrl["fraud_related"])
        self.assertListEqual(
            self._sort_sublists(controls_ordered_1),
            self._sort_sublists(controls_ordered_2),
        )
    ...
    def test_filter_control_by_assertions(self):
        ...
        controls = self._get_first_result_set(
            self._make_query_dict(
                "Control", expression=["assertions", "=", "privacy"]),
            "Control",
        )
        self.assertEqual(controls["count"], 3)
    ...
    @ddt.data("Regulation", "System", "Process", "Contract", "Policy",
              "Standard")
    ...
    def test_multiple_queries(self):
        ...
        data_list = [
            ...
            self._make_query_dict("Program", expression=["title", "~", "1"]),
            self._make_query_dict(
                "Program", expression=["title", "!=", "Cat ipsum 1"]),
            self._make_query_dict("Program", expression=["title", "!~", "1"]),
            self._make_query_dict(
                "Program", expression=["effective date", "<", "05/18/2015"]),
            self._make_query_dict(
                "Program", expression=["effective date", ">", "05/18/2015"]),
            {
        ...
        self.assertEqual(
            response_multiple_posts, response_single_post, sort_sublists=True)

    def test_is_empty_query_by_native_attrs(self):
        """Filter by navive object attrs with 'is empty' operator."""
        programs = self._get_first_result_set(
            self._make_query_dict(
                "Program", expression=["notes", "is", "empty"]),
            "Program",
        )
        self.assertEqual(programs["count"], 1)
        self.assertEqual(
            set([u'Cat ipsum 1']),
            set([program["title"] for program in programs["values"]]))

    @ddt.data(
        (all_models.Control, [
            all_models.Objective, all_models.Control, all_models.Market,
            all_models.Objective
        ]),
        (all_models.Issue, [
            all_models.Control, all_models.Control, all_models.Market,
            all_models.Objective
        ]),
    )
    @ddt.unpack
        ...
        with factories.single_commit():
            ...
            for relevant_obj in relevant_objects:
                factories.RelationshipFactory(
                    source=base_obj, destination=relevant_obj)
    ...
    @ddt.data(
        ...
        (all_models.Assessment, [
            all_models.Issue, all_models.Issue, all_models.Issue,
            all_models.Issue
        ]),
        (all_models.Issue, [
            all_models.Assessment, all_models.Control, all_models.Market,
            all_models.Objective
        ]),
    )
    @ddt.unpack
                ...
                if relevant_obj.type in Types.all:
                    related_obj = factories.SnapshotFactory(
                        ...
                        revision_id=models.Revision.query.filter_by(
                            resource_type=relevant_obj.type).first().id,
                    )
                factories.RelationshipFactory(
                    source=base_obj, destination=related_obj)
...
class TestSortingQuery(TestCase, WithQueryApi):
    ...
    """
        ...
        creators = [
            assignee for assignee, role in assignee_roles if role == "Creators"
        ]
        assignees = [
            assignee for assignee, role in assignee_roles
            if role == "Assignees"
        ]
        verifiers = [
            assignee for assignee, role in assignee_roles
    ...
    def test_sorting_assessments_by_assignees(self):
        ...
        self.assertListEqual(
            [ass["title"] for ass in assessments_by_verifiers],
        ...
        self.assertListEqual(
            [ass["title"] for ass in assessments_by_assessors],
            ["Assessment_2", "Assessment_1"])
...
class TestQueryAssessmentByEvidenceURL(TestCase, WithQueryApi):
    ...
    def test_query_evidence_url(self):
        ...
        self.assertItemsEqual(
            [asmt["title"] for asmt in assessments_by_evidence],
        ...
        self.assertItemsEqual(
            [asmt["title"] for asmt in assessments_by_url],
            ["Assessment title 1", "Assessment title 3", "Assessment title 4"])
...
class TestQueryWithCA(TestCase, WithQueryApi):
    ...
    def _flatten_cav(data):
        ...
        for entry in data:
            for cav in entry.get("custom_attribute_values", []):
                entry[cad_names[cav["custom_attribute_id"]]] = cav[
                    "attribute_value"]
        return data
    ...
    def _get_first_result_set(self, *args, **kwargs):
        ...
        return self._flatten_cav(
            super(TestQueryWithCA, self)._get_first_result_set(
                *args, **kwargs), )
    ...
    def test_ca_filter_by_person(self):
        ...
        programs = self._get_first_result_set(
            self._make_query_dict(
                "Program", expression=["ca person", "=", "three"]),
            "Program",
            "values",
        )
        titles = [prog["title"] for prog in programs]
...
class TestQueryWithUnicode(TestCase, WithQueryApi):
    ...
    @ddt.data(("title", u"программа A"), (CAD_TITLE3, u"Ы текст"))
    @ddt.unpack
