...
class TestServices(TestCase):
    ...
    def test_missing_resource_get(self):
        response = self.client.get(
            self.mock_url("foo"), headers=self.get_headers())
    ...
    def _prepare_model_for_put(self, foo_param="buzz"):
        ...
        response = self.client.get(
            self.mock_url(mock.id), headers=self.get_headers())
        self.assert200(response)
    ...
    def test_put_successful(self, _):
        ...
        self.assert200(response)
        self.assertNotEqual(original_headers["Last-Modified"],
                            response.headers["Last-Modified"])
        self.assertNotEqual(original_headers["Etag"], response.headers["Etag"])
        self.assertEqual("baz",
                         response.json["services_test_mock_model"]["foo"])
    ...
    def test_put_bad_request(self):
        ...
        url = urlparse(
            response.json["services_test_mock_model"]["selfLink"]).path
        response = self.client.put(
            url,
            content_type="application/json",
    ...
    def test_put_428(self):
        ...
        def check_response_428(response, missing_headers):
            ...
            self.assertSetEqual(
                set(header.strip() for header in error_message[len(
                    "Missing headers: "):].split(",")),
                set(missing_headers),
            )

        response_etag_and_date_missing = put_with_headers()
        check_response_428(
            response_etag_and_date_missing,
            missing_headers={"If-Match", "If-Unmodified-Since"})

        response_etag_missing = put_with_headers(
            ("If-Unmodified-Since", response.headers["Last-Modified"]), )
        check_response_428(response_etag_missing, missing_headers={"If-Match"})

        response_date_missing = put_with_headers(
            ("If-Match", response.headers["Etag"]), )
        check_response_428(
            response_date_missing, missing_headers={"If-Unmodified-Since"})
...
class TestFilteringByRequest(TestCase):
    ...
    def init_users(self):
        ...
        context = (db.session.query(
            all_models.Context).filter(all_models.Context.id != 0).first())
        user_role = (db.session.query(all_models.UserRole).join(
            all_models.Person).filter(
                and_(all_models.UserRole.person_id == all_models.Person.id,
                     all_models.Person.name == "john")).first())
