...
class TestPersonResource(TestCase, WithQueryApi):
    ...
    """
        ...
        with freeze_time("2017-10-16 08:09:10"):
            ...
            self.generator.modify_object(
                verified_tasks["task 1"], data={"status": "Finished"})
            response = self.client.get(
                "/api/people/{}/task_count".format(user_id))
            self.assertEqual(response.json, {
                "open_task_count": 4,
            ...
            self.generator.modify_object(
                verified_tasks["task 1"], data={"status": "Verified"})
            response = self.client.get(
                "/api/people/{}/task_count".format(user_id))
            self.assertEqual(response.json, {
                "open_task_count": 3,
            ...
            # tasks
            self.generator.modify_object(
                non_verified_tasks["task 1"], data={"status": "Finished"})
            response = self.client.get(
                "/api/people/{}/task_count".format(user_id))
            self.assertEqual(response.json, {
                "open_task_count": 2,
...
class TestPersonResourcePopulated(TestCase, WithQueryApi):
    ...
    @ddt.data(*[(user.id, user.email, user.name)
